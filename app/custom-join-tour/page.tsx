...
/** Ï§ëÏöî Íµ¨Ï†à??Î≥ºÎìúÎ°?Í∞êÏã∏???åÎçî (48 hours, 24 hours, cancelled and fully refunded ?? */
function GuaranteeBodyWithBold({ text }: { text: string }) {
  const phrases = ['48 hours', '24 hours', 'cancelled and fully refunded', '48?úÍ∞Ñ', '24?úÍ∞Ñ', 'Ï∑®ÏÜå?òÎ©∞ ?ÑÏï° ?òÎ∂à', '?ÑÏï° ?òÎ∂à'];
...
/** ?¨Ïù¥Î≤ÑÌéë??HUD ?§Ì???Î∞??ÑÏù¥ÏΩ?(Ï∏°Î©¥, Ï∞ΩÎ¨∏¬∑?§Îìú?ºÏù¥?∏¬∑Î∞î???? */
...
/** Format price in KRW for display: numeric (no "Îß?). Uses currency context for USD when selected. */
...
export default function CustomJoinTourPage() {
  ...
  /** ??Ï∞ΩÏóê???¥Ïóà????localStorage?êÏÑú ?ºÏ†ï Î≥µÏõê */
  useEffect(() => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || '?ºÏ†ï ?ùÏÑ±???§Ìå®?àÏäµ?àÎã§.');
        return;
      }
      setSchedule((data.schedule || []).map((d: DaySchedule, di: number) => ({
        ...d,
      ...
      const removedNotice = Array.isArray(data.removedPlaces) && data.removedPlaces.length > 0
        ? ` (?¥ÏòÅ Í∑úÏπô?ºÎ°ú ?úÏô∏???•ÏÜå: ${(data.removedPlaces as Array<{ name: string; reason: string }>).map((r) => `${r.name}`).join(', ')})`
        : '';
      ...
      setError(e instanceof Error ? e.message : '?ºÏ†ï ?ùÏÑ±???§Ìå®?àÏäµ?àÎã§.');
      setShowGenerateOverlay(false);
    } finally {
      setLoading(false);
    }
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        setError(data.error || '?ïÏ†ï Ï≤òÎ¶¨???§Ìå®?àÏäµ?àÎã§.');
        return;
      }
      setConfirmResult(data);
      setStep('confirmed');
    } catch (e) {
      setError(e instanceof Error ? e.message : '?ïÏ†ï Ï≤òÎ¶¨???§Ìå®?àÏäµ?àÎã§.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePropose = async () => {
    ...
    const title =
      schedule.length > 0 && schedule[0].places?.length > 0
        ? `${schedule[0].places[0].name || 'ÎßûÏ∂§'} ??${schedule.length}??
        : `ÎßûÏ∂§ ?¨Ïñ¥ ${schedule.length}??;
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Î∞úÏùò???§Ìå®?àÏäµ?àÎã§.');
      setProposedDone(true);
      router.push('/custom-join-tour/proposed');
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Î∞úÏùò???§Ìå®?àÏäµ?àÎã§.');
    } finally {
      setLoading(false);
    }
  };
  ...
  /** Format time "HH:mm" for display with localized AM/PM (e.g. "?§Ï†Ñ 09:00", "09:00 AM"). */
            ...
            >
              ...
              <div className="mb-5">
                <CustomSelect
                  ...
                  options={[
                    { value: 'ko', label: `?á∞?á∑ ${t('home.customJoinTour.guideKorean')}` },
                    { value: 'en', label: `?á∫?á∏ ${t('home.customJoinTour.guideEnglish')}` },
                    { value: 'zh', label: `?á®?á≥ ${t('home.customJoinTour.guideChinese')}` },
                    { value: 'ja', label: `?áØ?áµ ${t('home.customJoinTour.guideJapanese')}` },
                  ]}
                />
              </div>
        ...
        {step === 'itinerary' && (
          ...
          >
              ...
              {schedule.map((daySchedule, dayIndex) => (
                ...
                >
                  <div className="flex items-center gap-2 mb-3 pl-8">
                    ...
                    {dailyDistancesKm[dayIndex] != null && <span className="text-[11px] text-gray-400">?¥Îèô Í±∞Î¶¨ ??{dailyDistancesKm[dayIndex]} km</span>}
                  </div>
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                {(place as { type?: string }).type === 'restaurant' && (
                                  <span className="inline-flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded border border-amber-500/50 text-amber-400 shrink-0"><UtensilsCrossed size={10} /> ?ùÎãπ</span>
                                )}
                              </div>
            ...
            >
              ...
              {loading ? 'Í≤Ä??Ï§ë‚Ä? : t('home.customJoinTour.confirmItinerary')}
            </motion.button>
          </motion.div>
        )}

        {/* Step: Checkout ???¨Ïù¥Î≤??§ÌÅ¨ ?åÎßà */}
        ...
        {step === 'confirmed' && confirmResult && (
          ...
          >
            ...
            {confirmResult.pricing && (
              ...
              >
                {confirmResult.pricing.vehicleLabelKo} ¬∑ Ï¥?{formatPrice(confirmResult.pricing.totalPriceKrw)}
              </motion.div>
            )}
