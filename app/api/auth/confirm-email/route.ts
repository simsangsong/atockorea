 ...
 * accessToken은 필수이며 userId 본인 토큰이어야 합니다(N16).
 */
export async function POST(req: NextRequest) {
  try {
    ...
    // N16: accessToken is REQUIRED and must belong to `userId`. Previously the
    // ownership check only ran when a token happened to be supplied, so omitting
    // it let anyone force-confirm an arbitrary user's email (IDOR).
