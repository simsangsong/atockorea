...
export const TIMELINE_COPY: Record<RoomLocale, TimelineCopy> = {
  ...
  'zh-TW': {
    entry: '查看你的旅程紀錄',
    title: '你的旅程紀錄',
    summary: (s, p) => `${s} 個地點 · ${p} 張相片`,
    empty: '當你抵達每個地點並加入相片後，旅程紀錄就會慢慢完整。',
    stopsHeading: '你去過的地方',
    photosHeading: '你的相片',
    reviewCta: '撰寫評論',
    reviewHint: '請用你自己的話來寫 —— 我們絕不會代寫評論。',
    rewardProgressTitle: '有一份小獎勵在等你',
    rewardProgressBody: '抵達一個地點並至少加入一張相片，即可解鎖下次行程的優惠券。',
    rewardReadyTitle: '你的旅程紀錄完成了',
    rewardReadyBody: '領取一張感謝優惠券，下次行程就能使用。',
    claim: '領取獎勵',
    claiming: '領取中…',
    rewardDoneTitle: '獎勵已解鎖',
    rewardDoneBody: (code) => `下次預訂時輸入優惠碼 ${code}。`,
    rewardAlready: '你已領取過這份獎勵 —— 它就在你的帳戶裡。',
  ...
  fr: {
    ...
    title: 'Fil du voyage',
    summary: (s, p) => `${s} ${s === 1 ? 'étape' : 'étapes'} · ${p} ${p === 1 ? 'photo' : 'photos'}`,
    empty: 'Le fil du voyage se remplit à mesure que vous atteignez chaque étape et ajoutez des photos en chemin.',
    stopsHeading: 'Vos étapes',
    photosHeading: 'Vos photos',
    reviewCta: 'Laisser un avis',
    reviewHint: 'Partagez vos propres mots — nous n’écrivons jamais les avis à votre place.',
    rewardProgressTitle: 'Une petite récompense vous attend',
    rewardProgressBody: 'Atteignez une étape et ajoutez au moins une photo pour débloquer un coupon pour votre prochain tour.',
    rewardReadyTitle: 'Votre fil du voyage est complet',
    rewardReadyBody: 'Récupérez un coupon de remerciement pour votre prochain tour.',
    claim: 'Récupérer la récompense',
    claiming: 'Récupération…',
    rewardDoneTitle: 'Récompense débloquée',
    rewardDoneBody: (code) => `Utilisez le code ${code} lors de votre prochaine réservation.`,
    rewardAlready: 'Vous avez déjà récupéré cette récompense — elle est dans votre compte.',
  ...
  de: {
    ...
    title: 'Ihr Reiseverlauf',
    summary: (s, p) => `${s} ${s === 1 ? 'Stopp' : 'Stopps'} · ${p} ${p === 1 ? 'Foto' : 'Fotos'}`,
    empty: 'Ihr Reiseverlauf füllt sich, während Sie Stopps erreichen und unterwegs Fotos hinzufügen.',
    stopsHeading: 'Wo Sie waren',
    photosHeading: 'Ihre Fotos',
    reviewCta: 'Bewertung schreiben',
    reviewHint: 'Schreiben Sie in Ihren eigenen Worten — wir verfassen niemals Bewertungen für Sie.',
    rewardProgressTitle: 'Eine kleine Belohnung wartet',
    rewardProgressBody: 'Erreichen Sie einen Stopp und fügen Sie mindestens ein Foto hinzu, um einen Gutschein für Ihre nächste Tour freizuschalten.',
    rewardReadyTitle: 'Ihr Reiseverlauf ist komplett',
    rewardReadyBody: 'Sichern Sie sich einen Dankeschön-Gutschein für Ihre nächste Tour.',
    claim: 'Belohnung abholen',
    claiming: 'Wird abgeholt…',
    rewardDoneTitle: 'Belohnung freigeschaltet',
    rewardDoneBody: (code) => `Nutzen Sie den Code ${code} bei Ihrer nächsten Buchung.`,
    rewardAlready: 'Sie haben diese Belohnung bereits abgeholt — sie liegt in Ihrem Konto.',
  ...
  ru: {
    ...
    title: 'Лента вашего путешествия',
    summary: (s, p) => `Мест: ${s} · фото: ${p}`,
    empty: 'Лента путешествия заполняется по мере того, как вы добираетесь до остановок и добавляете фото в пути.',
    stopsHeading: 'Где вы побывали',
    photosHeading: 'Ваши фото',
    reviewCta: 'Оставить отзыв',
    reviewHint: 'Напишите отзыв своими словами — мы никогда не пишем отзывы за вас.',
    rewardProgressTitle: 'Вас ждет небольшой подарок',
    rewardProgressBody: 'Доберитесь до остановки и добавьте хотя бы одно фото, чтобы открыть купон на следующий тур.',
    rewardReadyTitle: 'Лента путешествия заполнена',
    rewardReadyBody: 'Заберите благодарственный купон на следующий тур.',
    claim: 'Забрать награду',
    claiming: 'Получаем…',
    rewardDoneTitle: 'Награда получена',
    rewardDoneBody: (code) => `Используйте код ${code} при следующем бронировании.`,
    rewardAlready: 'Эта награда уже получена — она в вашем аккаунте.',
  ...
  it: {
    ...
    title: 'Il tuo diario di viaggio',
    summary: (s, p) => `${s} ${s === 1 ? 'tappa' : 'tappe'} · ${p} foto`,
    empty: 'Il tuo diario si riempie man mano che raggiungi le tappe e aggiungi foto lungo il percorso.',
    stopsHeading: 'I posti che hai visitato',
    photosHeading: 'Le tue foto',
    reviewCta: 'Lascia una recensione',
    reviewHint: 'Racconta con parole tue — non scriviamo mai le recensioni al posto tuo.',
    rewardProgressTitle: 'C’è un piccolo premio che ti aspetta',
    rewardProgressBody: 'Raggiungi una tappa e aggiungi almeno una foto per sbloccare un coupon per il tuo prossimo tour.',
    rewardReadyTitle: 'Il tuo diario di viaggio è completo',
    rewardReadyBody: 'Ritira un coupon di ringraziamento per il tuo prossimo tour.',
    claim: 'Ritira il premio',
    claiming: 'Un attimo…',
    rewardDoneTitle: 'Premio sbloccato',
    rewardDoneBody: (code) => `Usa il codice ${code} alla tua prossima prenotazione.`,
    rewardAlready: 'Hai già ritirato questo premio — è nel tuo account.',
