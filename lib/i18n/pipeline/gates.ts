...
const NUMERAL_WORDS: Record<TargetLocale, Record<string, RegExp[]>> = {
  ...
  ru: {
    // 🔴 러시아어는 합성어에서 **사격 어간**을 쓴다 — `2-hour` → `двухчасовой`.
    //    수사가 낱말 속으로 들어가므로 어말 경계를 두면 못 잡는다(2026-07-26 실측:
    //    `The 2-hour stop` → `Двухчасовая остановка` 가 "숫자 소실"로 fail 이었다).
    //    `четыр`·`пят`·`шест`·`восьм`·`девят`·`десят` 은 이미 접두 매칭이라 합성형도 걸린다.
    //    어간이 따로 필요한 것만 아래에 더한다: 1 одно- · 2 двух- · 3 трёх- · 7 семи-.
    '0': [/\d\s*ч(?!\s*\d)/u],
    '1': [/(?<!\p{L})од(ин|на|но)(?!\p{L})/iu, /(?<!\p{L})перв/iu, /(?<!\p{L})одно(?=\p{L})/iu],
    '2': [/(?<!\p{L})дв[ае](?!\p{L})/iu, /(?<!\p{L})втор/iu, /(?<!\p{L})дву[хс]/iu],
    '3': [/(?<!\p{L})тр[еи][хй]?(?!\p{L})/iu, /(?<!\p{L})трет/iu, /(?<!\p{L})тр[ёе]х/iu],
    '4': [/(?<!\p{L})четыр/iu, /(?<!\p{L})четв[её]рт/iu],
    '5': [/(?<!\p{L})пят/iu], '6': [/(?<!\p{L})шест/iu],
    '7': [/(?<!\p{L})седьм/iu, /(?<!\p{L})семь(?!\p{L})/iu, /(?<!\p{L})семи(?=\p{L})/iu],
