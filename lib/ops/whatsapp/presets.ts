...
export const WA_PRESET_KEYS = [
  ...
  'cruise_d1', // 크루즈 쇼어 익스커션 D-1 (기항 스케줄 자동 프리필)
...
export const WA_PRESETS: WaPresetDefinition[] = [
  ...
  {
    // 크루즈 쇼어 익스커션 D-1 (사용자 확정 2026-07-27): 기항 스케줄
    // (`cruise_calls`)에서 {cruise_ship}/{cruise_arrive}/{cruise_return_by}가
    // 자동으로 채워진다. 세 가지가 핵심: ① 입항 시각 ② **크루즈명 확인 요청**
    // (배정 데이터와 손님 예약이 다르면 회신하게) ③ **출항 1시간 전 복귀 보장**.
    key: 'cruise_d1',
    label: '크루즈 D-1 (기항 자동)',
    timing: 'D-1',
    bodies: {
      en: L([
        'Hello {guest_name}, this is {operator}. 👋',
        '',
        'Your shore excursion for tomorrow — {tour_name} ({tour_date}):',
        '🛳️ Ship: {cruise_ship} — arriving {cruise_arrive}, departing {cruise_depart}',
        '📍 Meeting: {pickup_point}',
        '🕗 Pickup: {pickup_time}',
        '⏱️ Back-to-ship guarantee: we return you to the terminal by {cruise_return_by} — one full hour before your ship departs.',
        '🌤️ Weather: {weather}',
        '',
        '⚠️ If {cruise_ship} is NOT your ship, please reply here so we can adjust.',
        'Your tour room (live chat + return countdown): {room_link}',
        'See you at the pier! — {operator}',
      ]),
      ko: L([
        '안녕하세요 {guest_name}님, {operator}입니다. 👋',
        '',
        '내일 크루즈 쇼어 익스커션 안내 — {tour_name} ({tour_date}):',
        '🛳️ 선박: {cruise_ship} — {cruise_arrive} 입항 · {cruise_depart} 출항',
        '📍 만남: {pickup_point}',
        '🕗 픽업: {pickup_time}',
        '⏱️ 복귀 보장: 출항 1시간 전인 {cruise_return_by}까지 터미널로 모셔다 드립니다.',
        '🌤️ 날씨: {weather}',
        '',
        '⚠️ 탑승하시는 크루즈가 {cruise_ship}이(가) 아니면 이 메시지에 회신해 주세요.',
        '투어룸(실시간 채팅·복귀 카운트다운): {room_link}',
        '내일 부두에서 뵙겠습니다! — {operator}',
      ]),
      zh: L([
        '{guest_name} 您好，这里是 {operator}。👋',
        '',
        '明日岸上观光行程 — {tour_name} ({tour_date}):',
        '🛳️ 邮轮: {cruise_ship} — {cruise_arrive} 抵港 · {cruise_depart} 离港',
        '📍 集合: {pickup_point}',
        '🕗 接载: {pickup_time}',
        '⏱️ 返船保障: 我们保证在离港前 1 小时（{cruise_return_by}）将您送回码头。',
        '🌤️ 天气: {weather}',
        '',
        '⚠️ 如果您乘坐的不是 {cruise_ship}，请回复此消息以便我们调整。',
        '行程房间（实时聊天·返程倒计时）: {room_link}',
        '明天码头见! — {operator}',
      ]),
      'zh-TW': L([
        '{guest_name} 您好，這裡是 {operator}。👋',
        '',
        '明日岸上觀光行程 — {tour_name} ({tour_date}):',
        '🛳️ 郵輪: {cruise_ship} — {cruise_arrive} 抵港 · {cruise_depart} 離港',
        '📍 集合: {pickup_point}',
        '🕗 接載: {pickup_time}',
        '⏱️ 返船保障: 我們保證在離港前 1 小時（{cruise_return_by}）將您送回碼頭。',
        '🌤️ 天氣: {weather}',
        '',
        '⚠️ 如果您搭乘的不是 {cruise_ship}，請回覆此訊息以便我們調整。',
        '行程房間（即時聊天·返程倒數）: {room_link}',
        '明天碼頭見! — {operator}',
      ]),
      es: L([
        'Hola {guest_name}, somos {operator}. 👋',
        '',
        'Su excursión en tierra de mañana — {tour_name} ({tour_date}):',
        '🛳️ Barco: {cruise_ship} — llegada {cruise_arrive}, salida {cruise_depart}',
        '📍 Encuentro: {pickup_point}',
        '🕗 Recogida: {pickup_time}',
        '⏱️ Garantía de regreso: le devolvemos a la terminal antes de las {cruise_return_by}, una hora completa antes de que zarpe su barco.',
        '🌤️ Tiempo: {weather}',
        '',
        '⚠️ Si {cruise_ship} NO es su barco, responda a este mensaje para ajustarlo.',
        'Su sala de tour (chat en vivo + cuenta atrás de regreso): {room_link}',
        '¡Nos vemos en el muelle! — {operator}',
      ]),
      ja: L([
        '{guest_name} 様、{operator}でございます。👋',
        '',
        '明日の寄港地観光のご案内 — {tour_name} ({tour_date}):',
        '🛳️ 船: {cruise_ship} — {cruise_arrive} 入港 · {cruise_depart} 出港',
        '📍 集合: {pickup_point}',
        '🕗 ピックアップ: {pickup_time}',
        '⏱️ 帰船保証: 出港の1時間前（{cruise_return_by}）までにターミナルへお送りします。',
        '🌤️ 天気: {weather}',
        '',
        '⚠️ ご乗船のクルーズが {cruise_ship} と異なる場合は、このメッセージにご返信ください。',
        'ツアールーム（リアルタイムチャット・帰船カウントダウン）: {room_link}',
        '明日、埠頭でお会いしましょう! — {operator}',
      ]),
    },
  },
  {
