...
export function I18nProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    //    visitor whose localStorage/navigator says `ko` gets Korean client
    //    components — the header nav, the currency and login controls, the
    //    welcome-coupon popup, the sticky "check availability" CTA — layered
    //    over Traditional-Chinese or Japanese server copy. Measured 2026-08-07:
    //    every non-Korean product page rendered "투어 / AtoC 소개 / 고객지원 /
    //    장바구니 / 한국어 / 로그인" in its header.
