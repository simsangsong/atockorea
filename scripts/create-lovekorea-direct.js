/**
 * Direct script to create LoveKorea merchant account
 * This script reads from .env.local file
 * 
 * Usage: node scripts/create-lovekorea-direct.js
 * 
...
async function createLoveKoreaMerchant() {
  const email = 'lovekorea@lovekorea.com';
  ...
  try {
    ...
    if (existingUser) {
      ...
      // Delete existing merchant data
      const { data: oldMerchants } = await supabase
        .from('merchants')
        .select('id')
      ...
      if (oldMerchants) {
        await supabase.from('merchant_settings').delete().eq('merchant_id', oldMerchants.id);
    ...
    const { data: authData, error: authError } = await supabase.auth.admin.createUser({
      ...
      user_metadata: {
        ...
        needs_password_change: false, // Allow direct login with this password
    ...
    if (settingsError) {
      console.warn('⚠️  Warning: Could not create settings:', settingsError.message);
    } else {
      console.log('✓ Default settings created');
    }

    // 6. Create audit log (optional)
    console.log('\n6️⃣  Creating audit log...');
    await supabase
      .from('audit_logs')
      .insert({
        user_id: authData.user.id,
      ...
      .catch(() => {
        console.warn('⚠️  Warning: Could not create audit log (table might not exist)');
      });
    ...
    console.log('   🔑 Password: ', password);
    console.log('\n🌐 URLs:');
    console.log('   Login:      http://localhost:3000/merchant/login');
    console.log('   Dashboard:  http://localhost:3000/merchant');
    console.log('\n📝 Account Info:');
