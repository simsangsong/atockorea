...
(async () => {
  ...
  try {
    ...
    console.log(`   translations 필드 존재 여부:`, 'translations' in firstTour);
    console.log(`   translations 값:`, firstTour.translations);
    console.log('');
    
    if (!('translations' in firstTour)) {
      ...
      console.log('');
    } else if (firstTour.translations === null || firstTour.translations === undefined) {
      console.log('⚠️  translations 컬럼은 있지만 값이 null입니다.');
      console.log('   이것은 정상입니다. 빈 객체 {}로 초기화됩니다.');
    } else {
      console.log('✅ translations 컬럼이 존재하고 값이 있습니다!');
