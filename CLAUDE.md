...
## 🔴 진행 중 (최신): 스마트앱 **기능** 감사 — 잔여 5건

**이어받으면 이것부터:** **`docs/NEXT-SESSION-SMARTAPP-FEATURE-AUDIT-2026-08-04.md`**
(무엇부터 · 함정 넷 · 사장님 결정 · **§10 에 실행 프롬프트 통째로**)
...
**PR 10건 머지(#683~#692), 페이즈 F-1~F7 완주.**
기준선: tsc **0** · jest **5,784 pass / 0 fail**(539 스위트) · build **exit 0**.

🔴 **가장 큰 발견은 코드가 아니라 배달이었다.** 진짜 예약 18건 중 룸이 생긴 건 3건이고
그 3건 전부 **초대 0 · 참가자 0** — 기능 12개 중 11개가 실사용 0이고, DB 의 활동은
**전부 사장님 수동 테스트**(`manual-test-2026-07`)라 `sim_tag` 만으로는 안 보인다.
...
**남은 것(순서 고정):** K1a SSE 재연결(재현부터) → 자정 집합시각 엣지 → 동시 편집 →
F8 래칫 2종(⚠`unfilled-props` 49 는 **제외** — 결함 밀도 0이라 게이트가 꺼진다) →
"선언했는데 안 읽는 prop" 스캐너.

🔴 **이 트랙의 지배 결함이 한 겹 더 내려갔다 — 모듈이 아니라 필드다.**
`GuideConsole` 이 `participants` 를 **타입에만 선언하고 한 번도 안 읽어서**, 기사가 링크를
열었는지가 매 폴링마다 화면 코앞까지 와서 버려지고 있었다. `qa-caller-absence` 로는 못 잡는다
(컴포넌트는 렌더되고 필드도 페이로드에 실재한다). 게이트 `__tests__/audit/declaredButUnread.test.ts`.

🔴 **직전 세션에 내 진단이 세 번 틀렸고 세 번 다 실측이 잡았다**(부트스트랩 §7) —
K4 기사 커버리지 · 기사 D-1 조회 · unfilled-props 오독. **티켓은 *무엇이 문제인지*로만 읽고
처방은 그 자리의 코드를 읽고 다시 정하라.** 그리고 하니스가 낸 **거짓 실패 3건도 전부 하니스 잘못**이었다
(고정 sleep 이 "불러오는 중…" 을 찍음 · 칩 라벨 `+10분` · `getByRole` 부분일치).
**스크린샷 한 장이 매번 막았다.**

⚠ **로컬 `main` 이 14커밋 뒤처져 게이트를 헛쟀다** → 검증은 `git checkout -B x origin/main`.
