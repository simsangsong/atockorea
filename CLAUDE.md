...
## 🔴 진행 중 (최신): 스마트앱/관제 통합 트랙 — **X12·X10 완료, 다음은 X9**

**이어받으면 이것부터:** **`docs/NEXT-SESSION-SMARTAPP-2026-08-01.md`** (§4 순서 고정 · §9 실행 프롬프트).
**정본(왜):** `docs/ops-staff-design-unification-master-plan-2026-07-27.md` — **§G-3d/§G-3e 가 이번 결과.**
기준선: tsc **0** · jest **5,882 pass / 0 fail**(548 스위트) · build **exit 0**.

- **X12**(PR #701) — 돈 화면·확정 버튼에 처음 손이 닿았다. 🔴 패널의 전이 조건을
  **서버 `allowedExtraTransition` 과 직접 대조**한다(규칙의 두 번째 사본이 결함을 기른다).
  테스트를 쓰다 **전이 실패 시 목록 미갱신**을 잡았다 — 가이드가 서버가 부정한 금액을 계속 봤다
- **X10**(PR #703) — 🔴 **쓸어 담을 집합이 비어 있었다**(실측 236 컨트롤 / 5표면 / 덩어리 **0**)

🔴 **X1 · X7 · X10 — 세 번 연속으로 티켓의 처방이 틀렸고, X10 은 메커니즘까지 틀렸다**
(`min-width/min-height` 는 인라인 박스에 적용되지 않는다 — CSS 2.1 §10.4).
**티켓은 *무엇이 문제인지*로만 읽고, 숫자는 직접 다시 재라.**
