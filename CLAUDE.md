...
## 🔴 상시 규칙 — **Anthropic API 를 돌리지 마라. 작업은 Claude Code 가 플랜 한도 안에서 직접 한다.**

사장님 지시(오래된 것인데 **2026-08-07 까지 어디에도 기록돼 있지 않아 반복해서 어겼다**).

번역·요약·분류 같은 LLM 작업을 `ANTHROPIC_API_KEY` 로 API 에 던지지 마라. **그건 종량 과금이라
사장님 플랜과 별개 지갑에서 돈이 나간다.** 세션이 길어지면 **여러 세션으로 쪼개라** — 그게 지시다.
진척은 파일에 남으므로(예: `i18n-work/out/`) 다음 세션이 이어받는다.

🔴 **이 규칙이 세 번 뚫린 경로를 그대로 적어 둔다 — 같은 모양을 의심하라:**
① 세션이 길어 보이면 「스크립트로 자동화」하고 싶어진다 → 그 스크립트가 API 를 쓴다
(`scripts/i18n/translate.ts` 가 정확히 이 경로로 태어났다. 헤더에 「원래 서브에이전트에 물리도록
설계됐다」고 **자기 입으로 적어 두고** API 로 바꿨다).
② 그러면 실패가 `credit balance is too low` 로 나온다.
③ 그걸 **결함이 아니라 「사장님이 결제할 일」로 인수인계에 적는다** — 그 순간 규칙 위반이
「대기 중인 사람 게이트」로 세탁된다. 실제로 문서 3곳·메모리 3곳이 그렇게 적혀 있었다.

**「API 크레딧 대기」라고 적힌 인수인계를 보면 그건 대기가 아니라 이 규칙 위반이다. 직접 해라.**
새 스크립트에 `new Anthropic()`·`ANTHROPIC_API_KEY`·`GEMINI_API_KEY` 를 넣지 마라.
...
**문법 스윕 트랙: G0·G1·G4 완료 · G2 대부분 + G3 완료(2026-08-05, PR #725·726·727·730·732·733).**
**§1 시각 검수 6건 전부 실렌더로 끝났다** — 5건 정상, 1건 결함(서랍 검색 시각이 **기기** 로케일·
타임존으로 찍힘 → `formatBubbleTime` 으로 교체). 그 과정에서 결함 4건 더: **툼스톤 `key` 누락**
(`ChatFeed.tsx:722` — 툼스톤 있는 방마다 React 경고, 위치 기반 재조정 위험) · 명단에 DB 원값
`tour_product` 노출 · 승격 실패를 `.catch(()=>undefined)` 가 삼킴 · **`qa-cockpit-walk` 이 단일 룸에서
못 돌고 있었다**(→ 고치니 완주, **N5 콕핏 재질이 10스킨 전부 통과** — 「PIN 게이트로 판정 못 했다」 해소).
커버리지: **미방문 표면 2→0** · **rallyStage 0/5→5/5** · 히어로 그리드 216→440(로케일 `fr`+다크 스킨 2종) ·
판정불가 하니스 22→15. **G3 업그레이드 후보 11건**은 원장 맨 뒤(전부 실렌더에서 본 것).
🔴 **남은 것:** UX-006/007(실렌더 판정) · UX-010(조인 랜딩 막다른 길, 제품 결정) · UX-012(실패 UI) ·
P-18 TBT · 플랜 에디터 편집가능 변형 · 판정불가 15.
⚠ **워크트리는 HMR 이 안 먹는다 — 어젯밤에만 세 번 밟았다.** 고쳤으면 **dev 재시작 후** 검증하라.
부트스트랩 `docs/NEXT-SESSION-LOCAL-G2G3-2026-08-04.md`(§1 표는 이제 완료 기록으로 읽어라).
...
메시지 공유 **텍스트만**(`shareTimelineText` 재사용) · 서랍 **대화 검색**(→`focusMessageId` 점프) ·
**링크 프리뷰 텍스트만**(SSRF deny-by-class + og:image 의도적 제외) · 공지 승격(가이드, broadcast
재사용 서버 0줄) + 컴포저 이미지 붙여넣기. 격자 17/20 — **남은 ✗ 3(전달·피커·일괄저장)은 결정으로
닫힌 부재다. 다시 결함으로 올리지 말 것.**
**원장:** `docs/audit/UIUX-LEDGER-2026-08.md` — 발견 전부 + 재현 명령. **UX-000 을 먼저 읽어라.**
기준선(2026-08-04, PR #715 후): tsc **0** · jest **5,926 pass / 0 fail** · Vercel 빌드 green.
...
## ✅ 완료: 투어 카탈로그 개편 — **DB 반쪽까지 적용 끝** (2026-08-04)

**기록:** `docs/NEXT-SESSION-TOUR-CATALOG-2026-08-04.md`(§0′ 적용하며 드러난 것 · §2′ 검증 21항목)
레포는 PR #711·#714·#717·#718, DB 는 `2026-08-04-0{1..7}.sql` 7개 전부 적용 + `import-match-v18
--single` 6슬러그. 파일은 `applied/` 로 이동(pending 루트엔 2026-06-24 잔여 2건만, 무관).
**경주는 가시성 무변경을 전후 대조로 확인** — `is_active=false`·6로케일 `is_published=false`.

🔴 **적용은 첫 시도에 깨졌고, 그건 2026-06-24 와 똑같은 버그였다.**
`tour_product_pages.badges` 는 `text[]` 인데 **같은 이름의 `tours.badges` 는 jsonb** 라,
생성기 4개가 두 곳에 같은 `jsonb()` 헬퍼를 써서 페이지 쪽만 틀렸다. 그때는 **적용 스크립트가
메모리에서 SQL 을 고쳐** 넘어가고 **생성기는 안 고쳐서**, 두 달 뒤 같은 생성기가 또 뱉었다.
이번엔 **생성기를 고쳤고**(`textArray()` + 페이지 컨텍스트 5곳) **게이트를 붙였다** —
`__tests__/audit/pendingSqlColumnTypes.test.ts` + 스냅샷 `data/db-column-types.json`.
게이트 첫 실행이 레거시 1건을 즉시 찾아냈다(`applied/2026-06-24-07-…`, `KNOWN_LEGACY` 등록).
**기존 `schemaDrift` 는 테이블 이름만 본다 — 컬럼 타입은 아무도 안 봤다.**
**적용 시점의 우회는 수정이 아니다.**

🔴 **또 하나: 이 절은 오랫동안 「01~04 미적용」이라고 적어 둔 채로 틀려 있었다.**
...
교촌 남쪽 끝이 월정교 북단(도보 10~15분), 대릉원 서쪽 담장 건너가 황리단길. 박물관을 마지막에 둔 건
**입장 마감 17:30** 때문이고, 겨울엔 같은 시각이 일몰(17:15~17:30)이라 동궁과 월지 야경이 된다.
픽업/하차 3곳 전부 **지하철역 출구**로 못 박음(사장님 지시 — 부산역은 KTX 기차역 아님).
「경리단길」의 현지 명칭은 **황리단길**("황남동의 경리단길").

🔴 **하루가 10.5→11.5시간(하차 ≈19:50)이 됐다. 가격은 USD 39 그대로 — 조정은 사장님 결정.**
🔴 **이 상품은 지금 안 팔린다** — 경주 4종 전부 `CONSUMER_BLOCKED_TOUR_SLUGS`(Klook 12 SKU 축소).
생성 SQL은 `is_active`/`is_published`를 **일부러 제외**했다(무엇을 말하는지만 고치고 판매 여부는 안 건드림).
재오픈 = 블록리스트 제거 + SQL 끝 주석 블록, **두 쪽이 한 세트**.

✅ **DB 적용 완료(2026-08-04)** — `applied/2026-08-04-07-*.sql` + match 재동기화.
가시성은 적용 전후 대조로 무변경 확인(`is_active=false`·6로케일 `is_published=false`).

🔴 **적용 후 실렌더로 보니 소스만 읽어선 안 보이던 결함이 4건 더 있었다**(문서 §3-b):
리터럴 `**별표` 23곳 · 🔴 이모지가 손님 문구에 5곳 · 스톱6(대릉원&황리단길) **카드 사진 없음**
(from-scratch 로 만들어 `image` 를 아무도 안 채웠다 — 갤러리엔 이미 5장) · 황남빵 사진이
**교촌으로 캡션**(스톱 이동은 됐는데 doc 레벨 `galleryItems.location` 이 그대로).
**전부 원천에서 고치고 6로케일 재생성 → 실렌더 재확인.** 장치 2개 추가:
변환기 §18 표현-마크업 안전망 + 상설 게이트 `__tests__/audit/bundleUnrenderedMarkup.test.ts`.
🔴 **그 게이트가 훨씬 큰 걸 드러냈다 — 전체 번들 1,533건/165파일**(318건은 `**Walking** → Caminata`
꼴 LLM 번역 잔해, 170건은 번역기 메타노트 = PR #628 이 절반만 걷은 부채). **래칫으로 박아 둠, 별도 트랙감.**
⚠ **대조군이 두 번 내 판정을 뒤집었다** — `~17:30` 하차와 「East Jeju region」 날씨는
**경주 결함이 아니라 전 상품 공통 기존 결함**이다(§3-b 하단).

⚠ **코스만 고치는 작업이 아니었다 — 손님이 보던 오정보 11건이 같이 나왔다**(문서 §3):
박물관 **「월요일 휴관」이 거짓**(정기휴관 = 1/1·설날·추석뿐, `visitBasics`가 실제 렌더됨) · 픽업 시각이
스톱 본문과 아코디언에서 서로 다름 · 해운대를 1호선이라 함 · 교촌↔월정교를 「차로 10분」(실은 도보) ·
점심값이 한 페이지에서 ₩12,000~18,000 vs ₩15,000~25,000 · 은퇴한 자매 상품과 비교 · 신규 FAQ에 `id`가
없어 **아코디언 3개가 동시에 펼쳐짐** · 아코디언 `preview` 미갱신 · zh/zh-TW의 아홉산 표기 각 5가지.
**변환기가 이걸 다시 놓치지 않게 하는 장치 = ①시작 코스 가드 ②낡은 주장 안전망(`10.5`/`18:50`이
손님 화면에 남으면 throw, `_publication`만 예외) ③FAQ id 필수·중복 금지 ④`preview` 재생성
⑤`terminologyFixes` 명칭 통일.** 위 결함의 절반은 ②가 잡았다.

## ✅ 완료: 포천 투어 코스 개편 (2026-08-04, 사장님 지시 — 경쟁사 스크린샷 참조)

`pocheon-sanjeong-lake-herb-island-art-valley` — **허브아일랜드 내리고 재인폭포(연천) + 이동갈비 점심 추가**.
산정호수 10:00 → 점심 11:40 → 재인폭포 13:30 → 아트밸리 15:20. 픽업/하차는 **사장님 지정 2지점**
(홍대입구 8번출구 08:00 / 명동역 2번출구 08:30 → 명동 18:00 / 홍대 18:30) — 동대문 제외, 하루 창이
좁아져 전 스톱 재산정. 🔴 **슬러그는 일부러 그대로**(라이브 Klook SKU = URL·OTA 링크) — 이름에
herb-island 가 남아 있는 건 알고 남긴 것이다.
**서사 = 한탄강 유네스코 세계지질공원 지오사이트 2곳**(재인폭포=젊은 현무암 주상절리 / 아트밸리=오래된
화강암 기반암 「포천석」). 경쟁사가 놓친 축. ⚠ 세계지질공원 ≠ 세계유산 — FAQ 에 명시, 흐리지 말 것.
...
## 🔴 UI 불변 규칙: 손님 가로 레일의 스크롤바를 끄지 마라 (2026-08-07, 사장님 지적)

**가로로만 스크롤되는 컨테이너 위에서 마우스 휠을 세로로 굴리면 페이지가 스크롤된다 — 레일은
안 움직인다**(Chromium headless·headed 양쪽 실측). 화살표 버튼도 없고 레일은 `tabIndex=-1` 이다.
그래서 **스크롤바가 데스크톱의 유일한 어포던스**인데 22곳이 그걸 꺼 두고 있었다.
실측(라이브 1440px): `/tours/list` 선반이 **1748px**, 상품 페이지 추천 레일이 **588px** 을 숨김.

- 쓸 것: **`.rail-scrollbar`**(`app/globals.css`) — 터치에선 숨고, `(pointer: fine)` 에서만 얇게 보인다.
