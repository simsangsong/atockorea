...
남은 경계 — 🔴 **조인(다중 예약 한 차)은 기사 단독 불가**(`DriverConsole` 이 `rooms[0]` 만 소비,
"one party per day" 주석 = 의도된 경계. 해소는 T1v2) · 링크 잃은 손님의 현장 재합류 문이 스태프에게
없음(조인 QR 재발급 = capability URL 노출 트레이드오프, 사장님 결정) · 메시지 unsend 없음(3초 창뿐) ·
즉흥 정차 도착 카드 UI 미확인(서버는 임의 title 수용) · 원장 KRW 전용(환산 없음, 관제 rate 재사용 후보) ·
콘텐츠 생성 트리거가 plan confirm 이므로 **발송 절차에 "confirm 후 출발" 포함** 권고.
