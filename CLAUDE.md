...
## 🔴 진행 중(최우선): 스마트앱 / 관제 통합 트랙 — 열린 티켓 43

**다음 세션 부트스트랩:** `docs/NEXT-SESSION-SMARTAPP-2026-07-28.md` ← **이 트랙이면 이걸 먼저.**
단일 우선순위 · 작업 환경과 함정 5가지 · 첫 명령 · 머지 전 게이트가 거기 있다.
**마스터 플랜(SoT, 왜 그렇게 하는가):** `docs/ops-staff-design-unification-master-plan-2026-07-27.md` (Part A~L)

**1순위 5건만 옮겨 적는다** — 지난 두 라운드가 이걸 열어둔 채 UI 연마를 했다:
`K1` SSE 폴백 무한루프(실시간이 한 번만 흔들려도 손님 1명당 서버 함수 1개 영구 상주 + 2초마다 DB) ·
`L1` OTA 리스팅에 "투어 중 옵션·연장은 현장 현금" 한 줄(앱이 OTA 손님에게도 현금 청구를 만든다) ·
