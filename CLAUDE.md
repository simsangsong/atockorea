...
**직전 세션 기록:** `docs/SESSION-STATE-2026-07-30.md` (2026-07-30 야간 5라운드, PR #603~#607) ← **최신**
*(그 전: `docs/SESSION-STATE-2026-07-29B.md`, PR #579~#601)*
**🔴 지금 할 일의 정본:** `docs/OWNER-GATES-FOLLOWUP-2026-07-29.md` — 사장님 확인 6건의 조사 결과·초안·원장
**다음 세션 프롬프트:** `docs/NEXT-SESSION-PROMPT-2026-07-31.md` ← **이 트랙이면 이걸 복사해서 시작.**
**환경·함정·게이트:** `docs/NEXT-SESSION-SMARTAPP-2026-07-28.md`
...
**2026-07-30 기준 — [A]~[E] 완료(PR #603~#607).** 손님 사진·영수증 비공개화(출구 7개) ·
개인정보 `privacy.s16` 10로케일 · K4v2 Phase 0~1(가짜 통과 6건→진짜, 원장 54쌍) ·
**X13 손님 앱 763KB→447KB(−41%)** · N4 빈 채팅 · **X17 하루를 가져가기**(PR #609 — 사진 URL 0, 링크는 `reviewPolicy` 파생).

**남은 것:** 🔴 **[H] 플래너 전면 재편 P7** — 정본 `docs/planner-overhaul-p7-master-plan-2026-07-30.md`
(§A~§F 재리뷰 · §G 디자인 감사 실측 · §H 값 스펙 · §I 게이트 5종 · §J WBS 10단계).
**Part D 는 이제 역사다** — 무엇을 만들지는 저 문서. 🔴 **P2·P3·P4 는 이미 배포돼 있다**
(P0·P1 에 이어 세 번째). 그래서 "컴팩트하게"는 이미 실행됐고 불만이 남았다 = **처방이 틀렸다**.
실측 급소 4개: 사진 커버리지 **9.7% → 58.9%**(다른 컬럼을 읽는다) · 손님 **글자 크기·스킨이
플래너에 도달하지 않는다**(`useShellSurface` 미소비, 접근성 회귀) · **커밋 버튼 36px < 보조 44px**
(위계 역전) · **다크 승강 전부 `none`**.
· **K4v2 Phase 2~4**(실 LLM 비용) · [F] X15 Phase 1 · X18 · N3잔여 · N8 · X20 · [G] 5순위 전량.

🔴 **이번 라운드 교훈 5개(다음 사람이 같은 걸 또 하지 않도록):**
① **브라우저 워크가 "3로케일 통과"라고 해도 같은 로케일 3번일 수 있다** — 룸 언어는 기기 언어가 아니라
예약별 조인 선택이다. 로케일 검증은 렌더 테스트로.
② **`getComputedStyle`이 visible이어도 화면엔 없을 수 있다** — 포지션드 의사요소(`body::before`)가
통상 흐름 위에 칠해진다. DOM이 맞고 페인트가 틀린 경우는 스크린샷만 잡는다.
③ **측정 전에 포트가 비었는지 확인** — 안 죽은 이전 빌드 서버가 없는 CSS를 가리키는 HTML을 서빙해
수치가 두 배 튀었다. ⚠ Bash cwd 가 **메인 저장소로 드리프트**하기도 한다(거기 node_modules 손상).
④ 🔴 **디자인 문제로 보이는 것의 절반은 배선이다** — "사진이 없다"는 사진이 없는 게 아니라 **다른
컬럼을 읽는 것**이었고(9.7 vs 58.9%), "스킨이 안 먹는다"는 특이도가 아니라 **셸 계약 미소비**였다.
⑤ 🔴 **구조적으로 검수를 비껴가는 화면이 있다** — 플래너 편집기는 게이트가 둘이다(기본 시뮬은
고정 일정 상품 → 읽기전용 뷰, 새 프로필은 비-lead → 배너만). "봤다"는 사람이 다른 화면을 봤을 수 있다.
