...
export default function TourCardDetail({ tour }: Props) {
  ...
  return (
    <Link href={href} className="block">
      <article className="mx-auto mb-4 w-[90%] max-w-3xl cursor-pointer">
        <div
          className="
                  ...
                  <ol className="space-y-1.5">
                    {tour.schedule.slice(0, 4).map((item, index) => (
                      <li key={index} className="flex items-start gap-2">
                        ...
                        <div className="flex flex-col items-center">
                          ...
                          {index < Math.min(tour.schedule?.length ?? 0, 4) - 1 && (
                    ...
                    {tour.schedule.length > 4 && (
                      <li className="pt-1 text-[11px] text-[#6e6e73]">
                        + {tour.schedule.length - 4} more stops in the full itinerary
                      </li>
                    )}
                  </ol>
                </div>
