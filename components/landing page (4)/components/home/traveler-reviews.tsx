...
export function TravelerReviews() {
  ...
  useEffect(() => {
    // Add visible class to all review cards on mount
    ...
    <section className="py-14 md:py-20 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-10 md:mb-12">
          ...
          <p className="text-slate-600 text-[14px] md:text-[15px] font-medium">
            Real voices about fit, clarity, and pacing.
          </p>
        </div>
        ...
        <div ref={containerRef} className="space-y-3 md:space-y-4">
          {reviews.map((review, index) => (
            ...
            >
              {/* Left accent bar - Premium finish */}
              <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${review.color} opacity-90`} />
              
              {/* Header with Avatar and Rating - Improved hierarchy */}
              <div className="flex items-start justify-between mb-5 pl-1">
                <div className="flex items-center gap-3.5">
                  {/* Avatar - Refined sizing */}
                  <div className={`w-11 h-11 rounded-full ${review.color} flex items-center justify-center text-white text-xs font-bold shadow-sm shadow-${review.color}/20`}>
                    {review.initials}
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-slate-900">{review.name}</p>
                    <p className="text-xs text-slate-500 font-medium">{review.location} · {review.tour}</p>
                  </div>
                </div>
                {/* Star Rating - Refined */}
              ...
              {/* Quote - Clean sans-serif, no italics */}
              <p className="relative text-[15px] md:text-[17px] text-slate-700 leading-[1.6] mb-5 pl-1">
                <span className="text-slate-300 text-lg mr-0.5">"</span>{review.quote}<span className="text-slate-300 text-lg ml-0.5">"</span>
              </p>

              {/* Footer - Improved metadata hierarchy */}
        ...
        <div className="text-center mt-8">
          <Button
            variant="outline"
            className="border-slate-200/70 bg-white text-slate-700 hover:bg-slate-50 hover:border-slate-300 font-semibold px-6 py-2.5 rounded-xl transition-all shadow-sm hover:shadow-md inline-flex items-center gap-2"
          >
