    ...
    <div className="space-y-3 pb-4">
      {entries.map((room) => {
        ...
        return (
          <div
            key={room.id}
            className="rounded-2xl border border-[var(--tr-danger-soft)] bg-[var(--tr-danger-soft)] p-4 ring-1 ring-[var(--tr-danger-soft)]   "
            data-testid="ops-sos-card"
          >
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <p className="tr-body font-bold text-[var(--tr-danger)] ">
                  <span className="animate-pulse">🆘</span> {room.booking?.contact_name ?? '게스트'}
                  <span className="ml-1.5 font-normal text-[var(--tr-danger)] ">
                    {room.booking?.number_of_guests ?? 1}명 · {room.booking?.preferred_language ?? 'en'}
                  </span>
                </p>
                <p className="mt-0.5 truncate tr-label text-[var(--tr-danger)] ">{room.tour?.title ?? ''}</p>
              </div>
