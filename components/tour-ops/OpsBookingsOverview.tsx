      ...
      <div className="flex flex-wrap items-center gap-2">
        {(['week', 'month'] as const).map((k) => (
          <button
            ...
            aria-pressed={view === k}
            className={
              'text-cjk-safe flex min-h-[36px] items-center gap-1.5 rounded-full px-3 tr-label font-bold ' +
              (view === k
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-2">
            {(['confirmed', 'pending_review', 'unparsed'] as const).map((tier) => {
              ...
              return (
                ...
                >
                  <p className="tr-meta font-semibold text-[var(--tr-ink-3)]">{TIER_LABEL[tier]}</p>
                  {notConnected ? (
                    // B1-D6 — 숫자를 크게 쓰지 않는다. 0은 여기서 사실이 아니다.
                    <p
                      className="mt-1 flex items-center gap-1 tr-meta font-bold text-[var(--tr-warn,#b45309)]"
                    ...
                    <>
                      <p className="mt-0.5 tr-display font-bold tabular-nums text-[var(--tr-ink)]">{count}</p>
                      {tier === 'confirmed' && (
                        <p className="tr-meta text-[var(--tr-ink-3)]">{data.summary.confirmedGuests}명</p>
                      )}
                      {actionable && (
                        <p className="tr-meta font-semibold text-[var(--tr-accent)]" data-testid={`tier-${tier}-cta`}>
                          탭해서 처리
                        </p>
                      )}
                    </>
          ...
          {(['no_room', 'no_participant', 'no_seat'] as const).some((k) => data.summary.roomGaps[k] > 0) && (
            <div className="rounded-xl bg-[var(--tr-danger-soft)] p-2.5" data-testid="room-gaps">
              <p className="flex items-center gap-1.5 tr-meta font-bold text-[var(--tr-danger)]">
              ...
              <div className="mt-1.5 flex flex-wrap gap-1.5">
                {(['no_room', 'no_participant', 'no_seat'] as const)
                  ...
                  .map((k) => (
                    <span
                      key={k}
                      className="rounded-full bg-[var(--tr-surface)] px-2 py-0.5 tr-meta font-semibold text-[var(--tr-ink-2)]"
                      data-testid={`gap-${k}`}
                    >
          ...
          {data.records.length === 0 && (
            <p className="py-6 text-center tr-label text-[var(--tr-ink-3)]" data-testid="overview-empty">
              이 기간에는 예약이 없습니다.
            </p>
          )}
        </>
      ...
      {days.map((day) => {
        ...
        return (
          <div key={day} className="rounded-xl border border-[var(--tr-hairline)] bg-[var(--tr-surface)] p-2.5">
            <p className="tr-meta font-bold text-[var(--tr-ink-2)]">
              {day} <span className="font-normal text-[var(--tr-ink-3)]">· {rows.length}건</span>
            </p>
            {rows.length === 0 ? (
              <p className="mt-1 tr-meta text-[var(--tr-ink-3)]">—</p>
            ) : (
              <ul className="mt-1.5 space-y-1">
                {rows.map((r) => (
                  <li key={`${r.tier}-${r.id}`} className="flex items-center gap-1.5 tr-meta">
                    <span className="rounded-full bg-[var(--tr-surface-2)] px-1.5 py-0.5 tr-meta font-semibold text-[var(--tr-ink-3)]">
                      {r.channel}
                    </span>
                    ...
                    {r.reason && (
                      <span className="ml-auto truncate tr-meta text-[var(--tr-ink-3)]" title={r.reason}>
                        {r.reason}
                      </span>
                    )}
                  </li>
  ...
  return (
    <div data-testid="month-calendar">
      <div className="mb-2 flex items-center justify-between">
        ...
        <p className="tr-label font-bold text-[var(--tr-ink)]">{monthLabel}</p>
      ...
      <div className="grid grid-cols-7 gap-[3px] text-center">
        {['월', '화', '수', '목', '금', '토', '일'].map((d) => (
          <span key={d} className="tr-meta font-semibold text-[var(--tr-ink-3)]">
            {d}
          </span>
        ))}
        {valid &&
        ...
        {days.map((day) => {
          ...
          return (
            <button
              ...
              data-date={day}
            >
              <span className="block tr-meta text-[var(--tr-ink-3)] tabular-nums">{Number(day.slice(8))}</span>
              {rows.length > 0 && (
                <span className="block tr-meta font-bold tabular-nums text-[var(--tr-ink)]">
