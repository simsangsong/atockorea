...
> = {
  ...
  fr: {
    ...
    meeting: 'Rendez-vous',
    nextStops: 'Aujourd’hui',
    numbers: 'Urgences: 119 (secours) · 112 (police) · 1330 (interprète)',
    note: 'La reconnexion est automatique dès que le signal revient.',
    savedAgo: (a: string) => `(enregistré ${a})`,
    justNow: 'à l’instant',
  ...
  de: {
    title: '📴 Offline — gespeicherte Infos',
    meeting: 'Treffen',
    nextStops: 'Heute',
    numbers: 'Notruf: 119 (Rettung) · 112 (Polizei) · 1330 (Dolmetscher)',
    note: 'Sobald das Signal zurück ist, verbindet sich der Raum automatisch neu.',
    savedAgo: (a: string) => `(gespeichert ${a})`,
    justNow: 'gerade eben',
  ...
  ru: {
    title: '📴 Нет сети — сохраненная информация',
    meeting: 'Сбор',
    nextStops: 'Сегодня',
    numbers: 'Экстренные службы: 119 (скорая) · 112 (полиция) · 1330 (переводчик)',
    note: 'Когда сигнал вернется, комната переподключится автоматически.',
    savedAgo: (a: string) => `(сохранено ${a})`,
    justNow: 'только что',
  ...
  it: {
    title: '📴 Sei offline — info salvate',
    meeting: 'Ritrovo',
    nextStops: 'Oggi',
    numbers: 'Emergenze: 119 (soccorso medico) · 112 (polizia) · 1330 (interprete)',
    note: 'Quando torna il segnale, la stanza si riconnette da sola.',
    savedAgo: (a: string) => `(salvato ${a})`,
    justNow: 'proprio ora',
