...
import { IconAlarm, IconEventOn, IconEventOff } from '@/components/tour-mode/icons';
import {
  IconAdmission,
  IconArrived,
  IconExplore,
...
export default function ArrivalBundleCard({
  ...
  return (
    <div className="tr-stagger flex flex-col gap-2" data-testid="arrival-bundle-card">
      ...
      {!hasContent ? (
        <p className="tr-card-text px-1 font-semibold text-[var(--tr-ink)]">{arrivedLine}</p>
      ) : null}
      ...
      {meta.meeting_time ? (
        <div className="flex items-center gap-3 rounded-[var(--tr-radius-card)] border border-[var(--tr-hairline)] bg-[var(--tr-surface)] px-4 py-3">
          <IconAlarm size={TR_ICON.action} strokeWidth={TR_STROKE.default} aria-hidden className="shrink-0 text-[var(--tr-accent)]" />
          <div className="min-w-0 flex-1">
            <p className="tr-meta font-medium">{copy.meeting}</p>
            <p className="tr-body-lg tr-num font-bold leading-tight text-[var(--tr-ink)]">
              {meta.meeting_time}
              {point ? <span className="tr-card-text ml-2 font-medium text-[var(--tr-ink-2)]">{point}</span> : null}
            </p>
          </div>
          {meetingCoords ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="tr-pill tr-label flex shrink-0 items-center gap-1 px-3 py-1.5 font-semibold text-[var(--tr-ink)]"
            >
      ...
      <div className="flex flex-wrap gap-1.5">
        <span className="tr-pill tr-label inline-flex items-center gap-1.5 px-3 py-1.5 font-semibold text-[var(--tr-ink)]">
          {meta.follow_mode === 'follow' ? (
            <IconWalking size={TR_ICON.meta} strokeWidth={TR_STROKE.small} aria-hidden />
          ) : (
            <IconExplore size={TR_ICON.meta} strokeWidth={TR_STROKE.small} aria-hidden />
          )}
          {meta.follow_mode === 'follow' ? copy.follow : copy.free}
        </span>
        {meta.ticket_required ? (
          <span className="tr-pill tr-label inline-flex items-center gap-1.5 px-3 py-1.5 font-semibold text-[var(--tr-ink)]">
            <IconAdmission size={TR_ICON.meta} strokeWidth={TR_STROKE.small} aria-hidden />
            {copy.ticket}
            {typeof meta.ticket_krw === 'number' && meta.ticket_krw > 0 ? (
              <span className="tr-num font-bold">{formatTicketKrw(meta.ticket_krw)}</span>
            ) : null}
          </span>
        ) : null}
        {!meta.meeting_time && parkingCoords ? (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="tr-pill tr-label inline-flex items-center gap-1.5 px-3 py-1.5 font-semibold text-[var(--tr-ink)]"
          >
            <IconArrived size={TR_ICON.meta} strokeWidth={TR_STROKE.small} aria-hidden />
            {copy.parking}
          </a>
      ...
      {routeNote ? (
        <div className="flex items-start gap-2.5 rounded-[var(--tr-radius-card)] border border-[var(--tr-hairline)] bg-[var(--tr-surface)] px-4 py-3">
          <IconJourney size={TR_ICON.chip} strokeWidth={TR_STROKE.default} aria-hidden className="mt-0.5 shrink-0 text-[var(--tr-ink-2)]" />
          <div className="min-w-0">
            <p className="tr-meta font-medium">{copy.route}</p>
            <p className="tr-card-text whitespace-pre-line leading-relaxed text-[var(--tr-ink)]">{routeNote}</p>
          </div>
        </div>
      ) : null}
      ...
      {meta.event_status && (meta.event_label || meta.event_label_i18n) ? (
        ...
        >
          {meta.event_status === 'on' ? (
            <IconEventOn size={TR_ICON.chip} strokeWidth={TR_STROKE.default} aria-hidden className="shrink-0 text-[var(--tr-safe)]" />
          ) : (
            <IconEventOff size={TR_ICON.chip} strokeWidth={TR_STROKE.default} aria-hidden className="shrink-0 text-[var(--tr-ink-2)]" />
          )}
      ...
      {meta.next_leg ? (
        <div className="flex items-center gap-2.5 rounded-[var(--tr-radius-card)] border border-[var(--tr-hairline)] bg-[var(--tr-surface)] px-4 py-2.5">
          <IconTilePickup size={TR_ICON.chip} strokeWidth={TR_STROKE.default} aria-hidden className="shrink-0 text-[var(--tr-ink-2)]" />
          <p className="tr-card-text font-medium text-[var(--tr-ink)]" data-testid="arrival-next-leg">
