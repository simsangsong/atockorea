...
import RegionScriptDoor from '@/components/tour-mode/RegionScriptDoor';
...
> = {
  ...
  'zh-TW': { home: '首頁', chat: '聊天', map: '地圖', schedule: '今日', settings: '設定' },
...
const STOP_CARD_LABELS: Record<RoomLocale, { add: string; added: string; details: string }> = {
  ...
  'zh-TW': { add: '加入', added: '已加入', details: '詳情' },
...
const CONNECTION_HINT: Record<RoomLocale, { reconnecting: string; offline: string }> = {
  ...
  'zh-TW': { reconnecting: '重新連線中…', offline: '離線 — 重試中' },
...
export default function RoomShell({
   ...
   */
  regionScripts?: RegionScriptCard[] | null;
  /**
      ...
      >
        ...
        >
          ...
          <div className="relative mx-auto flex min-h-0 w-full max-w-2xl flex-1 flex-col">
            ...
            {/* 지역 공통 해설 — 일정 위에 얹는 문 하나. 아래 두 분기(리치 카드 /
                평문 타임라인)는 각자 스크롤 컨테이너를 갖고 있어 건드리지 않고,
                shrink-0 형제로 위에 붙인다.
                🔴 `undefined`(이 화면은 해설을 안 쓴다)와 `null`(아직 안 불러왔다)을
                구분한다. 안 불러온 동안 자리를 비워 두면 카드가 도착하는 순간
                일정 첫 장이 56px 아래로 튄다 — 그 판단은 컴포넌트가 한다. */}
            {tab === 'schedule' && regionScripts !== undefined && (
              <RegionScriptDoor
                cards={regionScripts}
                locale={locale}
