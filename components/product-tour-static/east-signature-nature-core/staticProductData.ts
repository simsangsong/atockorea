/**
 * 정적 전용 상세 본문 데이터 — API / DB / fetch 없음.
 * 카탈로그 필드·슬러그의 단일 출처: `../catalog/staticTourProductRegistry.ts`
 * 디자인·카피 기준: components/TourDetailV2 (v0 export)
 */

import { eastSignatureNatureCoreStaticProduct } from "../catalog/staticTourProductRegistry";
import { EAST_SIGNATURE_LIST_PRICE_USD } from "@/lib/tour-detail/east/constants";

export const EAST_SIGNATURE_NATURE_CORE_SLUG = eastSignatureNatureCoreStaticProduct.slug;

/** 상세 SEO·메타용 긴 설명 (등록 객체의 shortCardDescription과 구분) */
const EAST_SIGNATURE_LONG_DESCRIPTION =
  "A structured East Jeju route that begins with Jeju's stone and volcanic story, opens to the coast, builds toward Seongsan, and closes with Ilchulland and Seongeup Folk Village.";

export const eastSignatureNatureCoreProduct = {
  slug: eastSignatureNatureCoreStaticProduct.slug,
  title: eastSignatureNatureCoreStaticProduct.title,
  description: EAST_SIGNATURE_LONG_DESCRIPTION,
  hero: {
    imageUrl: eastSignatureNatureCoreStaticProduct.heroImage,
    imagePosition: "center 35%" as const,
    tagline: eastSignatureNatureCoreStaticProduct.subtitle,
    pills: eastSignatureNatureCoreStaticProduct.badges,
    meta: {
      duration: eastSignatureNatureCoreStaticProduct.duration,
      region: eastSignatureNatureCoreStaticProduct.region,
      stops: `${eastSignatureNatureCoreStaticProduct.stopsCount} stops`,
      rating: eastSignatureNatureCoreStaticProduct.rating,
      ratingStars: 4,
    },
  },
  /** 스티키·시드용 KRW — USD 표시는 CurrencyProvider의 실시간 환율로 변환 */
  price: {
    amountLabel: String(EAST_SIGNATURE_LIST_PRICE_USD),
    currency: "USD",
    per: "person",
  },
} as const;
...
export type GlanceItem = {
  label: string;
  value: string;
  icon: "camera" | "mountain" | "footprints" | "cloud-rain" | "users" | "scale" | "gauge";
};
...
export const glanceItems: GlanceItem[] = [
  ...
  { icon: "cloud-rain", label: "Rain Safety", value: "Medium" },
...
export const galleryItems: GalleryMediaItem[] = [
  ...
  },
  {
    ...
    location: "Ilchulland lava-tube garden",
    atmosphere: "Cool interior texture",
    alt: "Ilchulland lava-tube garden interior",
  },
  {
    id: 4,
    type: "photo",
...
export const itineraryStops: ItineraryStop[] = [
  ...
  },
  {
    ...
    smartNotes: {
      ...
      facilities: "Limited facilities—restrooms at entrance.",
  ...
  },
  {
    ...
    timeUsed: ["Arrival and seating", "Meal and short rest", "Departure preparation for Seongsan"],
    whyOnRoute: "Placing lunch here keeps the Seongsan stop more comfortable and less rushed.",
    visitBasics: {
      hours: "Restaurant hours",
      closed: "Varies",
      admission: "Included or à la carte",
      walking: "Minimal",
    },
    convenience: {
...
export const routeShapeIntro = {
  title: "How this day moves",
  subtitle: "Stone to coast, lunch, then crater to village. 09:00 to 17:00.",
} as const;

export const whyTourWorks = {
  bestFor: ["First-time visitors, couples", "Adult families", "Travelers who like context", "Active seniors"],
  lessIdeal: ["Avoiding stairs or caves", "Stroller-heavy groups", "Very young children"],
  routeLogicSections: [
    {
...
export const practicalAccordionItems: PracticalAccordionItem[] = [
  {
    id: "pickup",
...
export const practicalWeatherStatic = {
  today: { temp: "11°", label: "Today · Clear" },
  tomorrow: { temp: "18°/13°", label: "Tomorrow · Drizzle" },
} as const;
...
export const bookingSupportSteps = [
...
export type StaticQuestion = { id: string; question: string; answer: string };
...
/**
 * 공개 리뷰가 아직 없을 때의 기본 요약 — 실제 집계는 `assembleTourProductReviews`가
 * Supabase `reviews`에서 조립해서 주입. UI는 `totalReviews === 0`을 empty state로 처리.
 */
export const reviewsSummary = {
  ...
  highlights: [] as Array<{ label: string; count: number }>,
} as const;

export const guestReviews: Array<{
  id: number;
  author: string;
  avatar: string;
  location: string;
