 ...
 *
 * EN strings hardcoded for now — moved to `messages/<locale>.json` in
 * Phase 3 i18n task (auto-translate pipeline TBD).
 */
...
const CARDS: ReadonlyArray<RegionCard> = [
  {
    slug: "busan",
    name: "Busan",
    tagline: "Sea temples + ancient capitals + market crawls",
    stopsLabel: "20 curated stops",
    imageSrc: "/images/destinations/busan-card.jpg",
  },
  {
    slug: "jeju",
    name: "Jeju Island",
    tagline: "UNESCO Triple Crown — sunrise peaks, lava caves, coastal drives",
    stopsLabel: "25 curated stops",
    ...
    <section className="section-py-md relative overflow-hidden bg-slate-50 px-4 md:px-6">
      <div ref={blockRef} className="relative mx-auto max-w-6xl scroll-animate">
        <header className="mb-8 text-center md:mb-11">
          <p className="mb-3 text-eyebrow text-amber-700 md:mb-4">
            Custom itinerary builder
          </p>
          <h2 className="mb-3 text-balance text-display text-slate-900">
            Or build your own Korea day
          </h2>
          <p className="mx-auto max-w-2xl text-h3 font-medium text-slate-600">
            Pick the stops you actually want on the map. We'll quote it.
          </p>
        </header>
            ...
            >
              <div className="relative aspect-[16/10] overflow-hidden bg-slate-100">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-5">
                  ...
                  <p className="mt-1 text-sm text-white/90 drop-shadow">{card.tagline}</p>
                </div>
              </div>
              <div className="flex items-center justify-between p-5">
                <span className="text-sm font-semibold text-slate-700">Browse the map</span>
        ...
        <p className="mt-6 text-center text-xs text-slate-500 md:mt-8">
          Seoul + DMZ rollout planned after MVP.
        </p>
      </div>
    </section>
  );
