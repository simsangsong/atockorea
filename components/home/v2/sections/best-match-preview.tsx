...
import { toTourProductPageLocale, toTourProductBaseLocale } from "@/lib/tour-product/tourProductPageLocale";
...
export function BestMatchPreview() {
  ...
  const resultVm = useMemo(() => {
    if (matchResult) {
      return buildV2BestMatchResultViewModelFromApi(matchResult, (key: string) => t(key), toTourProductBaseLocale(locale));
    }
    return buildV2BestMatchResultViewModel((key: string) => t(key), toTourProductBaseLocale(locale));
  ...
  const meta = useMemo(() => {
    ...
    if (matchResult?.winner?.product_id) {
      const p = getStaticTourProductBySlug(matchResult.winner.product_id, toTourProductBaseLocale(locale));
      if (p) {
        return {
    ...
    const idle = getFeaturedJoinTourProduct(toTourProductBaseLocale(locale));
    return {
