...
| **Last commit touching this track** | (pending) — feat(builder): AI panel → slim trigger + tap-to-open dialog (post-launch; §C 2026-06-23 #3) |
| **Owner** | simsangsong |
| **Reviewers** | — |
...
| 2026-05-20 | V13 | **Tone-alignment renewal track (post-V2).** The V2 redesign shipped in the amber/ivory era (opaque `bg-slate-50` page/section surfaces, `bg-white ring-slate-200` cards). The rest of the site has since settled into the **site-native tone**: pastel-mesh `body::before` background + translucent frosted white surfaces (`bg-white/85–95 backdrop-blur-md` + thin `border-white/80` or `border-slate-200/70` + soft layered shadows) + slate-900 charcoal — see home v2, `/tours/list`, BottomNav, Header. **Reverses the surface assumptions of §G.1 / Phase 1 (slate-50 sections, white cards):** builder pages now go `main → bg-white/55`, sections → transparent (mesh shows in gutters), panels/cards → frosted white. **Amber is PRESERVED but re-scoped to functional wayfinding only** (photo-pin sequence badges, route polyline `#f59e0b`, timeline sequence nodes + dashed connector, AI seq chips, eyebrows) — this strengthens V5 (amber = sequence identity, not surface decoration). No layout/paradigm change; no schema change (V8/G.8 intact); no new deps (V11). | User directive 2026-05-20: "사이트 톤 통일 + 기능 amber 보존". Builder was the last page still in the pre-settlement amber/ivory tone, visually out of sync with the rest of the site. |
| 2026-05-21 | V14 | **Amber sequence identity moves to the MAP only — revises V5/V13's "timeline sequence nodes + dashed connector = amber".** Post-V2 user QA (richness track) found the amber "sticker" sequence node + dashed-amber connector + ivory/amber AI-result wash + `Sparkles` icons on the result/AI cards read tacky vs. the main-page / tour-detail premium tone. Decision: timeline + AI-result cards adopt the tour-product detail node (white `01`·`02` + soft layered shadow) + thin slate connector + pure-white float; the AI result stripe drops its `bg-amber-50/40` wash + amber Apply pill (→ slate-900); preset chips + count badge + empty-state de-ambered; `Sparkles` removed. **Amber stays a functional wayfinder but ONLY on the map** (photo-pin sequence badges + route polyline). This *narrows* the amber surface to one home (the geographic canvas) rather than reversing the "amber = sequence identity" principle. Executed in the richness sub-track — see `itinerary-builder-result-timeline-richness-master-plan-2026-05-21.md` §B RR8/RR9 + Phase R4. | User 2026-05-21 directive + a 3-way screenshot pick ("Variant 1 — tour-detail parity"). The amber-on-cards tone was the last thing making the builder result read differently from the rest of the site. |
| 2026-05-29 | V15 | **Result map shows ONLY the itinerary's in-route pins (1:1 with the timeline sequence). Reverses V2's "out-of-cart POIs keep a small slate dot" + drops `MarkerClusterer` entirely.** Out-of-route catalog POIs were fed into `MarkerClusterer` alongside the route pins; its default renderer drew blue circles with a *count* (2, 3, …) that the user read as bogus stop numbers, and even absorbed the real route pins into clusters — so map pins did NOT correspond 1:1 to the generated itinerary. New rule: only the in-route POIs are pinned (each with its `#seq` badge), no clustering (a day's route is a handful of stops; the existing <80m pin-offset keeps near-coincident stops readable). Map-tap discovery of uncarted POIs is dropped on purpose — browse/add still lives in `POICatalogGrid`; "show on map" from the grid still opens the InfoWindow by coords (no marker needed). | User report 2026-05-29 (screenshot: duplicate-numbered blue clusters over Jeju). 1:1 map↔timeline correspondence is the whole point of the geographic axis; cluster count badges actively broke it. |
...
| Date | Commit | Change |
|---|---|---|
| 2026-06-23 | (pending) | **AI panel → slim trigger + tap-to-open dialog (user 2026-06-23).** The always-open AI recommend *card* (eyebrow + intro + preset chip row + intent input + submit, ~5 stacked sections) was replaced with a single slim trigger button. Tapping opens a dialog (bottom sheet on mobile, centered card on md+) that leads with a "type anything" guide line, a free-text `<textarea>`, then the 5 example-prompt chips (still multi-select; matcher still uses each chip's seed intent + the user's free text via `buildCombinedIntent`). A successful match closes the dialog and the itinerary lands in `ResultTimeline` below (Phase 11 D28 auto-apply + Phase 12 D34 smooth-scroll preserved); the trigger button then shows the one-line result summary (`resultsSummary` + `appliedToTimelineHint`) and re-opens the dialog to refine. Empty-result + error feedback retained (inline pill under the button / inside the dialog). a11y: `role="dialog"` + `aria-modal` + `aria-labelledby`, Escape-to-close, `aria-haspopup`/`aria-expanded` on the trigger. Kept the Phase 11 D29 near-white **mint** identity (emerald icon chip + ring) in compact form — no amber added (V5/V14 intact). `collapsed` state + the `motion`/reveal wrappers removed; no new deps (V11). **i18n:** 6 new keys added to all 6 locales under `itineraryBuilder.ai` — `triggerTitle`, `triggerSub`, `dialogTitle`, `dialogGuide`, `examplesLabel`, `dialogClose` (hand-added per the translate-script-needs-API-key constraint; eyebrow/intro/intentLabel/getAnother now orphaned but harmless). tsc clean; `AIRecommendPanel.gate.test.ts` (isSameKeySet) still 6/6 green. Files: `AIRecommendPanel.tsx`, `messages/{en,ko,ja,zh,zh-TW,es}.json`. |
| 2026-06-23 | (committed) | **Spacing follow-up (user 2026-06-23).** `BuilderShell` page-placement map column `mb-3` → `mb-5` so the framed mobile map and the below-map trip card have clear breathing room (were nearly touching). |
| 2026-06-23 | (pending) | **Post-launch UI/UX polish #2 (user review 2026-06-23, 3 mobile screenshots) — 4 fixes on the page-placement `/itinerary-builder` surface.** (1) **Trip-summary bar relocated below the map + made prominent (`PlannerTopRail` + `BuilderShell`):** the mobile collapsed summary strip ("Busan · 2p · 한국어 · 8h") used to sit as a thin sticky bar ABOVE the map; user wanted it below the map and eye-catching. Added a `slot?: "all" \| "bar" \| "mobile"` prop to `PlannerTopRail` so the page placement renders the desktop bar above the map (`slot="bar"`) and a separate prominent mobile "edit your trip" card below the map (`slot="mobile"`) — dark slate-900 `SlidersHorizontal` icon chip + eyebrow + bold summary + chevron, rounded-2xl card with layered shadow + ring, `active:scale` tap feedback. Both instances read URL state so they stay in sync (no shared local state). Home embed keeps `slot="all"` (unchanged). (2) **Framed mobile map (`BuilderShell` mapColumnClass/mapInnerClass + glaze):** map was full-bleed (`-mx-4`) + square (`md:rounded-2xl` only) on mobile = "조잡"/crude. Dropped `-mx-4` (map insets with the band's px-4) and made it `rounded-2xl` on ALL breakpoints with a crisp `ring-slate-900/[0.06]` hairline over the mint glow + deeper layered drop shadow + inset top highlight; glaze overlay matched to `rounded-2xl`. (3) **2-up sheet grid (`PlannerTopRail` renderForm + `Field`):** the edit sheet stacked every field full-width (투어 시간 alone on its own row). Sheet body switched from `space-y-4` to `grid grid-cols-2 gap-x-3 gap-y-4`; `Field` gained a `span` prop. Chips (track/region) + ship/pickup/port span both columns; date·party and lang·duration pair two-per-row at equal width; all controls go `w-full` in-sheet. (4) **Compact badges (`PlannerTopRail` chipClass + `CategoryFilterBar`):** track/region chips dropped `text-caption`→`text-micro` + icons `h-3.5`→`h-3` + `gap-1` (matches the catalog filter chip family — a real site token, not an oversized one); catalog filter chips trimmed `py-1`→`py-[3px]` + count badge `text-[9px]`. Presentation-only (V8), no new amber (V5/V14 — slate-900 icon chip, not amber), no new deps (V11), no new copy (reused `editTrip`; V12 intact). tsc clean (changed files + whole tree minus pre-existing `__tests__` jest-types). Files: `PlannerTopRail.tsx`, `BuilderShell.tsx`, `CategoryFilterBar.tsx`. |
| 2026-06-23 | (committed b275416) | **Post-launch UI/UX polish (user review 2026-06-23, 3 screenshots) — 3 builder surfaces had drifted from the site's compact/premium language (V13).** (1) **PlannerTopRail "여행 정보 수정" sheet:** track/region chip rows were `flex` (no wrap) so the long KR label "개인 차량 / 육로 여행" broke mid-phrase inside the pill and overran its neighbours — added `flex-wrap` + `whitespace-nowrap` on `chipClass` so the *row* wraps, never the label. The lang/pickup/port fields were raw native `<select>` (un-brandable OS picker, cramped height) and party/ship were short native inputs — all three selects now use the shared `SelectDropdown` (same premium popover as date + duration; V11 no-new-deps) and party/ship adopt a shared `FIELD_INPUT_CLASS` (h-11/h-12 rounded-button frosted slate-50) so every control in the sheet reads as one h-11 family. (2) **CategoryFilterBar (유형별 필터):** chips were `px-3 py-1.5 text-caption` and wrapped to ~4 rows that shoved the catalog grid far down — shrunk to `px-2.5 py-1 text-micro` + `whitespace-nowrap shrink-0` and switched the container from `flex-wrap` to a single-row `overflow-x-auto scrollbar-hide` strip (matches the site's compact filter language). (3) **Map focus (`POICatalogMap` focus effect):** removed the forced `setZoom(12)` on "show on map" — it yanked the camera in so far the rest of the day's route fell off-screen and the user had to manually zoom back out; now pans only, keeping the auto-fitBounds frame. **POIInfoWindowContent compacted:** image 16:10 (~175px tall at the old 280px width) → fixed `h-[88px]`, card `max-w-[280px]` → `max-w-[208px]`, summary 140→90 chars + `line-clamp-2`, tighter paddings — so the card no longer overflows/clips on a 40vh mobile map. Presentation-only (V8), no new deps (V11), no new amber (V5). Files: `PlannerTopRail.tsx`, `CategoryFilterBar.tsx`, `POICatalogMap.tsx`, `POIInfoWindowContent.tsx`. |
| 2026-05-29 | (pending) | **Cross-track (driven under the landing plan) — two builder-owned files touched.** (a) `AIRecommendPanel.tsx` autoRun effect now reads `intent` from `useSearchParams()` instead of the `window.location`-seeded `intent` state — fixes the reported "redirected builder shows 0 stops, must press CTA again" bug (soft client-nav from the landing planner left the seed empty → `combined < 2` → silent no-op). (b) `IntakeDateField.tsx` premium restyle — day cells `h-9 w-9 mx-auto` → `aspect-square w-full` (fills grid cell; numbers can't overlap at any width), popover 18→20rem, gap-0.5→1, selected shadow + today emphasis. Full detail in landing plan §C 2026-05-29 + landing-matcher-builder-unified-plan handoff. |
| 2026-05-29 | (pending) | **Issue 1 (bug) — map pins now 1:1 with the timeline (§B V15).** `POICatalogMap.tsx`: removed `@googlemaps/markerclusterer` import + `clustererRef`; the marker loop `continue`s for out-of-route POIs (`if (!inRoute) continue;`) so only the day's stops are pinned; markers attach directly (`m.map = map`) instead of via a clusterer. Kills the blue "count" badges (2/3) that masqueraded as stop numbers. Verified safe: `fitBounds` + activeKey-hover effects only touch route pins; grid "show on map" opens the InfoWindow by coords (no marker dependency). |
| 2026-05-29 | (pending) | **Issue 2 (polish) — premium map glaze (Phase 15.1).** `BuilderShell.tsx`: added a `pointer-events-none` overlay over the map tiles (z-10, below the z-20 Reset button) — soft top sheen + corner vignette + inset hairline highlight via radial-gradient + inset box-shadow. Map tiles render via a Google vector Map ID (inline `styles` ignored), so this CSS glaze is the pin-safe way to make the canvas read editorial; the vignette stays at the edges so centre route pins are never dimmed. The existing mint glow ring (Phase 11 D29, user directive) is preserved. |
| 2026-05-29 | (investigated) | **Issue 3 (auto-generate from landing) — traced, code path already wired.** Landing build-submit (`landing-planner-card.tsx:191-208`) forwards `?region&date&party&duration&lang&intent&autoRun=1` to `/itinerary-builder`; that route renders `BuilderShell` directly (no intake step) and preserves params; `AIRecommendPanel` reads `intent` on mount + the `autoRun=1` effect (`:261-279`) fires `runMatch`, which **auto-applies** via `onAccept(recs)` (`:192`) — no separate Apply click. The landing default-mode flip to "build" (landing plan PR #106) is the mode that emits `autoRun`. Conclusion: auto-output is wired in `main`; the user's extra-click experience is likely the pre-#106 / pre-deploy production state. **Awaiting user re-test + exact remaining-click repro before changing correct code.** No code change this pass. |
| 2026-05-22 | (uncommitted) | **IntakeForm reveal-bug fix + premium date picker.** (a) **Reveal bug:** the form used the shared `useRevealContainerProps()` scroll-reveal (`whileInView`, once). Conditional fields (region / cruise hours / ship) that mount on track change stayed stuck at `opacity:0` — they rendered as blank gaps (user reported two empty bands on the cruise track). Switched the form to **mount-reveal** (`initial="hidden" animate="visible"` + local stagger `containerVariants`), so the parent's "visible" state propagates to any child that mounts later. Kept the premium fade + reduced-motion path. (b) **Premium date picker:** new `IntakeDateField.tsx` replaces the native `<input type="date">` (which popped the un-brandable green OS calendar on mobile). On-brand inline-expanding control: month grid (locale-aware via `Intl`, prev/next nav, disabled past days, navy selected, today ring) + type-in `YYYY-MM-DD` row + Today/Tomorrow chips. No new deps (framer-motion + lucide only, V11). Date field moved to its own full-width row (party below, capped width) so the inline calendar isn't cramped/clipped. `required` now enforced in `handleSubmit` (native `required` is gone) with an inline error. New i18n keys `intake.{dateSelect,dateToday,dateTomorrow,dateRequired}` hand-added to all 6 locales (NOT via the translate script — see the dropped-keys incident below). tsc + build green. |
| 2026-05-22 | (uncommitted) | **Post-V2 IntakeForm follow-up — pax + travel date pulled up into IntakeForm** (§E 2026-05-22 row #2). Added a `date` (required, `min`=today) + `party` (number 1-30, default "2") field pair to `IntakeForm.tsx`, rendered for ALL tracks just above submit; `handleSubmit` now forwards `?date=` + `?party=` into the `/[region]` push. Both already drive `pricing-policy.ts` (date→peak-season, pax→7-9/10-13 Solati tier + vehicle class) — they were previously collected only in `QuoteModal`, so the first auto-quote under-specified. **i18n reused** existing `intake.dateLabel`/`intake.partyLabel` (present in all 6 locales — no new keys). QuoteModal still reads `?date=`/`?party=` and keeps its own editable fields as a fallback (not removed — user didn't ask to). User decision: **date = required**. tsc clean. |
| 2026-05-22 | (uncommitted) | **Post-V2 IntakeForm follow-up — cruise "shore excursion" clarifier shipped** (§E 2026-05-22 row #1). Added `trackCruiseSub` ("Shore excursion") muted sub-caption under the "On a cruise" track card only (`IntakeForm.tsx` — optional `sub` on the track map + 10px muted line, `text-white/70` active / `text-slate-400` idle). Prevents the "do they sell cruise packages?" misread. i18n added to all 6 locales (en/ko 기항지 관광/ja 寄港地観光/zh 岸上观光/zh-TW 岸上觀光/es Excursión en tierra) — translate script needs API key (unavailable), so the 5 non-EN locales were filled by hand with standard shore-excursion terms. Deliberate exception to Phase 7's density call (which removed `trackCruiseHint`) — logged as an honesty clarifier, not a density reversal. JSON valid + tsc clean. **Parked (awaiting user):** §E row #2 (move pax+date into IntakeForm, user chose date=**required**) + §E row #3 (T-7 deadline — see clarification in that row; routes to payment-settlement track). |
| 2026-05-21 | (uncommitted) | **Follow-up track spun off** — user feedback (2 screenshots) found V8/V6 under-delivered: timeline cards are 56px thumbnail rows (not the tour-product `itineraryStop` magazine card) and AI result is a badge horizontal-drag rail (not merged into the timeline). New plan `docs/itinerary-builder-result-timeline-richness-master-plan-2026-05-21.md` (Result Timeline Richness, RR1–RR7, Phases R0–R3) corrects this. This V2 plan stays COMPLETE; the follow-up reuses this skill + honors V1–V13 as the floor. |
| 2026-05-21 | (uncommitted) | **Cross-plan touch (unified planner Phase 4)** — additive intent handoff from the home unified planner into the builder. `IntakeForm.handleSubmit` forwards `?intent=` into the `/[region]` push; `AIRecommendPanel` reads `?intent=` on mount and prefills `#ai-intent` (prefill only, no auto-submit). Client-side only — no server `searchParams` (ISR-static `[region]` page unchanged), no `BuilderShell` prop change, no new deps/copy (V11/V12 intact). Logged per `docs/landing-matcher-builder-unified-plan-2026-05-20.md` Gate 0.5. |
| 2026-05-20 | (this commit) | **V13 tone-alignment renewal shipped** (new post-V2 track, see §B V13). Bringing all builder surfaces from the pre-settlement amber/ivory tone to site-native frosted-over-mesh, preserving functional amber (sequence wayfinding). Targets: `app/itinerary-builder/page.tsx` (landing) + `IntakeForm` + `[region]/page.tsx` + `BuilderShell` + `AIRecommendPanel` + `POICatalogGrid` + `ResultTimeline` + `POICatalogMap` (loading/error fallback). Also flagged: "Oops/Map couldn't load" on region page = Google Maps `loadError` (graceful fallback working) — root cause is API-key HTTP-referer allowlist not covering the preview/tunnel domain (env config), NOT a code defect — loader version additionally pinned `weekly → quarterly` in `lib/google-maps.ts` as weekly-channel churn-avoidance hardening (separate commit). **Parallel same-day directive (not part of V13 tone, separate commit):** count-mention marketing copy removed site-wide across all 6 locales — `busanStops`/`jejuStops` "20/25 curated stops" → "Curated stops", `[region]` subtitles de-numbered, `trustReviews`/`exploreCta` "234 verified reviews" → number stripped, mockup "30+ tours" removed; `step2Body` "200 listings" bumped to "1000+". Dynamic counts (`{pois.length}`/`{count} stops`, search-result meta) intentionally preserved. |
...
| 2026-05-22 | **Cruise card "shore excursion" clarifier** — ✅ SHIPPED 2026-05-22. `trackCruiseSub` "Shore excursion" muted caption under the cruise card only (6 locales). See §C 2026-05-22. | Done. | — |
| 2026-05-22 | **Move pax + date INTO IntakeForm** — ✅ SHIPPED 2026-05-22 (date=required per user). `date`+`party` fields added to IntakeForm, forwarded via `?date=`/`?party=`; QuoteModal fields kept as editable fallback. See §C 2026-05-22. Open: availability check at intake (not done — date is free-pick with min=today, no calendar block-out yet). | Done (availability check parked). | — |
| 2026-05-22 | **T-7 booking-deadline lifecycle** — quote → tentative reservation if >7d out → customer must pay-or-card-hold by T-7 → else auto-cancel. **MOSTLY ALREADY BUILT** in the payment-settlement track (manual-capture ≤7d / SetupIntent >7d + `recapture-holds` cron ~6d before). Gap vs. current model: user's framing implies a "reserve WITHOUT card, then require card by T-7 or auto-cancel" state; current model vaults the card AT booking. Needs reconciliation, NOT a from-scratch build. | OUT OF THIS PLAN'S SCOPE (V8 = presentation-only, no schema/API/jobs). Belongs to the payment-settlement track. **User clarification 2026-05-22:** their concern is the Stripe ~7-day auth-hold EXPIRY ("if you hold >7d out it auto-releases"). The existing model already solves this — >7d bookings vault the card (SetupIntent) and `recapture-holds` cron places the hold ~6d before the tour (inside the non-expiring window), automatically, with no customer return needed. So the *goal* is met. The only remaining product fork: **card-required-at-booking (current, automatic)** vs **no-card tentative reservation → require card by T-7 or auto-cancel (needs new states + reminder + cancel job).** **RESOLVED 2026-05-22: user chose card-at-booking (model A); drop-off lever = reassurance messaging, not removing the card.** Shipped copy-only: `checkout.cardOnFile*` strengthened across 6 locales (₩0 today / card saved securely / free cancellation; dropped inaccurate "only on no-show"). No state-machine work needed. See payment-settlement memory + the tour-checkout commit. | Done (copy). Card-lifecycle backend unchanged. |
