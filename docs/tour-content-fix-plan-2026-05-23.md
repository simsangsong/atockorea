...
| **loc-C (i18n 191-key gap)** | (this commit) | (this commit) | **5 locales × ~191 키 추가 + 4 orphan deletes** in `messages/*.json`. Bridges the cross-locale key-parity gap that left non-EN UIs rendering bare keys (`nav.tours.list`) or English fallback. Per-locale delta: **ja/zh/zh-TW/es +188** (added 191 missing, deleted 3 orphan `mypage.dashboard|reviews|settings` string-vs-object collisions), **ko +12** (added 13 missing — `appShell.backButton.label` + 12 `tourCard.*` — deleted 1 orphan `home.customJoinTour.verifySuccessKo`). All five locales now match EN baseline of 2092 keys exactly (0 missing / 0 orphan). Translation scope: `mypage.*` 155 (dashboard, reviews.write, settings, common.toast/confirm, landing greetings + quickAccess + emptyHero + recommendations, bookings.guestsLabel/receiptCta/dayCountdown), `toursList.*` 20 (price filters, ARIA labels, results meta), `tourCard.*` 12 (pickup/included/duration/lunch/tickets), `signup.*` 3 (group titles), `appShell.backButton.label` 1. Per `feedback_i18n_translate_script_drops_keys` — hand-translated tone-matched against existing locale strings; did NOT use `scripts/translate-itinerary-builder-messages.mjs`. `scripts/i18n-key-parity-audit.mjs` + `scripts/i18n-loc-c-translations.mjs` + `scripts/phase-loc-c-key-parity.mjs`. Phase Z jest + Next build both green post-apply. |
| 5b EN deferred | — | — | ⏳ |
