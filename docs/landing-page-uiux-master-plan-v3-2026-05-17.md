...
| 2026-05-24 (3) | 헤드라인 뒤 검정 radial scrim 패널 dramatically lighten + 텍스트 그림자 강화 (사용자 요청, §B 2026-05-24 (3) row) | (pending) | `hero-section.tsx`: (a) scrim radial-gradient stops `0.88/0.72/0.45/0.18` → `0.35/0.25/0.12/0.04` (~60% opacity 감소, 거의 투명에 가까움 — 사진이 텍스트 패널을 통과해 읽힘). (b) H1 textShadow `0 2px 12px rgba(0,0,0,0.5) + 0 1px 3px rgba(0,0,0,0.45)` → `0 2px 16px rgba(0,0,0,0.6) + 0 1px 4px rgba(0,0,0,0.55)` (blur ↑ alpha ↑). (c) 서브카피 textShadow `0 1px 8px rgba(0,0,0,0.55) + 0 1px 2px rgba(0,0,0,0.45)` → `0 1px 12px rgba(0,0,0,0.65) + 0 1px 3px rgba(0,0,0,0.55)`, 텍스트 컬러 `text-white/85` → `text-white/90`. 패널 자체는 제거하지 않음 — 5장 슬라이드 중 벚꽃·하늘 등 밝은 영역에서 흰색 텍스트 contrast 안전판으로 옅게 유지. 검증: `preview_eval` 헤드라인 scrim `backgroundImage` + h1/sub textShadow computed style 모두 적용 확인 + `npm run build` exit 0 (에러/경고 0) |
