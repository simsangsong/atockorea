...
**현재 활성 Phase: 🔄 "투어타입 우선" 홈 개혁 Wave 1 + Wave 2(U7·U9) + V2 사진 텍스처 완료. 남은: U8 긴급성(실가용성 데이터 대기·블록), 라이브 QA·퍼널 모니터링. (U10 N/A.)** 플랜 `docs/landing-home-tour-type-reform-plan-2026-06-21.md`. 완성 IA: Hero(사진+H1+큐레이션) → 목적지+인원 스트립(U6) → 타입 3카드{소그룹·프라이빗·버스}(상단 듀오톤 사진 V2·실시간 가격 U1·동적 배지 U5·elevation V7) → 추천받기 카드(U3=매처) → 예약 안심 1줄(U7). **U9 입력 운반**: 홈 인원 스테퍼 → /tours/list → 상품 예약 카드 → 체크아웃 전 구간 party 보존(재질문 0); 빌더 경로는 기존부터 끝단 운반. hero 플래너 제거(W1e-3), landing-planner-card 코드 보존. (이전: v3 본 실행 + Phase D 완료, 통합 플래너 Phase 1–4 완료.)
