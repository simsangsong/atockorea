...
**현재 활성 Phase: 🔄 "투어타입 우선" 홈 개혁 — 코드 작업 완료(Wave 0+1+2 전부). 남은 건 라이브 QA·퍼널 모니터링(사용자측)뿐.** U7·U9·V2·U8(시즌성) 완료; U8 희소성은 N/A(가용성 무한—조작 금지), U10 N/A. 플랜 `docs/landing-home-tour-type-reform-plan-2026-06-21.md`. 완성 IA: Hero(사진+H1+큐레이션) → 시즌성 칩(U8, 활성/임박 시) → 인원 스테퍼(U6 목적지 칩은 2026-06-22 제거 — destination은 /tours/list에서) → 타입 3카드{소그룹·프라이빗·버스}(상단 듀오톤 사진 V2·실시간 가격 U1·동적 배지 U5·elevation V7) → 빌더 전용 카드 → 추천받기 카드(U3=매처) → 예약 안심 1줄(U7). **U9 입력 운반**: 홈 인원 스테퍼 → /tours/list → 상품 예약 카드 → 체크아웃 전 구간 party 보존(재질문 0); 빌더 경로는 기존부터 끝단 운반. hero 플래너 제거(W1e-3), landing-planner-card 코드 보존. (이전: v3 본 실행 + Phase D 완료, 통합 플래너 Phase 1–4 완료.)
...
| 2026-05-29 | **랜딩 플래너 카드 기본 모드 `match` → `build` 변경** (`landing-planner-card.tsx:94/97` `useState<PlannerMode>("build")`). Build/Match segmented control은 유지 — 방문자는 한 클릭으로 matcher 전환 가능 | 사용자 결정 2026-05-29 "builder 자동 선택으로 바꿔줘". 통합 플랜(`docs/landing-matcher-builder-unified-plan-2026-05-20.md:206-216`)의 "기본 match" 사유(첫 방문자 적합·홈 전환 의도 보존·builder 1클릭)를 제품 오너 판단으로 오버라이드. 영향: home_cta_copy A/B(match CTA 카피)는 노출 감소하나 §A상 이미 검정력 없음·conclude 권장 상태라 비차단. MatcherMorphingPanel(match 전용)은 첫 페인트 미렌더 | ← 통합 플랜 2026-05-20 "기본 match" 결정 번복 |
| 2026-06-21 | **"투어타입 우선" 홈 개혁 채택 — Match/Build 토글 제거 + 타입 카드(프라이빗/그룹/추천받기)를 1차 결정 표면으로.** 매처는 삭제 아닌 "추천받기" 동등 카드로 재배치(코드 보존), Private는 빌더-직행 아닌 큐레이션 리스트-먼저. 3 웨이브 점진(Wave 0 비파괴 골격→Wave 1 IA 스왑·토글 제거·실시간 가격→Wave 2 마감) | 사용자 결정 2026-06-21. 추상적 도구 선택(match/build)이 고객 마찰 → 구체적 "어떤 형태의 투어" 결정으로. 상세·감사·비주얼 = `docs/landing-home-tour-type-reform-plan-2026-06-21.md` | ← 통합 플랜 2026-05-20 "단일 segmented planning surface" 전제 부분 번복 + 2026-05-29 "기본 build 모드"는 토글 제거로 무의미화(Wave 1) |
| 2026-06-23 | **히어로 정체성 피벗 — H1·서브헤드 교체 + curation 3-stat 블록 → 신뢰 칩 4개.** H1 "단순한 여행사가 아니라, 끝까지 함께하는 한국 여행 에이전트"(EN "Not just an agency — a travel agent that plans, guides, and stays with you"); 서브헤드=AI 일정+4–10h 프라이빗+투어 중 챗봇·상담; 칩 4개=미국·한국 이중인증/글로벌 안전결제/예약먼저·투어당일 결제/24h전 100% 환불. **직접 교체**(A/B 아님), 6 로케일. "한국 유일" 최상급 제외(허위광고·expectation 인플레 가드) | 사용자 요청·확정 2026-06-23. "대형 OTA 대리점" 톤 탈피→사이트 정체성(스마트 에이전트·유연 프라이빗·투어 중 지원·이중인증·안전결제·후불+환불). §13 "시 H1 A/B 외 교체 금지"/"baseline 없이 카피 변경 금지"를 product-owner 결정으로 오버라이드(이전 hero 변경 패턴 동일, baseline 미수집→directional). **사실 수정**: §B 2026-05-23 "4.9★ 평점 유지" 주장 코드 실사상 무효 — 실제 curation 3-stat은 30/Vetted/Korea(평점 없음), 통째로 칩 대체 | ← 2026-05-17 "H1 즉시 교체 금지·A/B로만" + 2026-05-23 curation 블록 대체 |
| 2026-06-21 | **카드 구조 번복: 소그룹+버스→"그룹" 통합 취소.** 최종 = 타입 카드 3장 {소그룹·프라이빗·버스} 유지 + "추천받기" 카드를 그 아래 풀폭으로. 콘솔리데이션(W1e-2) 폐기 | 사용자 결정 2026-06-21 "기존 3카드 밑에 추천 카드, 3카드로 줄이지 말고". 통합보다 타입 명시 노출을 선호 | ← 2026-06-21 "3카드 {프라이빗/그룹/추천} 통합" 결정 대체 |
| 2026-06-22 | **reform U4 부분 번복: 빌더를 보조 링크 → "전용 카드"로 격상.** 리스트-먼저는 유지하되, `/itinerary-builder`(시간 단위 선택 + 지도 일정 커스터마이징) 진입을 추천받기 카드와 동등한 풀폭 전용 카드로 승격(타입 3카드 아래·추천받기 위). Private 카드 quiet 링크는 컨텍스트 보조로 존치 | 사용자 2026-06-22: 빌더(시간+지도 커스터마이징)가 핵심 매리트인데 보조 링크라 묻혔다고 지적 → "빌더 전용 카드 추가" 선택 | ← reform U4 "Private=리스트-먼저, 빌더=보조 링크" 부분 번복(리스트-먼저 전제는 유지) |
| 2026-06-22 | **reform U6 목적지 스트립 제거.** 홈 타입 섹션 상단의 목적지 칩(All/Jeju/Busan/Seoul, 인원 스테퍼 위)을 제거 — 스트립은 인원 스테퍼만. destination 선택은 `/tours/list`에서. 카드 링크의 `&destination=`/빌더 `&region=` 운반 제거(party만 운반) | 사용자 결정 2026-06-22 "랜딩 히어로 밑 destinations 옵션(인원수 위) 지워" | ← reform U6 "목적지+인원 스트립" 부분 번복(인원 스테퍼는 유지; destinationLabel/destAll i18n·homeDestinationCardClick은 dead로 잔존) |
...
| 2026-05-29 | 플래너 카드 4건 — 검증 배너 + autoRun 수정 + 프리미엄 달력/언어드롭다운 (사용자 보고 스크린샷, in-flight bug-fix 1c) | (pending) | **(1) 빨간 검증 배너**: `landing-planner-card.tsx` build CTA가 필수항목 누락 시 silent no-op + 테두리 flash만 했음 → `buildError` state 추가, `handleStartBuilding`이 누락 필드(date/guests) 목록을 계산해 CTA 위 `role="alert"` rose 배너로 표시. i18n `home.premium.v2.planner.buildMissingError` ("Please add: {fields}") 6 로케일 hand-add(translate 스크립트 API 키 부재). 필드 편집 시 자동 클리어. **(2) autoRun 수정(핵심 버그)**: 랜딩→빌더 soft nav 후 `AIRecommendPanel`이 일정 자동생성 안 되고 0 stops로 CTA 재클릭 요구(사용자 스크린샷 2269f062 — 패널 expanded=runMatch 미실행). 원인: autoRun 게이트가 `intent` state(=`window.location.search` useState 초기값)에 의존 → soft client navigation에서 seed가 비어 `combined < 2`로 silent return. 수정: autoRun effect가 `sp.get("intent")`(reactive, route-consistent)에서 intent를 읽도록(`AIRecommendPanel.tsx:263-`). **(3) 프리미엄 달력**: `IntakeDateField.tsx` day 셀을 고정 `h-9 w-9 mx-auto` → `aspect-square w-full`(그리드 셀 꽉 채움, 폭 무관하게 숫자 겹침 원천 차단) + 팝오버 18→20rem + gap-0.5→1 + 선택 shadow/today 강조. **(4) 프리미엄 언어 드롭다운**: 네이티브 `<select>`(모바일 OS 피커 팝업, 스크린샷 1fac6882) → 커스텀 `LangDropdown`(button+팝오버 listbox, IntakeDateField 패밀리, right-0 앵커, 외부클릭/Esc 닫힘, framer 모션, reduce-motion 가드). 신규 lib 0(§13 준수). node_modules 미설치로 full build/lint 미실행 |
| 2026-05-29 | 랜딩 플래너 카드 기본 모드 match → build (사용자 결정, §B 2026-05-29 row) | (pending) | `landing-planner-card.tsx:94-97` `useState<PlannerMode>("match")` → `"build"` + 사유 주석. 통합 플랜 2026-05-20 "기본 match" 결정 번복(§B 참조). 부수효과 검증: mount 시 match 강제 effect 없음(`useEffect` 120/147은 PLANNER_BUILD_EVENT 리스너 + home_cta_copy A/B 폴링, mode 무관), AnimatePresence(`key={mode}`, `initial={false}`) build body가 첫 at-rest 렌더, `mode==="match"` 게이트(MatcherMorphingPanel 252 / match body 419)는 그대로. node_modules 미설치로 full build 미실행 — useState 초기값 한정 변경 |
...
| 2026-05-29 | 플래너 카드 build 모드 인원/시간 컨트롤 정리 (사용자 보고 스크린샷, in-flight bug-fix 1c) | (pending) | `landing-planner-card.tsx`: (1) 시간 옵션 `[4,6,8,10,12]`→`[4,6,8,10]` (12h 제거 — 우측 삐져나옴 주원인). (2) 인원 입력창을 시간 뱃지와 동일 높이/형태로: `h-12 w-20 rounded-button md:h-14` → `h-12 w-16 rounded-full`(고정 높이 48px, md 성장 제거). (3) 시간 뱃지를 고정 높이 pill로: `rounded-full px-3 py-1.5 text-[11px] md:px-3.5/py-2/text-caption` → `flex h-12 min-w-[3rem] items-center justify-center rounded-full px-3.5 text-caption`(인원창과 높이 일치, 원형 pill). (4) 행 컨테이너 `gap-1.5 overflow-x-auto pb-1 md:gap-2` → `gap-2 overflow-x-auto`(좌우 드래그 유지, 4개라 표준 폭에서 fit → 우측 잘림 해소). node_modules 미설치로 full build/lint 미실행 |
| 2026-06-21 | "투어타입 우선" 홈 개혁 플랜 draft 작성 (신규 스코프, 사용자 요청 "과감한 개혁 플랜") | (pending) | `docs/landing-home-tour-type-reform-plan-2026-06-21.md`. 에이전트 코드/데이터 실사: ChooseTravelStyle 현황(5번째 슬롯·전부 /tours/list), 데이터에 배타적 type enum 부재(fit 0~1), 가격엔진 per-vehicle private only. **사실 정정(사용자 지적)**: 활성 카탈로그 35 = 프라이빗 6 + 그룹 계열 29(제주~12/부산8/서울8) → 인벤토리 충분. §D 보류 row 추가. 코드 0줄 |
| 2026-06-21 (2) | 개혁 플랜에 CRO/UX 리뷰 13 업그레이드(U1~U13) 전면 채택 + 3 웨이브 재구조화 (사용자 "13개 다 채택"/"오케이") | (pending) | reform 플랜 §3.5/§4/§5/§6/§7. 전환 척추 = U1 실시간 가격 + U2 관문 제거 + U3 추천받기 동등 카드(party 공유). 코드 0줄 |
| 2026-06-21 (3) | 개혁 플랜 전수 감사 + 12 흠집 일괄 패치 (사용자 "니 추천대로 진행" 위임) | (pending) | **확정: 3카드 {프라이빗/그룹/추천받기}**(그룹 통합). 해소: §0 동기화 / 가격·임계 `N*=ceil(PV/GP)` 도출(프라이빗=엔진·그룹=from) / 게이트를 Wave 0 앞 / 롤백지표 빌더진입률→예약 CVR / 카드당 ≤3요소(简单粗暴). 코드 0줄 |
| 2026-06-21 (4) | Phase 0 게이트 마감(3항목 추천 확정) + 서울 재활성화 ship | `a64de76d` | 게이트: 분류 `small_group_fit≥0.8` / 가격 N*=ceil(PV/GP) / 인원 스테퍼=카드 위 슬림 스트립. **서울 재활성화(사용자 결정, 코드 2파일)**: DB는 이미 is_active=true(코드 주석 스테일) → `CONSUMER_BLOCKED_TOUR_SLUGS` 서울 11 제거 + landing-planner coming-soon 게이트 제거. PR #115 머지 `a64de76d`. ⚠ 운영자 재검증 hold 되돌림(사용자 책임). Wave 0 착수 대기 |
| 2026-06-21 (5) | 개혁 플랜에 비주얼 디자인 섹션(§12) 정식 추가 (사용자 "정식 추가") | (pending) | reform 플랜 §12 = 고급 5레버(V1 에디토리얼 세리프·V2 듀오톤 사진·V3 가격 장부·V4 머티리얼 깊이·V5 마이크로 모션) + 직관 6레버(V6 스테퍼·V7 추천 1장 격상·V8 ≤3요소·V9 CTA 위계·V10 펼침·V11 연속성) + 카드 역할 재매핑 + design-system 가드. Wave 0/1 수용 기준에 비주얼 항목 추가. 기존 ChooseTravelStyle 자산 상속, 새 라이브러리 0. 코드 0줄 |
| 2026-06-21 (6) | **Wave 0 구현 (첫 코드 — 비파괴 골격+계측)** (사용자 "시작해, 누락없이, 100% 구현") | (pending) | (a) `components/home/v2/ui/PartyStepper.tsx` 신규 — 슬림 pill 스테퍼(− N +, 기본 2, ≥44px, min1/max13=MAX_AUTO_PAX, tokens·focus-ring·motion-reduce). (b) `choose-travel-style.tsx` — 스테퍼 카드 위 마운트 + party를 3 카드 링크로 운반(`?…&party=N`, U9 시드) + 타이틀 `font-magazine-serif-ko font-light`(V1) + `home_tour_type_card_click {type,party}` 발화. (c) `analytics.ts` — `homeTourTypeCardClick`·`homePartyStepperChange` 2 메서드 + `HomeTourTypeCard` 타입. (d) 6 로케일 `chooseStyle.party{Label,Caption,Decrease,Increase}` i18n(`{count}` 보간 검증). **U12 hero H1 = §B 큐레이션 보호로 보존, Wave 1 연기**(reform §13). U6 IA 스왑·토글 제거·실시간 가격 = Wave 1. 검증: tsc/lint/build (이번 커밋) |
| 2026-06-21 (7) | **V1 세리프 되돌림** (사용자 "세리프 톤이 페이지 전체랑 안 맞아, 혼자 따로 노는 느낌") | (pending) | `choose-travel-style.tsx` 섹션 h2 `font-magazine-serif-ko font-light` → 표준 `text-h2`(Inter). 근거: plain 배경 섹션 헤딩에 매거진 세리프는 톤 충돌(다른 섹션 헤딩 Featured/Why/Process 전부 Inter). 세리프는 사진 카드 타이틀(Destinations 지역명)에만. reform §12 V1 수정 + §13 V1 반려 로깅. 검증: tsc/build |
| 2026-06-21 (8) | **Wave 1 W1a — U1 실시간 가격(스테퍼 활성화)** (사용자 "다음 스텝") | (pending) | `choose-travel-style.tsx` 3 카드에 party 기반 실시간 가격 라인: 그룹(소그룹 $59/인·버스 $39/인)→×party 총액(`totalForParty`), 프라이빗($198/대)→÷party 1인당(`perPersonForParty`). 고객이 스테퍼만 움직여 크로스오버 직접 확인(4인↑ 프라이빗 유리 등). `formatPrice` 통화변환·`≈` 근사·"정밀가 다음 단계" 함의. `partyCaption` 가격연동 승격. 6 로케일 i18n 3키. Wave 1 분할 진행(reform §14): 다음 W1b=U5 동적 배지→U3 매처 카드→U4 Private 리스트→IA 스왑→비주얼. 검증: tsc/build |
| 2026-06-21 (9) | **Wave 1 W1b — U5 동적 추천 배지 + 가격 글씨 키움** (사용자 "글씨 조금만 더 키우고 다음 스텝") | (pending) | `choose-travel-style.tsx`: "Recommended" amber 배지가 party 따라 소그룹↔프라이빗 이동(N*=ceil(198/59)=4; party<4 소그룹, ≥4 프라이빗 — 가격 실제 역전점이라 정직). 소그룹 배지 추천 아닐 때 숨김, 프라이빗 neutral "Private"↔amber "Recommended" 토글. 신규 i18n 0(`recommended` 재사용). W1a 실시간 가격 라인 `text-micro`→`text-caption` 확대. V7 elevation·V5 슬라이드 모션은 W1f. 검증: tsc/build |
| 2026-06-21 (10) | **Wave 1 W1c — U3 매처 "추천받기" 카드** (사용자 "다음 진행") | (pending) | `choose-travel-style.tsx`: 3 타입 카드 아래 풀폭 보조 카드(아웃라인·Sparkles·"어떤 게 맞을지 모르겠다면?"+추천받기 CTA) → `/match`(`HOME_CTA_MATCHING_HREF`) 라우팅, `home_tour_type_card_click {type:"recommend"}` 발화. 그리드 무손상, 미결정 세그먼트 동등 노출(U3). 인페이스 매처 트리거는 hero 토글 제거(W1e) 때 통합. 6 로케일 i18n 3키. 검증: tsc/build |
...
| 2026-06-21 (17) | **프라이빗 가격 차량등급 tier 적용** (사용자 "프라이빗은 6명 이상부터 가격 뛰어 — 내가 준 가격표에 맞춰") | (pending) | `choose-style-card-usd.ts`: private 고정 $198 → `privateVehicleUsd(party)` tier 함수(세단≤6 $198 / 밴≤9 $235 / 솔라티≤13 $310 — `PAX_TIERS` ₩50k·₩150k 추가요금 반영, 점프 at 7·10). `choose-travel-style.tsx`: 프라이빗 메인가·1인당가·추천 비교 모두 tier 가격 사용(고정값 제거). ⚠ 카드는 USD 마케팅 anchor라 엔진(₩)과 통화체계 다름 → tier **구조**는 정확, USD 절대값은 근사(상수 파일에서 조정 가능). 스테퍼 max=13(MAX_AUTO_PAX)이라 14+ 케이스 없음. 검증: tsc/lint/build green |
| 2026-06-21 (18) | **Wave 2 U7 — 예약 안심(리스크 리버설) 한 줄** (사용자 "다음 스텝부터 진행") | (pending) | `choose-travel-style.tsx`: 타입 카드 묶음 아래 1회(카드 반복 금지·§12 V9) ✓체크 3점: 24시간 전 무료취소 / 오늘 결제 없음(카드 홀드만) / 검증한 운영자만. **전부 이미 제품 명시 사실**(en.json `freeCancellation`·`cardOnFileSubtitle`·큐레이션 — 허위 0, U8 정직성 원칙 준수). i18n 3키 6 로케일. Check 아이콘. **남은 Wave 2**: U8 긴급성(실가용성 데이터 필요·블록), V2 사진 텍스처(에셋 필요), U10(칩 제거로 N/A). 검증: tsc/lint/build green |
| 2026-06-21 (19) | **Wave 2 U9 — 입력 끝까지 운반(그룹/투어 경로 party 관통)** (사용자 "U9 입력 끝까지 운반") | (pending) | **실사(Explore)**: 빌더 경로(홈 Private 보조링크→`/itinerary-builder`→빌더 체크아웃)는 이미 끝단 운반(서버 param generic-loop·BuilderShell `party/date/lang/region`·체크아웃 booking row JSONB 상속). Flow A 업스트림 입력은 party뿐(date/lang은 예약 카드서 처음 선택→sessionStorage로 이미 운반). U9 잔여=그룹 경로 party 드롭 3곳: (#1) `ToursListClient` URL `?party=` 미독·미운반 → `carriedParty`+`detailHrefWithParty()`로 카드 `detailHref`에 부착(필터 state 불편입=운반만). (#2) `tour-product/[slug]` 서버 locale만 읽음 → `RouteSearchParams.party`+`parsePartyParam()` → `TourProductDetailClient` `initialGuests`. (#3) 예약 카드 `DEFAULT_GUESTS` 하드코딩 → `TourDesktopBookingCard`·`TourStickyBookingBar` `initialGuests?` prop+`clampGuests` seed. 체크아웃 코드 0(sessionStorage 상속). date/lang 무변경(상류 소스 없음·허위 운반 금지). 코드 6파일·i18n 0. 검증: tsc 0/lint 0(잔여 6 pre-existing)/build green. **남은 Wave 2 전부 블록/N-A**: U8(실데이터), V2(에셋), U10(N/A) |
| 2026-06-21 (20) | **프라이빗 가격: 엔진 SoT 감사(100% 일치) + 카드 USD 엔진 자동 도출** (사용자 실제 ₩가격표 제공, "둘 다 엔진+카드") | (pending) | **엔진 감사**: 사용자 프라이빗 ₩표(언어×시간 베이스·인원 tier 밴+50k/솔라티+150k·성수기+200k·5구간·지역 추가요금·DMZ/크루즈)를 `lib/quote-engine/pricing-policy.ts`와 한 줄 대조 → **100% 일치, 엔진 코드 0 변경**(세 예시 원단위 재현: 서울8h4명 340k/제주10h8명 410k/부산8h11명성수기 590k). **카드**: `choose-style-card-usd.ts` 하드코딩 $198/$235/$310 제거→엔진 자동 도출(`baseForTierHours("english",8)+PAX_TIERS.surcharge`÷₩1480)=**$230/$264/$331**. 앵커=영어×**8h**(featured 소그룹 East Jeju "8 hours" $59/인과 동일 day-length→U5 크로스오버 사과대사과, N*=4 유지; 4h 앵커는 §10 허위라 기각). 엔진 도출=stale 0. `private.compareAt` dead 제거. 검증: tsc 0/lint 0(잔여 6 pre-existing)/build green |
| 2026-06-21 (21) | **V2 카드 듀오톤 사진 (보류 해제 — 기존 사진 재활용)** (사용자 "기존 사진 재활용, 프라이빗=서울 프라이빗 썸네일·버스=버스 들어간 사진·소그룹=적당한거") | (pending) | `choose-travel-style.tsx`: `CardPhotoStrip` 헬퍼(카드 상단 풀블리드 h-24/md h-28 듀오톤 = Vogue 필터 saturate1.1/contrast1.06/brightness0.96 + 카드색 멀티플라이 톤 + 바디로 페이드 + 히어로 SVG noise 그레인). **새 에셋 0**: 프라이빗=`seoul-private-charter/seoul-private-gyeongbokgung-from-car.webp`(세단서 본 경복궁)·버스=`busan-cruise-terminal/busan-cruise-terminal-bus-pickup.webp`(AtoC 코치버스)·소그룹=`jagalchi-market/jagalchi-interior-foreign-tourists.webp`(시장 외국인 여행객). 톤 소그룹 slate/프라이빗·버스 emerald(§12 카드 아이덴티티). `next/image fill`+`loading=lazy`(히어로 아래 LCP-safe). 추천 카드 사진 없음. **남은 Wave 2 = U8(실데이터·블록)만.** 검증: tsc 0/lint 0(잔여 6 pre-existing)/build green |
| 2026-06-21 (22) | **U8 — 희소성 N/A(가용성 무한) 확정 + 정직한 시즌성 칩만 (사용자 "가용성은 무한이야"→"시즌성 띠 추가 후 종료")** | (pending) | **희소성 N/A**: 온디맨드 투어=가용성 무한(`product_inventory` 0행 정상), "N석 남음"은 만들면 §10 조작 → 영구 비노출. **시즌성만**: `lib/home/season-notes.ts`(신규 순수함수) — `PEAK_RANGES` scenic 부분집합(벚꽃/여름/단풍, 황금연휴·연말 제외)에서 활성(종료 카운트다운) 또는 ≤30일 lead 다음 시즌 도출. `choose-travel-style.tsx` `SeasonNoteChip`(헤딩 아래 앰버 칩, Flower2/Sun/Leaf) — `useSyncExternalStore`(server=null/client 1회 캐시)로 SSR/CSR 날짜 미스매치 방지 + set-state-in-effect lint 회피. i18n 5키×6 로케일(`{season}`/`{count}` 보간). 미활성·미임박 시 미표시(예 06-22→여름 28일 후·04-01→벚꽃 9일 남음·09-01→미표시; 6 날짜 검증). 검증: 6 JSON parse / tsc 0 / lint 0(잔여 6 pre-existing) / build green. **→ 홈 개혁 Wave 0+1+2 코드 완료. 남은 건 라이브 QA뿐.** |
| 2026-06-22 | **빌더 전용 카드 추가 (reform U4 부분 번복, §B row 2026-06-22)** (사용자 "프라이빗 스케줄 빌더 어디 있어? 시간+지도 커스터마이징이 큰 매리트" → "빌더 전용 카드 추가") | (pending) | `choose-travel-style.tsx`: 타입 3카드 아래·추천받기 카드 위에 빌더 전용 풀폭 카드(emerald 톤+MapPin, builderCard 카피) 추가 → `/itinerary-builder?party=&region=` 운반, `home_tour_type_card_click {type:"builder"}` 발화. `analytics.ts` `HomeTourTypeCard`에 "builder" 추가. i18n 3키×6 로케일(`builderCard{Title,Desc,Cta}`). 리스트-먼저 유지, Private quiet 링크 존치. 빌더 코드/지도 무변경. 검증: 6 JSON / tsc 0 / lint 0(잔여 6 pre-existing) / build green. ⚠ 라이브 QA: 빌더 지도 정상 렌더 확인 권장(과거 gmaps 이슈) |
| 2026-06-22 | **reform U6 목적지 스트립 제거 (사용자 "히어로 밑 destinations 옵션, 인원수 위에 있는거 지워")** | (pending) | `choose-travel-style.tsx`: 인원 스테퍼 위 목적지 칩(All/Jeju/Busan/Seoul) 블록 삭제 → 스트립=인원 스테퍼만(중앙 정렬). `destination` state·`handleDestinationChange`·`DESTINATIONS`·`homeDestinationCardClick` 호출 제거, `withParams`=party만, 빌더 링크 2곳 `&region=` 제거. destination 선택은 /tours/list에서. `destinationLabel`/`destAll` i18n·analytics 메서드는 dead로 잔존(무해). §B 번복 row 기록. 검증: tsc 0 / lint 0(잔여 6 pre-existing) / build green |
| 2026-06-23 | 히어로 정체성 카피 피벗 (사용자 확정, §B 2026-06-23 row) | (pending) | `hero-section.tsx`: (a) H1=`premium.hero.headlineLine1`·서브헤드=`atfHeroSubhead` 값 교체(6 로케일, "단순한 여행사가 아니라…" / "AI가 일정을 짜고, 4–10시간 프라이빗…"). (b) curation proof 블록(h2 `curationHeadline` + 3-stat `curation{Shortlist,Bookings,Rating}{Value,Label}` + `curationProof`) **제거** → 신뢰 칩 4개 grid(`assurances` 배열, lucide `ShieldCheck/Lock/CalendarClock/RotateCcw`, 2-col 모바일/`md:grid-cols-4`, slate 톤 rounded-xl 칩). `messages/*.json` 6 로케일: headlineLine1/atfHeroSubhead 값 교체 + `assureCert/assurePay/assureBookNow/assureRefund` 4키 추가(node 정규식 패치 스크립트, parse+키 패리티 6/6 검증 후 삭제). curation* i18n 키는 dead 잔존(비용 0). **사실 수정**: 실제 curation 3-stat=30/Vetted/Korea(§B 2026-05-23의 "4.9★ 평점" 주장 무효). 검증: tsc 0 errors / `npm run build` green |
| 2026-06-23 (2) | 히어로 서브헤드 재다듬 + 프라이빗 카드 유연성 증거 (사용자 피드백 "AI가 짜는 게 아니라 커스터마이징, 전 구간 동행 강조; 4–10h는 카드로") | (pending) | **IR 재배치**: 스펙(4–10h)을 히어로에서 빼 프라이빗 카드로, 히어로는 포지셔닝만. (a) `atfHeroSubhead` 6 로케일 교체 → "계획부터 투어 종료까지, AI 에이전트 + 휴먼 상담. 당신만의 프라이빗 한국 여행을 설계하세요." ("AI가 짜준다" 과장 제거→전 구간 AI+휴먼 동행 + 사용자 설계). (b) `choose-travel-style.tsx` 프라이빗 카드: privateDesc 아래 `privateFlexLine`(Clock 아이콘, emerald, "4–10시간 자유 선택 · 코스 직접 커스터마이징") 추가 → badge "유연성 UP"의 실증. i18n 6 로케일 `premium.v2.chooseStyle.privateFlexLine` 신규 + atfHeroSubhead 교체(node 패치, parse+flexLine 패리티 6/6). 검증: tsc 0 / build green | 
...
| ~~모바일 첫 화면 fold — 트러스트 카드 압축/히어로 축소로 회수~~ **→ 사장님 기각 (2026-08-07)** | 재실측 2026-08-07 (§2.6.2) | 🔴 **내가 §B "프리미엄=enrichment, 축소 금지" 를 어긴 제안이었다.** 사장님: *"뭔 트러스트카드를 한 줄로 고친다는둥 이상한 UI 다운그레이드를 시도하지 말고."* 인원 스테퍼가 fold 아래인 건 **스크롤로 해소되는 흐름 콘텐츠**라 애초에 기능 결함이 아니다 — 고칠 것이 아니라 그냥 두는 것이다. §2.6.2 의 측정치는 기록으로만 유효 |
...
| **AI 에이전트(챗봇) 홍보/강조 트랙 — "멍청한 챗봇 아님" 포지셔닝** (Phase F 제안). 풀퍼널 능력(상담·추천/매칭·견적·고객정보 수집·예약 조회/관리·휴먼 핸드오프) 실증. 4계층: (L1) 플로팅 런처 프로모션화(idle 티저 버블 + 능력 라벨), (L2) 전용 랜딩 밴드 "당신의 한국 여행 에이전트"(5능력 칩 + 샘플 대화 + CTA), (L3) hero/추천받기 카드 ↔ 챗봇 직접 연결(프로그램 오픈 훅 `atc:open-assistant` 신규), (L4) FinalCTA/Sticky 보조 경로. | 사용자 요청 2026-06-23 | **신규 스코프 — 활성 reform Phase(코드 완료·라이브 QA 대기)와 별개.** 코드 실사 완료: 챗봇=`GlobalAiAssistant`→`TourProductAiAssistantWidget`(scope=site), 백엔드 `app/api/tour-product/assistant/route.ts`가 RAG(`retrieveKnowledge`)+매처(`recommendToursViaMatcher`)+예약조회(`bookingLookup`)+휴먼 핸드오프(`support/escalation`+telegram)+QA/카탈로그 지식 보유 → 사용자 주장 능력 전부 실재(허위광고 가드 통과). 위젯에 외부 오픈 훅 부재(`:289` 내부 state·`:909` FAB만) → L3/L4 선행 작업 필요. 가드: §13 "최적/Best 인플레 카피 금지"(형용사 대신 동사·실능력으로 증명) + "긴 설명형 페이지화 금지"(밴드는 타이트하게) + baseline 없음→directional. **L1–L4 전부 SHIPPED 2026-06-23** (사용자 승인 "L1+L3 진행" → "이어서 진행"으로 L2+L4까지). §C 2026-06-23 (3)/(4) 참조. |
