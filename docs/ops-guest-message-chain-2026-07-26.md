...
- **가이드에게도 알린다.** 손님에게는 D-1 안내가 나가면서 가이드에게는 아무것도
  안 나가고 있었다. 가이드 1명당 1통으로 묶고, 단가 미설정은 `미정`으로 적는다
  (0원으로 적으면 무보수 합의처럼 읽힌다). 실패한 사람은 `notified_at`이 NULL로
  남아 "안내 안 함"으로 계속 보인다.
...
| 16 | **가이드 배정 안내 메일** | `/api/admin/guides/assignments/notify` · `lib/ops/guides/assignmentNotice.ts` · `notified_at` | ✅ **신규** |
| 17 | **오토파일럿 점검** | `ops_autopilot_suggestions` — 미배정·미배차·정원부족·단가누락·미정산·**안내 미발송** | ✅ **W5 신규** |
