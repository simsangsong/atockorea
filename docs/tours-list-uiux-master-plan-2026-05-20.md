...
| 2026-05-20 | **B25 §B reversal — Noto Serif KR (본명조) 적용.** 사용자 7차 지시: "한글폰트". app/layout.tsx Google Fonts link에 `Noto+Serif+KR` 추가. app/globals.css에 `.font-magazine-serif-ko { font-family: 'Noto Serif KR', 'Cormorant Garamond', Georgia, serif; ... }` 추가. CatalogueHero h1/sub/curator에 `font-magazine-serif-ko` 적용. h1 tracking -0.03 → -0.025 (명조 가독). sub/curator size 상향 (명조는 큰 weight 필요). | (pending) | type-check clean. 카드 SHA `a931fe4e` 유지. italic className = 0. scope = catalogue 전용 (Phase 2-4 surfaces 재사용 가능). |
