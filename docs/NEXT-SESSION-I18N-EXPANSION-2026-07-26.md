...
| 2e | 🔴 **`busan-private-car-charter-cruise-shore` stop 4 가 두 스톱이 섞여 있다** — 이름은 `Return to Busan Cruise Port` 인데 하위 필드가 갈린다: `timeUsed/0`·`highlights/0~2` 는 실제 귀항 내용(복귀 여유·터미널 하차)이고, **`timeUsed/1~4`·`highlights/3~8` 은 감천문화마을 내용**(Star Viewing Hall, Sky Garden 어린왕자 벤치, UN-HABITAT 상, 연 140만 방문객, 벽화 골목 25분 도보, 스탬프 투어 ₩2,000) | 손님은 "크루즈항 복귀" 제목 아래 25분짜리 골목 도보와 사진 명소 안내를 보게 된다. **동선 오해가 실제로 가능한 결함**이다 — 배로 돌아가는 구간에 관광 일정이 붙어 있는 것으로 읽힌다. 번역가가 발견했고 규칙 5대로 손대지 않았으므로 **4로케일 전부에 그대로 복제돼 있다.** 원문을 고치면 재추출·재번역이 따라온다 |
| 7h | **원문 로마자가 뭉개진 지명 1건** — `Songdo Yonggungungu Suspension Bridge` (송도 용궁구름다리) | `Yonggungungu` 는 `용궁구름다리` 가 뭉개진 형태다. 러시아어는 전사해야 하는데 **이 철자를 전사하면 존재하지 않는 지명이 만들어진다.** 두 유닛이 독립적으로 같은 판단을 해서 이 고유명만 라틴으로 두고 문단은 살렸다. 원문을 고치면 4로케일 재추출이 따라온다 — de·fr·it 은 로마자를 그대로 두므로 지금은 문제가 드러나지 않는다 |
...
| 21 | **G3 오탐 11 (ru 두 번째 슬러그)** — `The 2-hour stop` → `Двухчасовая остановка` | 러시아어는 **합성어에서 사격 어간**을 쓴다(`двух-`·`трёх-`·`одно-`·`семи-`). 수사가 낱말 속으로 들어가므로 어말 경계가 있으면 못 잡는다. 독일어 `4-story`→`vierstöckig` 와 같은 계열인데 어간이 아예 달라 기존 표로는 안 걸렸다 | 어간이 따로 필요한 1·2·3·7만 추가했다. `четыр`·`пят`·`шест`·`восьм`·`девят`·`десят` 은 이미 접두 매칭이라 합성형이 걸린다. `2-hour`→`Трёхчасовая` 같은 **진짜 값 변경은 여전히 fail** — 가드 테스트 포함 |

수정 19·20·21 반영 후 **de·fr·it 112 unit 재검증에서 실패 0 유지** — 이미 통과하던 것이 느슨해지지 않았음을 확인했다.
