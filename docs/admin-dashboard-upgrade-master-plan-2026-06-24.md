...
| 2026-06-26 | **W4.1 주문상세 머니시트(구현, §6.2/§8.2 — 사용자 범위 승인=안전+토큰화)** — Wave 4 머니 직결 페이지(`/admin/orders/[id]`). **핵심 안전: `window.confirm()` 2곳(상태변경·capture/release) 전면 제거→`ConfirmSheet`(바텀시트, 금액 prominent·문구·멱등).** iOS WebView가 `confirm()`에 무대화상자 `true` 반환→무확인 카드청구(돈 버그) 차단이 동기. **§6.2 톤**: capture/완료·release/현장=slate neutral("지금 [금액] 청구·취소불가"/"카드 청구 안 함"), 노쇼=red `destructive`(위약금 경고). **멱등**: 시트 confirm 버튼 `confirming=sheetBusy`로 첫 탭 후 disable + 액션버튼 `disabled={settling}`. **`alert()` 8개→`sonner` toast**(한국어). **sticky 모바일 머니 액션바**(`authorized`일 때만·`sticky bottom-16 md:hidden`=고정 바텀내비 h-16 위에 안착): hold 금액+만료칩(<48h amber·만료 red)+`카드 청구` 기본액션(동일 시트). **토큰화**: 카드쉘 `bg-white rounded-xl shadow-sm border-gray-200`→admin 토큰×6·`text-gray-600/900`→slate·indigo 액센트→blue-600 단일·상태노트 gray→slate·`min-h-11`·`📋`→`ClipboardList`(Lucide). 신규 로직 0(순수 UI·executor만 toast화)→신규 테스트 0. **refund는 별도 라우트(`/api/admin/orders/[id]/refund`)·UI 미도입(범위 외).** **연기**(풀 §8.2): 주문정보/결제/이티너리 아코디언 재구성·데스크톱 sticky 머니바 | (commit) | tsc 0·jest 560pass/8fail(=main 기존, 회귀0)·`next build` ✓ Compiled(`/admin/orders/[id]`). 신규의존성0. 라이브 검증 포인트: authorized hold 예약에서 capture/release/노쇼 시 바텀시트 금액 확인·toast·sticky바(모바일). 남은 Wave 4=W4.10 설정(게이트)·W4.11 횡단 |
| 2026-06-26 | **W4.10 설정 페이지 토큰 채택(구현, §5 저임팩트 — 토큰 부분만)** — `/admin/settings`(site_settings 영속은 W3.1 마이그레이션 게이트라 기능 불변, **순수 UI/UX 토큰 패스만**). `alert()` 3개(미로그인·저장성공·저장실패)→`sonner` toast(한국어). 카드쉘 `bg-white rounded-xl shadow-sm border-gray-200`→`bg-admin-surface rounded-design-md shadow-admin-card border-admin-border`·`text-gray-*`→slate·indigo(인풋 포커스링·토글·저장버튼·로딩 스피너) 전부→`blue-*` 단일 액센트·입력 `text-base min-h-11`(iOS 16px/44px)·h1 `text-3xl`→`text-xl font-semibold`(§5.1)·로딩 스피너→`Skeleton`. **dirty 가드 신설**: `baseline` 스냅샷 vs `settings` JSON 비교→Save 버튼 `disabled={!dirty||saving}`·저장 중 "저장 중…"·미저장 시 amber 힌트·`beforeunload` 경고(merchants-create 패턴 재사용). 감사로그 UI는 W5.4 데이터트랙→미도입 | (commit) | tsc 0·jest 560pass/8fail(=main 기존, 회귀0)·`next build` ✓ Compiled(`/admin/settings`). 신규의존성0·신규 로직 추출 없음(폼 상태만)→신규 테스트 0. 라이브 검증: 값 변경 시 Save 활성+amber 힌트·저장 시 toast·미저장 이탈 경고. 남은 Wave 4=W4.11 횡단(U-1 Realtime·U-3 가상 DataTable·U-6 undo·U-7 bulk·U-8 saved views) |
| 2026-06-26 | **W4.11-U8 Saved Views(구현, §P U-8)** — 횡단 인터랙션 첫 항목. 주문목록(`/admin/orders`, 이미 `useUrlFilters` 보유)에 **이름붙인 필터 프리셋**을 localStorage 영속. 순수 `lib/admin/saved-views.ts`(parse/serialize/upsert/remove/viewIsActive·`viewId` 슬러그·`MAX_SAVED_VIEWS=12` 캡·악성 JSON·비문자 필터 방어)+테스트 **+15**. `components/admin/SavedViews.tsx`(localStorage I/O만·`window.prompt()` 금지=인라인 이름입력·뷰 칩 클릭→적용·active 하이라이트·X 삭제·toast·dashed "현재 필터 저장"). 주문목록 배선: 적용 시 `setFilters({...FILTER_DEFAULTS, ...view})`로 전 키 리셋(부분뷰가 잔여필터 안 남김)·`filtersAreDefault` 가드(기본+무뷰면 숨김). **U-3(가상 DataTable)=가상화 lib 필요→신규의존성0 규칙 충돌로 보류·U-6(undo)=soft-delete 컬럼 DDL 게이트·U-1(Realtime)/U-7(bulk)=후속.** | (commit) | tsc 0·jest **575pass**/8fail(=main 기존, 회귀0·+15)·`next build` ✓ Compiled(`/admin/orders`). 신규의존성0. 라이브 검증: 필터 건 뒤 "현재 필터 저장"→이름→칩 생성·새로고침 생존·칩 클릭 복원·X 삭제. 다른 리스트(merchants/contacts/support) 확산=후속(S-U2) |
| 2026-06-26 | **S-U2 배치1 — merchants·support URL 필터 영속 + Saved Views(구현, §S-U2/§P U-8)** — useState만 쓰던 리스트의 필터 컨텍스트가 새로고침/공유/뒤로가기로 소실되던 갭(S-U2). **merchants**(`/admin/orders` 패턴 미러): `statusFilter` useState→`useUrlFilters({status})`(검색어는 per-keystroke URL 오염 막으려 로컬 state 유지, orders와 동일)·status select/effect/params 재배선·`SavedViews`(storageKey `admin:merchants:saved-views`). **support**: `filter` useState→`useUrlFilters({status})`·load() dep·status 쿼리 `encodeURIComponent`(소폭 보정)·필터 select/refresh 버튼 admin 토큰+`min-h-11`(탭타깃)·`SavedViews`(storageKey `admin:support:saved-views`). 둘 다 적용 시 `setFilters({...FILTER_DEFAULTS,...view})` 전키 리셋. W4.11-U8 컴포넌트/순수헬퍼 재사용→신규 로직0. **support 잔여 토큰 패스(bg-white 테이블·✓이모지·rounded-xl)는 별도 후속**(이번엔 필터 컨트롤만 토큰화). **contacts는 pagination 얽힘→S-U2 배치2 후속.** | (commit) | tsc 0·jest 575pass/8fail(=main 기존, 회귀0)·`next build` ✓ Compiled(`/admin/merchants`·`/admin/support`). 신규의존성0·신규 테스트0(기존 saved-views 15커버). 라이브 검증: 두 리스트서 상태필터→URL 반영·새로고침 생존·Saved View 저장/복원. 남은 S-U2=contacts(pagination)·support 풀 토큰패스 |
| 2026-06-26 | **S-U2 배치2 — contacts URL 필터 영속 + Saved Views(구현, §S-U2/§P U-8)** — 마지막 리스트. `filters` useState({status,is_read,search})→**status·is_read는 `useUrlFilters({status:'all',is_read:'all'})` URL 영속**(기본 sentinel `'all'`은 URL서 드롭)·**search는 로컬 state 분리**(per-keystroke URL 오염 방지). **pagination 처리**: 필터/검색 변경 시 `resetPage()`(page→1)로 빈 페이지 착지 방지·fetch effect deps `[status,is_read,search,page]`. `SavedViews`(storageKey `admin:contacts:saved-views`·isDefault=status·is_read 둘 다 'all'·적용 시 `setFilters({...DEFAULTS,...view})`+resetPage). `alert()` 2개(403·메모저장)→toast(한국어). 필터 카드 토큰화(bg-white→admin-surface·gray/indigo→admin/blue·셀렉트·검색 input `text-base min-h-11`). **상세 패널 풀 토큰 패스(gray/indigo 다수·로딩 스피너·목록 카드)는 별도 후속**(이번엔 필터 영역만). | (commit) | tsc 0·jest 575pass/8fail(=main 기존, 회귀0)·`next build` ✓ Compiled(`/admin/contacts`). 신규의존성0·신규 테스트0(saved-views 15커버). 라이브 검증: status·읽음 필터→URL 반영·새로고침 생존·페이지>1서 필터변경 시 1로 리셋·Saved View. **S-U2 전 리스트(orders·merchants·support·contacts) URL필터+Saved Views 완료.** 잔여=support·contacts 상세 풀 토큰 패스(별도) |
| 2026-06-26 | **support·contacts 풀 토큰 패스(구현, §5.x 폴리시 마무리)** — S-U2서 필터 영역만 토큰화하고 남긴 본문을 마저 정리(순수 UI·게이트0·의존성0). **support**(`/admin/support`): 카드쉘 `bg-white border-slate-200 rounded-xl shadow-sm`→admin 토큰·로딩 `loading…`→Skeleton 6행·`↻` 글리프→`RefreshCw`·`✓`→`Check`(emerald)·테이블 행 보더 slate-100→admin-border·error box rounded-design-md. **contacts**(`/admin/contacts`): gray/indigo 42곳 일괄→slate/admin/blue(`text-gray-*`→slate·`border-gray-*`→admin-border·`bg-white`→admin-surface·`bg-gray-100/50`→slate·hover gray→admin-surface-hover)·로딩 스피너(indigo)→Skeleton·선택행 indigo→blue·메모 textarea/저장버튼 indigo→blue-600+`min-h-11`·카드쉘 rounded-lg→rounded-design-md·shadow-sm→shadow-admin-card·h1 text-2xl→text-xl. 로직/데이터/모바일 시트 동작 불변 | (commit) | tsc 0·jest 575pass/8fail(=main 기존, 회귀0)·`next build` ✓ Compiled(`/admin/support`·`/admin/contacts`). 신규의존성0·신규 테스트0(순수 토큰). **Wave 4 페이지 토큰 폴리시 전 리스트 마감.** 남은 Wave 4=W4.11 잔여(U-1 Realtime=realtime config·U-7 bulk=신규 batch API·U-3/U-6 게이트)·W4.5 정산UI(게이트) |
| 2026-06-26 | **W4.11-U7 QA 리뷰 일괄 승인/거절(구현, §P U-7 — 사용자 대상선택=QA)** — `/admin/qa-review` 한장씩 플래시카드에 **목록(bulk) 모드** 추가. 헤더 모드토글(한장씩↔목록)·목록=다중선택 체크박스+전체선택+선택카운트·sticky 액션바(`bottom-16 md:bottom-0`=모바일 내비 위·승인 blue·거절 outline)·`ConfirmSheet` 확인(승인=색인 임베딩 안내 neutral·거절=비활성+색인제거 warning·멱등 `bulkBusy`). **신규 batch API `PATCH /api/admin/qa-pairs`** `{ids,action}`: 1쿼리 일괄 status update(authoritative)+per-id `syncQaPairToIndex`(approve=임베딩·그외=삭제, **순차·비치명적**→실패는 `failed` 카운트로 리턴, status는 SoT라 `rag:index` 재동기화). 공유 순수헬퍼 `lib/admin/qa-review.ts`(`QA_REVIEW_STATUS_MAP`·`resolveReviewStatus`·`parseBulkIds` 정수/양수/dedupe/캡100)+테스트 **+12**·per-id 라우트도 이 맵 재사용(중복제거). 키보드 단축키는 card 모드만. 로드 시 선택 초기화 | (commit) | tsc 0·jest **587pass**/8fail(=main 기존, 회귀0·+12)·`next build` ✓ Compiled(`/admin/qa-review`·`/api/admin/qa-pairs`). 신규의존성0. 라이브 검증: 목록 모드서 다중선택→승인/거절→toast(updated/failed)·승인분 챗봇 RAG 즉시 반영(대량은 `rag:index` 재동기화). U-7 주문 bulk은 돈 머신 표면→사용자 요청 시 별도. 남은 Wave 4=U-1 Realtime(config 게이트)·W4.5 정산UI(게이트)·U-3/U-6(의존성/DDL 게이트) |
| 2026-06-26 | **W4.11-U1 Realtime 라이브 피드(구현, §P U-1 — 사용자 승인 DDL 적용)** — supabase realtime publication에 **bookings·contact_inquiries·support_tickets 추가**(사용자 승인 후 `apply_migration` 라이브 적용·`supabase/migrations/20260626000000_realtime_enable_orders_inquiries_tickets.sql` repo 기록·idempotent). **보안 검증**: 세 테이블 SELECT RLS 확인→bookings=본인/머천트/admin·contact_inquiries·support_tickets=admin only→realtime은 SELECT RLS 적용이라 비-소유자/anon 누출 0(advisor 신규0). 재사용 훅 `lib/admin/useRealtimeActivity(table,{event})`(채널 구독·로드 후 도착분 카운트·`removeChannel` 클린업). 비파괴적 UX: 리스트에 **"새 N건 · 불러오기" 핑 칩**(자동 리오더 안 함·클릭 시 reset+refetch)을 orders(새 주문)·support(새 티켓)·contacts(새 문의)에 배선. inbox(3소스 통합)는 후속 | (commit) | tsc 0·jest 587pass/8fail(=main 기존, 회귀0)·`next build` ✓ Compiled(`/admin/orders`·`/admin/support`·`/admin/contacts`). 신규의존성0·신규 테스트0(I/O 훅·순수 로직 없음). 라이브 검증: 새 예약/문의/티켓 INSERT 시 핑 칩 등장→클릭 시 갱신. **U-1 핵심 3리스트 완료.** 남은 Wave 4=W4.5 정산UI(W5.2 게이트)·U-3/U-6(의존성/DDL 게이트)·U-7 주문 bulk(돈 표면)·inbox realtime(후속) |
| 2026-06-26 | **W4.11-U7 주문 일괄 상태변경(구현, §P U-7 — 사용자 범위=확정+완료만)** — `/admin/orders`에 다중선택 일괄 처리. **부작용 분석 후 범위 한정**: PUT 라우트에서 cancel=고객 취소이메일+재고복원(외부 발신)·no_show=정산 전용→**bulk은 confirmed/completed(부작용 0)만** 노출(사용자 확인). 신규 `PATCH /api/admin/orders`{ids,status}=대상행 status 조회→**B-3 상태머신 per-row 가드**(`partitionBulkTransitions`)로 valid만 1쿼리 update·illegal/terminal/cancelled는 skip 카운트로 리턴. confirm/complete는 머니/외부 부작용 0→단일 update 안전. 순수헬퍼 `lib/admin/orders-bulk.ts`(`isBulkOrderStatus`·`parseBulkOrderIds` uuid dedupe/캡200·`partitionBulkTransitions`)+테스트 **+8**(booking-status-transition 재사용). UI: 데스크톱 테이블 체크박스열+모바일 카드 체크박스(Link 분리)+메타 전체선택+sticky 액션바(`bottom-16 md:bottom-0`·확정 outline·완료 blue)+`ConfirmSheet`(결제 무영향·불가건 자동 skip 안내·멱등). 로드 시 선택 초기화 | (commit) | tsc 0·jest **595pass**/8fail(=main 기존, 회귀0·+8)·`next build` ✓ Compiled(`/admin/orders`·`/api/admin/orders`). 신규의존성0. 라이브 검증: 행 선택→확정/완료→toast(updated/skipped)·이미 완료/취소건은 skip. **일괄 취소는 의도적 제외(개별 화면 전용).** 남은 Wave 4=W4.5 정산UI(W5.2 게이트)·U-3/U-6(의존성/DDL 게이트)·inbox realtime(후속) |
| 2026-06-26 | **W4.11-U1 inbox realtime(구현, §P U-1 후속)** — 통합 인박스(`/admin/inbox`)에 실시간 "새 항목 N건·불러오기" 핑 칩. `useRealtimeActivity` 두 번(`contact_inquiries`+`support_tickets`, 이미 publication 등록됨) 카운트 합산·클릭 시 reset+`refreshNonce` 증가로 인박스 재조회(기존 load useEffect deps에 nonce 추가). 필터바 소스칩과 "미처리만" 토글 사이에 칩 배치. **received_emails는 publication 미등록→별도 DDL 승인 후 합류**(현재 contact·ticket 2소스만 카운트). 무-DDL·무신규API·신규 로직0 | (commit) | tsc 0·jest 595pass/8fail(=main 기존, 회귀0)·`next build` ✓ Compiled(`/admin/inbox`). 신규의존성0·신규 테스트0(I/O 훅). 라이브 검증: 새 문의/티켓 INSERT 시 인박스 칩 등장→클릭 시 갱신. **U-1 realtime=핵심 리스트(orders·support·contacts)+inbox(2소스) 완료.** 남은 Wave 4=W4.5 정산UI(W5.2 게이트)·U-3(가상화 lib 게이트)·U-6(soft-delete DDL 게이트)·received_emails realtime(DDL) |
| 2026-06-26 | **inbox realtime 3소스 완성(received_emails 합류, 사용자 승인 DDL)** — received_emails(admin-only SELECT RLS)를 supabase_realtime 퍼블리케이션에 추가(라이브 `apply_migration`+repo `supabase/migrations/20260626010000_realtime_enable_received_emails.sql`·idempotent·누출0). 인박스 `useRealtimeActivity` 3번째 소스(rtEmail) 합산→**인박스 핑 칩이 문의+티켓+메일 전 소스 실시간 반영**. reloadInbox에 rtEmail.reset() 추가 | (commit) | tsc 0·jest 595pass/8fail(=main 기존, 회귀0)·`next build` ✓ Compiled(`/admin/inbox`). 신규의존성0·신규 테스트0. **U-1 realtime 전면 완료(orders·support·contacts·inbox 3소스).** 남은 Wave 4=W4.5 정산UI(W5.2 게이트)·U-3(가상화 lib 게이트)·U-6(soft-delete DDL 게이트) |
| 2026-06-26 | **W4.11-U6 파괴적 액션 undo — merchants 소프트삭제 v1(구현, §P U-6 — 사용자 승인 DDL)** — 머천트 삭제를 hard delete→**소프트삭제(되돌리기 가능)**로. DDL(라이브+repo `supabase/migrations/20260626020000_merchants_soft_delete_u6.sql`): `merchants.deleted_at` + 부분인덱스 `idx_merchants_active(created_at) where deleted_at is null`. DELETE 라우트=`.delete()`→`.update({deleted_at:now})`(활성예약 가드·user role→customer 강등 유지). **신규 `POST /api/admin/merchants/[id]/restore`**=deleted_at=null+role 재승격('merchant'). 리스트 GET `.is('deleted_at', null)`로 숨김. UI: 삭제 성공 시 sonner **undo 토스트("실행취소", 10s)→restore**·ConfirmSheet note "토스트에서 바로 실행취소" 갱신(기존 "되돌릴 수 없음" 정정). 소프트삭제라 row 보존→`bookings.merchant_id` 조인 무손상·advisor 신규0 | (commit) | tsc 0·jest 595pass/8fail(=main 기존, 회귀0)·`next build` ✓ Compiled(`/admin/merchants`·`/api/admin/merchants/[id]/restore`). 신규의존성0·신규 테스트0(I/O). **연기(v1 범위 외)**: 퍼지 cron(소프트삭제 행 영구정리)·휴지통 뷰(토스트 외 복구 UI)·tours/QA 등 타 테이블 확장·create 라우트의 소프트삭제 user 재생성 충돌 처리. 라이브 검증: 삭제→리스트서 사라짐+토스트 실행취소→복구. 남은 Wave 4=W4.5 정산UI(W5.2 게이트)·U-3(가상화 lib 게이트)·U-6 확장(tours/QA·퍼지·휴지통) |
