...
**현재 활성 Phase: ✅ Wave 0(6건) main 머지완료(PR #167). ✅ Wave 1 기반 W1.1~W1.8 완료(토큰·16px·Toaster·유틸키트·44px·AdminPageShell·Tier1/2 키트; 테스트 30 그린). W1.9(이모지sweep)·W1.10(inbox 의존)·W1.11(CI)은 preview/Wave4 세션으로 연기. Wave 3(기능 안정화) 배치1 완료(LIB-1·S-F2·D-4·B-3·M-4·M-6·D-14·N-5·S-F7, 테스트+42). 남은 Wave3(D-1 대시보드·D-15 analytics math·M-7·AR-3·LIB-2/3 등)는 시각/마이그레이션/pricing 민감 → 일부는 preview/사용자 결정 세션. 진단(Phase 0~0.13) 종결. 실행은 §R WBS 기준. 다음 액션 = 사용자 결정(§J #11~14 + #2/#6/#8~10) → R0(N27 REVOKE·마이그레이션 정합·미진단라우트)+R2.0(모바일 기반) → Phase 1(§R-1). 🔒 보안 트랙(§R-9)이 P1·P2·W-3·CB-1/2·PA-1/2 등으로 대폭 확장(라이브 즉시 리스크 → 어드민 Phase와 병행/선행 권고). 💴 머니패스 무결성 트랙(§T.10) 신설 — W-10 환불경로는 §G 정산 선행. ⚠️ R0.2(repo≠live 정합)는 Phase 4 하드 선행. 준비사양 §Q — 승인 시 즉시 실행.**
...
| 2026-06-25 | **Wave 3 기능 안정화 배치(구현, 테스트 가능 로직 우선)** — LIB-1(구 이메일 템플릿 XSS escape)·S-F2(머천트 임시비번 Math.random→crypto 헬퍼)·D-4(create 응답 평문비번 제거)·B-3(주문 상태 state machine)·M-4(투어 DELETE 활성예약 가드)·M-6(stats "오늘" KST)·D-14(qa-pairs 카운트 legacy 값 정리)·N-5/S-F7(머천트 PUT allowlist+빈PUT 가드) | 682f8f54~08d22fe1 | 신규 단위/transport-mock 테스트 +42 그린, 전체 413 pass(8 fail=main 기존), 빌드 green. 헬퍼: temp-password·booking-status-transition·kst-day·merchant-update(전부 테스트) |
