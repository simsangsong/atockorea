...
- **예외 (비공개 유지 중에도 공개)**: **동부 소규모 투어 상세** — `/tour-preview/east-small-group-v2` 및 `/tour/...`의 East 소그룹 슬러그(`east-jeju-signature-small-group` 등), 각각 `-*` 접두, locale 접두 경로. **체크아웃·확인 하위 경로**(`/tour/.../checkout` 등)는 비공개.
