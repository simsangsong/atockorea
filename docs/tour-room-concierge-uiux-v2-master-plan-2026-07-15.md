...
하단 탭바 4개 중 "채팅" 탭만 브랜드 로고(검은 원 + 흰 "N") 아이콘, 나머지 3개(지도·오늘·설정)는 lucide 아웃라인 아이콘 — 탭바 안에서 아이콘 언어가 섞여 있다. 통일 필요.
...
### 기타 개정
- **탭바 아이콘 통일**: "채팅" 탭의 브랜드 로고를 lucide `MessageCircle`로 교체(다른 3탭과 언어 통일). 브랜드 마크는 헤더 등 다른 위치에서 이미 충분히 노출되므로 탭 아이콘에서까지 반복할 필요 없음.
...
### Wave V0 — 버그 수정 + 색 토큰 교체 【4】
- V0.1 ✅ 완료(`8a215289`) `/tour-mode`를 `GlobalAiAssistant` 격리 목록에 추가
- V0.2 `app/tour-room-theme.css` 토큰 전면 교체(§C 표) — 라이트/다크 둘 다
- V0.3 탭바 아이콘 통일(브랜드 로고 → `MessageCircle`)
- V0.4 가이드 아바타 색상 교체(오렌지 → 잉크/파치먼트)
...
| 웨이브 | 상태 | 비고 |
|---|---|---|
| V0 버그+색 토큰 | 🟡 진행 중 | V0.1 완료(`8a215289`), V0.2~V0.4 대기 |
| V1 색 적용 감사 | ⬜ 대기 | |
| V2 컨시어지 Tier0 | ⬜ 대기 | |
| V3 컨시어지 Tier1/2 | ⬜ 대기 | |
| V4 타임라인+쿠폰 | ⬜ 대기 | |
