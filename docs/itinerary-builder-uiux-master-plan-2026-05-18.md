...
| G — `POIDetailModal` (gallery + type) | ✅ complete | 2026-05-18 | 2026-05-18 | `d4c32edb` |
| H — `QuoteModal` + `/thanks` premium pass | ✅ complete | 2026-05-18 | 2026-05-18 | `ee055daa` |
| I — Motion identity (entrance + scroll-animate) | ✅ complete | 2026-05-18 | 2026-05-18 | (this commit) |
| J — Responsive sweep + Lighthouse + a11y | ✅ baseline complete; manual sweep deferred to ops | 2026-05-18 | 2026-05-18 | (this commit) |
...
| 2026-05-18 | (Phase I+J commit) | Phases I + J complete (combined). **Phase I — motion identity**: map section now wraps in a `motion.section` + `useRevealContainerProps()` + inner `motion.div` with `REVEAL_ITEM_VARIANTS` (extracted into a small `<MapSection>` sub-component so we don't nest motion children inside CartPanel). With this, the four builder surfaces (intake form, AI panel, POI grid, map+cart) each reveal as the user scrolls in — same cadence as home v2. All motion respects `useReducedMotion()` via the shared reveal helper. **Phase J — a11y baseline**: Esc-to-close on POIDetailModal + QuoteModal; `role="dialog" aria-modal="true"` on modals; aria-labels on close/grip/remove buttons; reset-view button gets `focus-visible:ring-2 focus-visible:ring-slate-400`; image thumbnails get aria-label per index. Manual Lighthouse + DevTools responsive sweep deferred to ops verification — not blocking. **UI/UX upgrade track ✅ COMPLETE.** Build green; all 3 routes 200 SSR. |
| 2026-05-18 | (Phase H commit) | Phase H complete — `QuoteModal` + `/thanks` premium pass. **QuoteModal**: AnimatePresence + backdrop fade + modal spring entrance (matches POIDetailModal motion identity). New gradient hero header (`from-slate-900 via-slate-800 to-amber-900`) with amber `Sparkles` eyebrow + slate `Custom itinerary` label — same identity as `/thanks` auto-quote variant so the funnel reads cohesive. Submit CTA gets a `Mail` icon (was no icon) when idle, `Loader2` when submitting; button retains width during submit via `opacity-90` shade rather than shrinking. Error banner upgraded to `AlertCircle` + 2-line `rose-50 ring-rose-200`. **`/thanks` pending variant**: card split into a gradient hero (`emerald-50 → amber-50`) + a "What happens next?" 3-step list (Mail → Search → Send) each as a `slate-50 ring-slate-100` row with numbered amber circle, ending with the existing CTAs. Build green. |
| 2026-05-18 | `d4c32edb` | Phase G complete — `POIDetailModal` polish. **Entrance**: `AnimatePresence` wrap + backdrop fade 200ms + modal `motion.div` spring (opacity/y/scale) — opens with the same identity as home v2 sections. **Hero gallery**: 16:10 mobile, **2:1 on desktop** (md+); bottom 48px fade overlay so dark heading text on the white card below reads cleanly when the image is bright. Thumbnail strip restyled — snap-x snap-mandatory scrollbar-hide, `w-16 h-12` thumbs, active `ring-2 ring-amber-500`, inactive `opacity-70 hover:opacity-100`. **Highlights**: bullets replaced with lucide `Check` icons (amber-600) per plan; cap visible at 6 + "Show all {N} highlights" / "Show less" toggle. **Description**: defaults to `line-clamp-5`; if longer than 320 chars, "Read more" / "Read less" toggle expands inline without scroll loss. **Footer**: primary CTA bumped to `bg-slate-900` slate variant (cohesive with home v2 dark CTAs), Add icon stays; Remove keeps the rose-50 outline style. **Esc key** closes the modal now (a11y). Build green. |
