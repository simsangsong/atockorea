...
| **Last commit touching this feature** | `9e1929b7` — feat(admin): match_pois editor + auto-upsert API (Phase 8 / D10) |
| **Owner** | simsangsong |
| **Reviewers** | — |
...
| 8 — Admin `match_pois` editor (tooling track) | 🔄 build complete; awaiting authed UI sign-off | 2026-05-21 | — | `29b9f646` · `9e1929b7` |
...
| 2026-05-21 | `9e1929b7` | **Phase 8 build complete (8a–8g).** Shipped sidebar/breadcrumb entry, `GET /api/admin/match-pois` (collection), `GET + PATCH /api/admin/match-pois/[poi_key]` (auto-upsert with whitelist + validation + poi_meta `{}`-on-insert default), `lib/admin/poi-override-pins.ts` (defensive, no-op on main), and the `/admin/match-pois` list+editor UI (Identity/Location/Image/Content/Matching-profile sections, jsonb sub-forms + validated JSON textareas, dirty-track + Save + Sonner toasts + override-pin badge + new-POI creator). **Verified:** `npm run build` green on the main base (3 routes emitted); SQL upsert round-trip proves INSERT-new + UPDATE-existing (`onConflict: poi_key`) incl. poi_meta `{}` default + `is_operational` generated; data-quality self-check clean (0 out-of-range coords, 0 null poi_meta, no new empty-`{}`, test row cleaned). **Pending:** authed in-browser UI sign-off (admin login required). |
| 2026-05-21 | (recovery) | ⚠️ **Concurrent-session collision + recovery.** The shared working dir `C:/Users/sangsong/atockorea` was switched off `feat/admin-match-pois-editor` back to `feat/unified-landing-planner` by another active session mid-build; my planner commit (`fbc37c45`) + my uncommitted feature files were absorbed there (the feature files were `git add -A`-swept into the other session's commit `0338ff9f "feat: add match POI admin workflow"`, which **mixes** my 9 admin-editor files with that session's 3 `components/itinerary-builder/{BuilderShell,POICatalogGrid,ResultTimeline}.tsx` result-timeline changes). To avoid rewriting another session's branch, I recovered into an **isolated worktree** `C:/Users/sangsong/atockorea-match-pois` on `feat/admin-match-pois-editor` (off clean `main`): cherry-picked my planner edit (`29b9f646`) + checked out **only my files** from `0338ff9f` (`9e1929b7`), then re-verified the build on the main base. `feat/unified-landing-planner` was left intact for the other session. **Residual:** my work is duplicated (entangled) on `feat/unified-landing-planner`; the clean, independent copy is on `feat/admin-match-pois-editor` per the user's branch requirement. |
...
**Tasks:**
- [x] (8a) Sidebar + breadcrumb: add `{ path: '/admin/match-pois', label: '매칭 POI 관리' }` to `adminMenuItems` + `pathToBreadcrumb` in `app/admin/layout.tsx`. — `9e1929b7`
- [x] (8b) `GET /api/admin/match-pois` — collection list route. `requireAdmin`; returns trimmed rows (poi_key, name_en, name_ko, region, category, default_image_url, is_attraction, stop_role); `_`-prefixed junk filtered out server-side; `override_pinned` flag per row. — `9e1929b7`
- [x] (8c) `GET /api/admin/match-pois/[poi_key]` — fetch one full row (`requireAdmin`, `maybeSingle`), include `override_pinned`. — `9e1929b7`
- [x] (8d) `PATCH /api/admin/match-pois/[poi_key]` — auto-upsert. `requireAdmin` → validate → whitelist columns (exclude `is_operational`/`created_at`/`updated_at`/`embedding`) → default `poi_meta` to `{}` if a brand-new row → `supabase.from('match_pois').upsert(payload, { onConflict: 'poi_key' })` → return `{ data, message }` (message says "created" vs "updated"). — `9e1929b7`
- [x] (8e) Validation: lat ∈ 33.0–38.7, lng ∈ 124.6–131.0 (when non-null); highlights/images = arrays of non-empty strings; visit_basics/convenience/smart_notes = object-or-null (treat `{}` as null, Empty-Object Rule); names_other_locales/matching_profile/poi_meta = object; integers for default_stay_minutes/builder_profile_version; reject unknown columns. — `9e1929b7`
- [x] (8f) Defensive override-pin helper `lib/admin/poi-override-pins.ts` — reads `poi-image-overrides.mjs` only if present; returns empty `Set` on `main`; activates when dq branch merges. — `9e1929b7`
- [x] (8g) `app/admin/match-pois/page.tsx` + `_components/` (list pane + editor pane) + `_hooks/` (list hook + single-row hook + save). List pane: search + region-cluster filter (busan/jeju) + is_attraction filter + thumbnail/poi_key/name. Editor pane: sticky header + Save + dirty-tracking + Sonner toasts; sections **Identity / Location / Image / Content / Matching profile**; jsonb rendered as structured sub-forms (names_other_locales, highlights/images string-list) + validated JSON textareas (visit_basics, convenience, smart_notes, matching_profile, poi_meta); override-pinned badge + "a re-seed wins" note in the Image section. — `9e1929b7`

**Acceptance:**
- [x] `npm run build` green — verified in the `feat/admin-match-pois-editor` worktree (main base): 3 routes emitted (`/admin/match-pois`, `/api/admin/match-pois`, `/api/admin/match-pois/[poi_key]`).
- [x] A brand-new `poi_key` INSERTs via auto-upsert; an existing one UPDATEs — proven via SQL round-trip exercising the exact `ON CONFLICT (poi_key) DO UPDATE` semantics `supabase.upsert` emits (`zzz_admin_editor_test` INSERT → UPDATE in place, row_count stayed 1; `poi_meta` defaulted `{}` on insert; `is_operational` computed `false`; test row deleted after).
- [x] SQL data-quality self-check: 0 out-of-range lat/lng, `null_poi_meta` 0, no NEW empty-`{}` fields (the pre-existing 1×visit_basics + 1×convenience untouched), 0 test rows leftover, total back to 91.
- [ ] **Pending authed browser sign-off (user):** `/admin/match-pois` loads behind the admin gate, lists the 84 attractions (junk `_`-rows excluded), filters work, and an in-UI edit + Save persists. Route compiles + is emitted and the API/upsert contract is proven above, but the full authenticated click-through needs an admin login that the build agent cannot perform.
