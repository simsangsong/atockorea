...
| **Blocked on** | — |
| **Last updated** | 2026-06-23 — **Builder refinement track (Phase A–D + D.2) ✅ COMPLETE** (A=사진숨김+동선60%경고 / B=AI추천 재활성화 / C=지역 묶음 / D=제주 추가요금 East-aware+캡10만 / D.2=구글맵 호텔 zone 자동판정). ⚠ §A의 "Phase 13" 이후 PR #110(빌더 de-AI)·홈 reform은 이 doc에 미반영 — 코드 실사 우선. |
| **Last commit touching this feature** | `2964ffde` — Phase 12 merged (PR #101) on main. Refinement track Phase A: see §C 2026-06-23. |
| **Owner** | simsangsong |
| **Reviewers** | — |
...
| 2026-06-23 | D41 | **Builder refinement track 착수 (Phase A–D, 신규 스코프).** A=사진없는 POI 숨김 + 동선 60% 경고 / B="AI 추천받기" 재활성화(de-AI PR #110 부분 번복, 보존 매처 재사용, 카테고리 필터 유지+AI 추가) / C=지역 묶음 규칙(제주 East 격리·W+S 묶음 / 부산 시내·영남·경주 / 서울·가평·파주·포천 각각) / D=제주 추가요금(East-mix +6만·시외호텔+6만·West호텔→East투어+4만). 순서 A→B→C→D, 각 독립 PR. | 사용자 요청·확정 2026-06-23. ⚠ §A가 stale(Phase 13/2026-05-29 이후 PR #110 de-AI·홈 reform 미반영) — 코드 실사 우선으로 진행. |
| 2026-06-23 | D42 | **지역 묶음 위반 동작: 서울·부산 클러스터=하드 블록(추가 불가), 제주 East 혼합=경고+요금(+6만).** | 사용자 결정. 제주 East는 추가요금으로 허용, 타 권역(서울/가평/파주/포천, 부산시내/영남/경주)은 동선·상품 구조상 혼합 무의미 → 차단. |
| 2026-06-23 | D43 | **서울권 세분(가평/파주/포천)을 위해 region="gyeonggi" 17 POI를 좌표/이름으로 자동 재태깅 후 사용자 검토.** | 데이터 공백: 현재 가평/파주/포천이 전부 gyeonggi로 뭉쳐 세분 불가. Phase C 선행 데이터 패스. |
| 2026-06-23 | D44 | **사진없는 POI 숨김 = 빌더 한정·서버필터(`hasBuilderPhoto`), 일시적.** ⚠ 제주 가시 POI 61→26(35 숨김). POI 사진 트랙(Track B) 진행 시 자동 복구. | 사용자 "일단 숨겨". 임시 카탈로그 빈약 수용. |
...
| 2026-06-23 | (pending) | **Refinement track Phase A 완료 — 사진없는 POI 숨김 + 동선 60% 경고.** (A1) `lib/itinerary-match-engine/poi-taxonomy.ts` `hasBuilderPhoto()`(default_image_url ‖ images[0]) 추가 → `/api/itinerary-builder/pois/route.ts` + `app/itinerary-builder/page.tsx` 필터에 `&& hasBuilderPhoto(p)` (⚠ 제주 가시 61→26, D44). (A6) `ResultTimeline` `durationMinutes` prop + Footer `driveTooHigh`(driveMin/durationMin>0.6) amber 경고; `BuilderShell`이 `tourDurationHours*60`(URL duration/hours 기본8) 전달. i18n `itineraryBuilder.cart.driveRatioTooHigh` 6 로케일. §B D41–D44 logged. 검증: tsc 0 / build green. **다음: Phase B (AI 추천 재활성화).** |
| 2026-06-23 | (pending) | **Refinement Phase B 완료 — "AI 추천받기" 재활성화 (de-AI PR #110 부분 번복, D41).** 보존된 `AIRecommendPanel`(intent+5 preset+`POST /api/itinerary/match`)을 `BuilderShell` 우측 레일(LivePriceCard↔ResultTimeline 사이)에 재마운트: `onAccept`→`reorder(keys)`(추천 시퀀스를 카트에 교체 적용), `onPreview`→신규 `previewKeys` state→`POICatalogMap`(매칭 중 핀 프리뷰, 기존 `previewKeys={null}` 대체), `track`=URL. 빈카트 "가이드에게 일정 맡기기" CTA 제거(`onGuideCurate` prop + `handleGuideCurate` 삭제) — 추천 경로가 AI로 대체. CategoryFilterBar(수동 브라우즈) 유지(D42). i18n 기존 `itineraryBuilder.ai.*` 재사용(eyebrow "AI가 추천하는 나만의 하루", CTA "추천받기"). 검증: tsc 0 / build green. **다음: Phase C (지역 묶음 규칙) — 서울 gyeonggi 재태깅 선행(D43).** |
| 2026-06-23 | (pending) | **Refinement Phase C 완료 — 지역 묶음 규칙 (D41–D43).** gyeonggi 18 POI를 좌표박스로 클러스터화 사용자 확정(가평 lng≥127.33 / 파주 lat≥37.85·lng≤127.0 / 포천 lat≥37.85·lng>127.0 / 남부근교=나머지). 신규 `lib/itinerary-builder/clusters.ts`: `tourCluster(poi)`(region+좌표→cluster, **DB region 미변경** — REGION_CLUSTER·가격 안전), `combine(a,b)`(같으면 ok / 제주 city중립·W+S무료·East혼합 warn / 그 외 cross=block), `cartAddDecision`, `cartHasJejuEastMix`(East+W/S). `BuilderShell`: `cartClusters`/`blockedKeys`/`jejuEastMix` 메모 + `addGuarded`(block이면 add 무시) → grid·map onAdd에 적용, grid에 `blockedKeys` 전달(추가버튼 비활성+"다른 권역"). 제주 East 혼합 시 amber 경고 배너(요금은 Phase D). i18n `grid.blockedRegion`+`cart.jejuEastMixNotice` 6 로케일. 유닛 7 / tsc 0 / build green. **다음: Phase D (제주 추가요금 — East-aware 리팩터 + 시외/West-hotel-East-tour, ⚠ 중복부과 여부 확인).** |
| 2026-06-23 | (pending) | **Refinement Phase D 완료 — 제주 추가요금 (D41·D44, 트랙 종료).** 사용자 확정: 시외호텔=기존 outer존(+6만), 동쪽섞음=East+W/S(+6만), 시외 반대편 픽업(+4만), **합산 상한 10만**, 픽업드롭다운에 방향 추가. `pricing-policy.ts`: `JejuPickupZone` enum `city/north/outer/cross_island`→`city/out_west/out_east/out_south`(+6만), `isJejuCrossRegion`→`isJejuEastMix`(East+W/S), 신규 `jejuPickupSide`+`isJejuCrossSidePickup`(서hotel↔동tour 양방향 +4만), `JEJU_SURCHARGE_CAP=10만`(jeju 3종 합산 초과 시 `jeju_distance_capped` 단일라인). `quote()` 제주 블록 재작성. UI: `PlannerTopRail`/`api/book` PICKUP_ZONES 새 값, `price-line-labels`/`LivePriceCard` 신규 코드 라벨, i18n `pickupZones`(2블록)+`lines.{jejuEastMix,jejuCrossSide,jejuDistanceCapped}` 6 로케일. 테스트 39(+신규 east-mix/cross-side/cap/city). ⚠ 후속 Phase D.2(선택): 구글맵 호텔 선택→zone 자동판정. ⚠ 기존 builder booking의 옛 zone값은 재검증 시 null(건수 ~0). 검증: 유닛39 / tsc0 / build green. **트랙 A–D 완료.** |
| 2026-06-23 | (pending) | **Refinement Phase D.2 완료 — 구글맵 호텔 자동 zone 판정.** `pricing-policy.ts` `classifyJejuHotelZone(lat,lng)`(시내 박스 lat33.48–33.55·lng126.44–126.575 / 남부 lat≤33.32 / 서 lng≤126.42 / 동 lng≥126.62 / 중앙은 126.51 기준 서·동). 신규 `HotelZoneAutocomplete`(클래식 `google.maps.places.Autocomplete`, places는 POICatalogMap이 이미 로드 → window.google 대기·폴링·12s 타임아웃, 미로드 시 input disabled로 degrade). `PlannerTopRail` 제주 픽업 Field에 호텔검색 input 추가(선택 시 `patch({pickup: zone})`) + 기존 드롭다운은 수동 폴백 유지. i18n `planner.hotelSearchPlaceholder` 6 로케일. 유닛40(+classifier 7케이스). ⚠ Places API가 키에 enable+referrer 허용돼야 동작(env, 코드 아님). 검증: 유닛40 / tsc0 / build green. **트랙 A–D + D.2 완전 종료.** |
...
- [x] (10.6) `/admin/orders` source filter + builder-row itinerary section; currency-aware sweep across admin/customer surfaces; builder cancellation email parity. — Phase 10.6a `64069fb4` (shared `lib/format/currency.ts` helper; /api/admin/orders accepts source filter + returns currency/source/itinerary/booking_reference; admin orders list adds chip strip + builder badge + currency-aware amounts; admin detail page branches to Custom Itinerary card for builder rows + currency-aware Pricing + Hold Amount derived from final_price+currency; admin dashboard splits revenueByCurrency into {usd,krw} and shows USD headline + KRW sub; customer receipt /api/bookings/[id]/receipt uses formatBookingPrice). 10.6b `514083d8` (new `lib/email-templates/builder-booking-cancellation.ts` emerald-tone template; admin cancellation route branches on source so builder bookings actually receive a cancellation email; inventory restore guards on tour_id with explicit log). `/admin/itinerary-quotes` sidebar entry was already removed in 10.5c (the route is deleted; no sidebar reference exists in app/admin/layout.tsx). tsc clean, build green.
- [x] (10.7) i18n hand-edit for all new keys × 6 locales (per `feedback_i18n_translate_script_drops_keys`) — `6de0159b`. Added `itineraryBuilder.planner.*` (27 keys × ko/ja/zh/zh-TW/es) + `errorDateRequired` / `errorOutOfScope` / `errorPriceChanged` + transcreated `getQuoteCta`/`getQuoteHint` to the new "Book + hold card · card saved · captured on tour day · 24h cancellation" tone in all 5 locales. 6-locale parity restored.
- [x] (10.8) Phase 10 complete — awaiting PR review + merge to main. All 8 sub-phases + 3 audit checkpoints landed; 63/63 Phase 10 unit tests green; tsc + build clean; browser-verified (premium stone-50 + mint floating cards) in 414×896 + 1280×800 viewports; Stripe stack KRW-ready (no live charges yet — cut-over runbook §M lists the in-prod smoke test). Pre-existing test failures (logger, error-handler, /api/tours, test-utils) confirmed unchanged from origin/main — not Phase 10 regressions.
