...
| 2026-05-19 | Seasonal variations carousel 모바일 vertical scroll 갇힘 fix (사용자 보고) | (다음 커밋) | `TourPracticalDetails.tsx:334` `touch-pan-x` Tailwind class가 `touch-action: pan-x` 적용 — y축 touch pan을 차단해 carousel 위에서 vertical scroll이 막힘 ("화면 아래로 쭉 내리다가 여기서 계속 멈춰버려"). 제거 → browser default `touch-action: auto`로 환원, browser가 swipe dominant axis 자동 판단 (horizontal = carousel scroll, vertical = page scroll). Apple Photos / Klook carousel 표준. `overflow-x-auto + snap-x snap-mandatory + overscroll-x-contain + scroll-smooth + [-webkit-overflow-scrolling:touch]` 모두 유지 (horizontal carousel 기능 보존). 이 carousel만 영향 (코드베이스 grep 결과 `touch-pan-x` 사용처 1곳뿐). §6.8 scroll-freeze 가드 보호 강화. |
