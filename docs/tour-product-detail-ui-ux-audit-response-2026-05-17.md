...
| **Sprint 5 (정보 과밀 reversal + Apple/Airbnb 럭셔리 미니멀 — §B-P4/P5/P6 가드)** | 🔄 (5.1-5.4 ✅ / 5.5 ready) | 2026-05-18 | — | (5.4 pending commit) |
| Sprint 6+ (장기 polish) | 📦 | — | — | — |
...
| 2026-05-18 | Sprint 5.4 — Fit Best For 5+개 시 첫 4 visible + "Show all X" underlined link (conditional, 4 이하면 그대로) | (이 커밋) | `TourFitSection.tsx`. `BEST_FOR_VISIBLE_LIMIT = 4` 상수 + `showAllBestFor` state + `overflowBestFor` 계산 + `visibleBestFor` slice. Best For 헤더 row를 `flex justify-between` 으로 변환, 우측에 conditional underlined link (Sprint 5.2 Included "Show all X" 패턴 그대로 — `text-[12.5px] font-semibold text-[var(--primary)] underline decoration-[1.5px] underline-offset-[3px]`). 4 visible item grid (bg-slate-50 + ring + persona icon) 변경 0 → §B-P6 통과 유지. §B-P5 4조건 (preview = 카드 meta line의 "{n} traveler types" count + underlined link affordance + single shadow tier 유지 + smooth 0.30s 불필요 — instant toggle). typecheck clean. **테스트:** 6-item tour (busan-plum-cherry-blossom-day-tour-to-yangsan-gyeongju) DOM 검증 — "Show all 6" 버튼 렌더링 ✓, 닫힌 상태 4 visible ✓. preview tooling은 layout 계산 환경 제약으로 click toggle 시각 검증 못함 (offsetParent null), 그러나 패턴은 5.2와 동일하므로 코드 안전. |
...
| Animation tempo 4-step scale 통일 | Sprint 6+ | 분기별 |
...
목표: Sprint 3에서 over-expand된 1차 visible scope를 §B-P4 기준으로 회복하되, **§B-P5 4조건 + §B-P6 6조건 모두 유지**. **"펼쳐서 텍스트 덩어리" → "닫혀도 작은 책 한 페이지처럼 정성스러운 미니멀 카드 + 열면 더 자세히"** 전환. 텍스트 부피 ↓, 그러나 *editorial richness · premium material · typography hierarchy · micro-hover · photo prominence* 모두 ↑.
...
| **5.5** | **Practical accordion preview text editorial 강화** — default closed 유지 + preview text "3 items · pickup, drop-off, what to bring" 같은 utility-list → editorial parallel ("Pickup zones · drop-off bands · what to pack" 식). preview line typography: title (font-semibold) + preview (text-[12.5px] muted parallel structure) — 2-layer 명확. | `TourPracticalDetails.tsx:337-355` | §B-P6 1+6 (typography + editorial copy) |
| **5.6** | **Reviews summary prose 1줄 + Show all link prominence ↑**. summary 카드에 hero data 외 1줄 editorial prose 추가: "{averageRating}★ · {count} reviews · guests highlight {top2highlights}" (이미 `reviewsSummary.highlights` 데이터 있음). Show all link이 이미 underlined 패턴이지만 hierarchy 강화 (icon + 폰트 size). | `TourReviewsSection.tsx:144-238` | §B-P5 1+2 + §B-P6 1+6 |
| **5.7 (신규)** | **Card material 통일 — Inner highlight + hover lift 누락 카드 보완**. Sprint 2.9 SEASON_THEME_SHARED 패턴 (`absolute inset-x-0 top-0 h-1/3 bg-gradient-to-b from-white/65 to-transparent` + `hover:-translate-y-[1px] hover:shadow-elevated`)을 누락 카드에 보완: Included 카드 / Fit 카드 / At-A-Glance 카드 / Reviews summary 카드. Booking Support Trust 3-grid는 이미 ✅. | `TourIncludedSection.tsx`, `TourFitSection.tsx`, `TourAtAGlance.tsx`, `TourReviewsSection.tsx` | §B-P6 3+5 |
...
- 모든 2차 reveal에 **§B-P5 4조건** 통과 (미리보기 1줄 + underlined link + single shadow tier + smooth 0.30s)
...
- typecheck clean
- §B-P3 premium layout 손상 없음 (모든 카드 single shadow tier 유지)
