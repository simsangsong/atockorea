...
| 2026-06-23 | **별도 트랙 — 프라이빗 차터 상세페이지 "샘플 일정" 슬롯 + 프라이빗 투어 규칙 섹션** (사용자 "동/남/서남 버스투어 일정을 샘플로, 입장료·식사만 미포함, 일정 슬롯만 만들고 import는 나중에, logic 지우고, 픽업/드롭오프 호텔 디폴트, 일정 섹션 밑에 규칙 상세 설명") | (이번 commit) | 대상=`jeju-island-private-car-charter-tour` (6 로케일). **Sprint 1-4 비주얼 작업과 무관한 신규 콘텐츠/데이터 트랙** (사용자 전용 브랜치 `claude/private-tour-sample-itinerary-7kwkjp`). 변경: ①`itineraryStops` 기존 4 루트(동/서/남/맞춤) → **3 샘플 슬롯(동/남/서남)** 으로 교체, 각 슬롯=제목·지역·요약·하이라이트4·timeUsed4 골격 + `_poi_meta.source_bus_tour`+`pending_import:true` (상세 stop은 추후 import). ②`whyTourWorks.routeLogicSections` → `[]` ("logic" 제거); `TourFitSection`은 sections 비면 route-logic 카드 **숨김**. ③included 항목 `variant` 로컬라이즈 토큰("포함/含む/…") → 정규 `"included"` 정규화(=ko/ja/zh/zh-TW/es에서 안 뜨던 pre-existing 버그 수정) + `includedCount:4` → **입장료·식사만 미포함**. ④`hero.pills[2]` 동/서/남 → 동/남/서남. ⑤신규 `privateTourPolicy` 필드 + `TourPrivateTourPolicySection` 컴포넌트, `#itinerary` 바로 아래 렌더(조건부, 타 상품 무영향). 픽업/드롭오프는 이미 `hotel_pickup` 디폴트 — 유지. 색상=master plan color-diet 준수(slate 중립 + icon chip). 검증: 6 JSON parse OK / 데이터 검수 OK. ⚠ 라이브 QA + 실제 버스투어 일정 import 후속. |
