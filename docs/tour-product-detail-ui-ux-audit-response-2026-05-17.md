...
| 2026-05-18 | Sprint 3 ✅ — 9/9 + acceptance 통과 | (이 커밋) | accordion 8→2 binding 달성 (Pickup/Included/Fit Best+Less+Route/Support 5 폐기). eyebrow 10→1 spec. section heading 12 통일. At-A-Glance 6색 text pill. Subnav underline + 양쪽 fade + IO top-most. |
| 2026-05-18 | Sprint 3.1 — At-A-Glance 6색 무지개 dots + 5-step progress dots → text pill | 161b751e | ROW_ACCENT_COLORS 6색 cycle (emerald/amber/orange/rose/violet/sky-500) + L72 5-step progress dots 폐기. pill 디자인: bg-slate-50 + text-slate-900 + ring-slate-200 + font-bold + tracking-wide (Klook/Booking pattern). rounded-[26px] → rounded-2xl (§1.3 body card scale). ARIA role="img" + aria-label "(n/5)" 유지. 5 decoration 인스턴스 제거. §B-P1: ring + font-bold = editorial weight (premium up). |
| 2026-05-18 | Sprint 3.8+3.9 — Subnav pill → underline 2px + 양쪽 fade + IO top-most 정렬 | a1b30662 | 3.8: bg-foreground filled pill + shadow → border-b-2 var(--primary) brand underline (Apple TabBar 패턴). 3.9: right-only fade → left+right gradient + IntersectionObserver 모든 currently-intersecting target 중 boundingClientRect.top 최소 ID 선택 (oscillation 제거). §B-P1: filled pill 강한 무게 → 2px underline 절제 = Hermès UI editorial weight. |
| 2026-05-18 | Sprint 3.4 — Pickup row expand 폐기, note 있는 행만 항상 표시 | 4924512e | useState(expandedOrder) + ChevronDown rotate + grid-rows accordion 전체 폐기. hasNote ? <note panel /> : null 단순 분기. row click 비활성. §B §2.3 accordion 8→2 카운트 1 감소. §B-P1: 정성 노출이 premium (Apple/Klook). |
| 2026-05-18 | Sprint 3.3 — Included accordion 해제 → 5 always visible + Show all link | 4e704fb7 | 외부 accordion (open state) 폐기. 첫 5개 항상 노출. 6개+ 시 "Show all N" link (text-link, no chevron). excluded 항상 노출 (보통 ≤5). VISIBLE_LIMIT=5 상수. §B §2.3 카운트 누적 2 감소 (Pickup 1 + Included 1). §B-P1: critical info hiding 폐기 = Klook 표준. |
| 2026-05-18 | Sprint 3.2 — Fit 3중 nested accordion 해제 → 2-col flat + Route Logic 펼침 | 6fda4d78 | Best For (open) + Less Ideal (showLessIdeal) + Route Logic (showLogic) 3개 useState 폐기. Best For + Less Ideal sm:grid-cols-2 + sm:divide-x. Route Logic 별개 white card 항상 펼침. data routeLogicSections.iconBg sky-50/amber-50 decoration → bg-slate-100 neutral 무시. §B §2.3 카운트 5 감소 (누적 5/8). |
| 2026-05-18 | Sprint 3.5 — Support default-open accordion 폐기 → 항상 펼친 timeline | addff9d8 | showTimeline state + ChevronDown rotate + grid-rows transition 전체 폐기. card header (h3 + subtitle) + border-t로 분리된 timeline 영역. mobile vertical / desktop horizontal scroll 유지. §B §2.3 카운트 1 추가 감소 (누적 6/8). 남은 accordion: FAQ + Practical = 2 → §2.3 binding 목표 도달. §B-P1: default-open accordion = click 시 닫힘 (정성 손실) → 항상 펼침 = Apple Card pattern. |
