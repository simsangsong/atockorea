...
| 2026-08-08 | **§B-P5 — Stop Detail Drawer(§8.6) 프리미엄 패스는 사용자 직접 지시로 선행 실행** — Sprint 순서와 별개로 "itinerary drawer 저급함 원인 분석 + 프리미엄 업그레이드" 지시(2026-08-08). §8.6 권고 중 blue `Full description` 버튼만은 **2026-08-02 사장님 결정(눈에 띄는 파랑 유지)이 우선**이라 손대지 않음 | 실행 상세 = §C 2026-08-08 행. 이 행이 §8.6의 "Full description pill → 텍스트 링크" 권고를 대체한다 |
...
| 2026-08-08 | **Stop Detail Drawer 프리미엄 패스(§8.6 실행 + 실렌더 재감사)** — 사용자 지시 "itinerary drawer 저급함 원인 분석·프리미엄화". Playwright 실렌더+DOM 실측으로 §8.6이 못 본 기계적 원인 3건 발견: ① **타임라인 `space-y-7` 마진이 fixed 서랍·백드롭에 상속 → 28px 내려앉아 상단에 페이지 미차폐 띠**(모바일에서 day-flow 스트립이 서랍 위에 떠 보임) ② **`/알파` 브랜드색 유틸 ~15곳 전멸**(config 색이 `var(--x)` 문자열이라 Tailwind가 클래스 자체를 드랍 — `ring-primary/85`→스톡 blue-500 폴백, `bg-sand-blush/80`·`bg-muted/80`·`bg-primary/[0.08]`·액센트바·드래그핸들 전부 투명) ③ **`--font-display-serif` 미정의**로 모달 세리프 리드가 sans 폴백. 수정: **portal(가장 가까운 scope root, 폴백 body+`contents` 래퍼)** · 죽은 유틸 전부 `color-mix()` 아비트러리로 소생 · 세리프를 `--font-tour-v2-serif`(Bodoni) 스택으로 · 모달 **첫언급 밑줄 폐지+28자 초과 볼드는 잉크 강등**(링크팜 해소) · `*이탤릭*` 파서 추가(리터럴 별표 노출 해소) · 히어로 5플로트→3(중복 duration 필 제거·크레딧 오버레이 제거·닫기 `bg-white/15`→white/95) · 카테고리 배지→앰버 아이브로·H2 20→22/24px·메타 정돈 · 썸네일 active 브랜드 링+inactive 딤 · 콜랩시블 셰브론 드라마 제거·`whyOnRoute`=`--tpc-amber-wash` · whyOnRoute "Read more" 영어 하드코딩→`reviewsReadMore` 재사용(6로케일) · 서랍 슬라이드 0.42→0.36s. 검증: tsc 0 · 관련 게이트 13 pass · 모바일/데스크톱 18샷 재렌더 확인 | (이번 커밋) | 스마트앱 플랜 에디터(PlanStopCards)도 같은 서랍 재사용 — portal 폴백 경로가 그쪽 문맥 보존 |
