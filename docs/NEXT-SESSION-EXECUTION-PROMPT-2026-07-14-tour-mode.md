...
**현재 상태:** **Wave T0 완료(9/9, T0.3 라이브 적용만 잔여) + Wave T1 12티켓 중 10.5개 완료**(T1.1~T1.8, T1.10, T1.12 완료 / T1.11 부분 / T1.9 미착수). 테스트 164개 통과, tsc 0. 플랜은 58티켓으로 개정됨(2026-07-14 사용자 결정 2건 반영).
...
2. **T0.3 라이브 DDL 적용 — 사용자 승인은 이미 받았음(2026-07-14).** 먼저 `mcp list_projects`로 **atockorea 프로젝트(`cghyvbwmijqpahnoduyv`)가 보이는지 확인**(2026-07-14 원격 세션은 Supabase MCP가 타 계정(Kursoflow)이라 적용 불가했음 — 같은 함정 주의). 보이면 `supabase/migrations/20260714090000_tour_room_v1_core.sql` → `20260714091000_tour_room_v1_rls_realtime.sql` 순서로 `apply_migration` + `get_advisors`(security/performance) 재실행해 신규 경고 0 확인. 안 보이면 이 단계는 건너뛰고 사용자에게 보고만.
3. 마스터 플랜을 이 순서로 읽는다: **§0(요약) → §B(결정 D-1~D-10) → §O(확정 사양) → §F(WBS에서 `✅` 없는 첫 티켓)**.
4. 다음 미완 티켓 순서: **T1.9(E2E — playwright 인프라 신설 필요) → T1.11 잔여(로비 전용 뷰; QR·취소 revoke는 T5.1과 함께) → Wave T2(음성 — 개정된 T2.2 확인 필수)**.
