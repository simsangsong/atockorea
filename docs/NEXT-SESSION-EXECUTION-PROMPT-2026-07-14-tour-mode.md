...
**개발 브랜치:** `claude/tour-mode-iql6ho` (플랜 문서 브랜치 `claude/tour-mode-dev-iaa0b8` 머지 포함; 원격 세션은 각자 지정 브랜치에서 이 브랜치를 머지해 이어가고 §F 완료 표시를 갱신한다)
**현재 상태:** Wave T0 구현 완료(T0.1~T0.2, T0.4~T0.9 — §F에 커밋 해시 기록). **T0.3 라이브 DDL은 사용자 승인 대기. 다음 = T0.3 → Wave T1.**
...
3. §F에서 `✅` 표시가 없는 첫 티켓부터 착수. 현재 첫 미완 티켓 = **T0.3(라이브 적용, 사용자 승인 필요)** → 그다음 Wave T1.
