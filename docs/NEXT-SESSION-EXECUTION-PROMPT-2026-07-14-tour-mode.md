...
**현재 상태 (2026-07-15):** **코드 트랙 전체 완료 — T0~T7 + T8.1(하드닝).** T0~T2 = PR #307, T3~T5 = PR #308 머지(플래그 OFF), T6(가이드 콘솔·팬아웃·공지·탑승체크)·T7(관제센터·SOS)·T8.1은 브랜치에 완료. 테스트 290 그린 / E2E 그린 / tsc 0 / advisors 신규 0. **잔여 = 사람 게이트 3건:** ① §I-4 실기기 리허설 ② 파일럿 스팟 좌표 검수(`docs/tour-mode-pilot-spot-checklist-2026-07-14.md`) ③ T8.2 파일럿 런칭(env 체크리스트 = `docs/tour-mode-hardening-T8-2026-07-15.md` §5). 코드 백로그(§H 이월): O-8 수동 스팟 트리거, O-12 µ$ 적산, 관제 미읽음 커서, 관제 임베드 지도.
...
4. 다음 미완 티켓 순서: ~~T1.9~~(✅ 83c79209) → ~~T1.11 로비 뷰~~(✅ 8c7c30d6) → ~~Wave T2~~(✅ 5298db9d·df44bba0·ee8b5f4a — 서버 stt/tts/captions + 클라 푸시투토크/래더/자막) → **Wave T3(위치공유·지도, §F T3.1부터)**. T1.11 ⑧ QR·취소 revoke는 T5.1과 함께.
