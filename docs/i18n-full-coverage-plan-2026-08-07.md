...
| ✅ **S14a** | de `suwon-…-gwangmyeong-cave-…` 9 unit | 35,947자 | **끝** | **1행 발행** |
| **S14b~** | 8슬러그 × 4로케일 잔여 — **31행** | — | — | — |

🔴 **「슬러그군 × 한 로케일」 배치가 실제로 값을 했다 — 숫자로 확인됐다.**
수원 3종의 첫 슬러그를 독일어로 끝내고 나머지 둘을 **재추출**하니 파이프라인 TM 이 이렇게 찼다:

| | TM 자동 | 손번역 고유 | 손번역 분량 |
|---|---|---|---|
| gwangmyeong-cave (1번째) | 156/430 | 245 | 32,739자 |
| folk-village (2번째) | **242/415** | 171 | 31,558자 |
| waujeongsa (3번째) | **186/390** | 196 | 31,596자 |
...
✅ **S14a 도 끝났다** — de `suwon-…-gwangmyeong-cave-…` 발행(9 unit / 430 세그먼트, fail 0).

🔴 **다음은 de `suwon-…-folk-village-…` 다. 입력은 이미 재추출돼 있고 TM 이 242/415 채워져 있다.**
손번역 고유 171건 · 31,558자. 그다음이 de `suwon-…-waujeongsa-…`(186/390 · 196건 · 31,596자).

배치는 **슬러그군 × 한 로케일**로 잡아라(§6-3′ 에 실측표). 권장 순서:
**① 수원 3종(de, 1/3 완료) → ② 설악 2종(de) → ③ 부산 소그룹(de) → ④ 서울근교+부산전세(de) → fr·it·ru 반복.**
⚠ **한 슬러그를 끝낼 때마다 `verify` 를 돌리고 다음 슬러그를 재추출하라** — 그래야 TM 이 넘어간다.
