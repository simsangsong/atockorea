...
### ✅ 오픈 완료 (2026-08-08, 사장님 지시 — PR #812)

**`TOUR_PRODUCT_FALLBACK_URL_LOCALES` 를 `[]` 로 비웠다.** de·fr·it·ru 가 언어 토글에서
실제로 서빙된다. 위 문단은 오픈 전 기록으로만 읽어라.

🔴 **오픈하면서 드러난 것 — 이 트랙이 스스로 만든 함정 두 개:**

① **「UPDATE 금지」가 결함을 얼리고 있었다.** `apply.ts` 는 INSERT-only 라 **이미 발행된 행의
오역·낡은 코스를 고칠 수단이 아예 없었고**, 그래서 아래 부채 ①②가 「손대지 않음」으로 적혀 있었다.
→ `--overwrite`(upsert) 추가. **비가시 스테이징 시절의 안전장치를 오픈 후에도 규칙으로 들고 있지 마라.**

② **`TourProductPageLocale` 을 10로케일로 넓히면 tsc 가 52개 터진다** — 전부 한 부류다
(6로케일 고정 표 23개). 컬럼 4개를 손으로 채우지 말고 **`TourProductBaseLocale` +
`toTourProductBaseLocale()`** 로 좁혀라. 챗봇 라벨·정적 번들처럼 **6로케일이 정본인 곳만** 좁힌다 —
🔴 **상품 페이지 콘텐츠 경로에 쓰면 오픈이 그대로 되돌아간다.**

### ✅ 낡은 코스 도태 (2026-08-08, PR #812) — 발행된 행이 은퇴 코스를 설명하고 있었다

| 슬러그 | 로케일 | 무엇이 틀려 있었나 |
|---|---|---|
| `jeju-eastern-unesco-spots-day-tour` | de · fr · it | **성산 먼저 · 만장굴은 「악천후 대체」**. 라이브는 만장굴 개장 직후 → 성읍 → 점심 → 성산 → 해녀 → 함덕. **ru 는 이미 맞아서 안 건드렸다** |
| `busan-cruise-shore-excursion-bus-tour` | de · fr · it · ru | 정차 **11곳**(라이브 12) · 라이브에 없는 「송도 해수욕장」 · **08:30→19:15**(라이브 09:00→**17:00**) |

🔴 **부산 기항지가 더 위험했다** — 크루즈 손님은 출항 시각에 맞춰 움직이는데 **복귀를 2시간 15분
늦게** 말하고 있었다. 재번역 4,510세그먼트, 전부 손번역, 발행 후 DB 재조회로 확인.

🔴 **낡음은 매니페스트로 못 잰다 — 정차 이름을 눈으로 대조해라.** 영문 마커 정규식 · 산문 필터 ·
이미지 경로 비교, **세 가지 휴리스틱이 연속으로 틀렸다**(사진 큐레이션 리네임을 코스 변경으로 오판).

### 🔴 남은 부채 3건 — 전부 별도 티켓

① **추천 레일 카드·챗봇 라벨은 아직 영어** — 정적 번들이 6로케일 고정이라 그렇다.
② **여러 행이 예전 사진 경로를 들고 있다.** 파일은 살아 있어 깨지진 않고 **옛 사진이 나온다.**
전면 갱신 = 21슬러그×4로케일 ≈ **1만 세그먼트 재생성**.
③ **은퇴 상품 14슬러그** — 범위 밖(§0-3-1).

*(닫힘: it 오역 3건 중 `a Jeju` 는 **오역이 아니었다** — `headlineLine1`/`Line2` 2행 분할이라
붙여 읽으면 맞다. de 구행 7건 중 `jeju-eastern` 은 위 표에서 처리됐다.)*
