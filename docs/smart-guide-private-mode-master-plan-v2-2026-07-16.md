...
| **P-D3** | **드라이버 = 신규 토큰 scope `{scope:'tour-date', role:'driver', tourId, tourDate}` + 입장 시 차량번호 뒤 4자리 PIN 확인.** PIN은 `tour_bus_details`(기존 테이블) 참조. 손님 채팅·PII 접근 없는 전용 라우트 5개만 인가. 링크 유출 시 노출 범위 = 당일 일정 스톱명+시각뿐(연락처·이름 없음)이므로 PIN 수준으로 충분 | 오픈퀘스천 #3. 위험 표면 분석 결과 |
...
| driver | KO 5버튼 뷰만. 채팅·PII 접근 없음 | 신규 driver scope + 차량번호 뒤 4자리 PIN |
...
| **BRIDGE** | 정형 문구 양방향 + STT 딥링크 | STT 라우트, Composer 보이스 모드, 번역 캐시 | `phrase_templates` 시딩(~20문구), 문구 카드 UI |
...
| C7 | 드라이버↔손님 단독 상황 | BRIDGE 정형 문구 + ✏️기존 컴포저 보이스 모드 딥링크 | ◆ |
...
| D2 | free 스톱 콘텐츠 없음 | tier-3 null 템플릿 렌더(기존) + harvest 큐 적재 | ★ |
