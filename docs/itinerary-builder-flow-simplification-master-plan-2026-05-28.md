...
| **D12** | **`bookings.merchant_id` default for builder**: tour bookings inherit `tour.merchant_id`. Builder has no tour. Options: (a) NULL; (b) an "AtoC self" sentinel merchant_id from env (`ATOC_DEFAULT_MERCHANT_ID`); (c) hardcode from a constants file. | **(b) env sentinel.** Settlement / payout flow joins on `merchant_id` (verify: `/api/admin/orders/[id]/settle`); NULL would either break or silently route to ops-default. Env keeps it tunable without redeploy. | If settle endpoint doesn't actually need `merchant_id` (i.e., AtoC keeps 100% itself), NULL is fine and (a) wins — verify during Phase 2. |
