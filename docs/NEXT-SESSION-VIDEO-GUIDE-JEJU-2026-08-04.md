...
## §11 재인폭포 투어 가능날짜 (사장님 스크린샷 2026-08-04) — 🔴 상품이 아직 없다

**스크린샷에서 읽은 것(2026년 8월 캘린더, Klook):** 판매가 **US$54.55**, 통화 US$.
예약 가능일 = **6·8·10·13·15·17·20·22·24·27·29·31** → 전부 **월·목·토**.
(8/1 토, 8/3 월이 회색인 건 리드타임 컷오프 — 스크린샷 촬영일이 8/4다. 요일 규칙의 예외가 아니다.)
녹색 점(=departure confirmed)은 6·8·15·17·20·22 에 찍혀 있었다.

**🔴 우리 리포에 재인폭포 상품이 없다.** `jaein|재인|Yeoncheon|Hantan` 전수 검색 결과 걸리는 건
포천 상품(`pocheon-sanjeong-lake-herb-island-art-valley`)뿐이고, 그건 산정호수·허브아일랜드·아트밸리
3스톱이다(한탄강은 본문 언급만). 재인폭포는 **연천**, 한탄강 유네스코 세계지질공원 권역이다.

**가능날짜를 넣는 자리(코드 확인함):** `app/api/tours/[id]/availability/range/route.ts` 가
① `product_inventory`(날짜별 `is_available`·capacity·`price_override`)를 1차 소스로 읽고
② 그 위에 하드코딩 요일 규칙을 얹는다(현재 유일한 사례: `weekday===1 && isJejuEast` → 월요일 차단)
③ 인벤토리 행이 없으면 기본 capacity 50 으로 **열려 있다고 본다**.
→ 월·목·토만 열려면 **`product_inventory` 행을 그 날짜에만 넣는 방식은 안 통한다**(행 없는 날이 열림).
   화·수·금·일을 닫는 **요일 규칙**을 추가하거나, 전 날짜 인벤토리를 채워야 한다. 전자가 맞다.

**사장님 확인 필요:** ⓐ 재인폭포 상품을 **새로 만드는 것**이 맞나(스톱 구성·소요시간·픽업은
...
```bash
npm run tour:days -- --tour=<uuid 또는 slug> --days=mon,thu,sat --months=6          # 드라이런
npm run tour:days -- --tour=<uuid 또는 slug> --days=mon,thu,sat --months=6 --apply   # 반영
```

🔴 **먼저 고친 것 — 라이브 버그.** `app/api/tours/[id]/availability/range` 가 인벤토리를
`.eq('is_available', true)` 로 조회하고 있었다. 그러면 **닫아 둔 날짜 행이 결과에서 빠지고**,
바로 아래 "인벤토리 행 없음" 분기가 그 날짜를 **기본 정원 50 으로 다시 열어 버린다.**
즉 지금까지 운영자가 어떤 날짜를 닫아도 캘린더에는 그대로 열려 있었다. 필터를 제거했다 —
맵 생성부가 이미 `inv.is_available` 을 읽으므로 행만 남기면 닫힘이 그대로 반영된다.

스크립트가 지키는 것: ① 열 날짜뿐 아니라 **닫을 날짜도 행을 쓴다**(행이 없으면 열린 것으로 간주되므로
월·목·토만 넣으면 나머지 4일이 그대로 열린다) ② 기존 행은 `is_available` 만 갱신 — 운영자가 넣은
