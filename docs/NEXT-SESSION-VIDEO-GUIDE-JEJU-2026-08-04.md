...
## §7 추가(2026-08-04 후속): 표준 문법 확정 + 기존 3편 재단 — PC에서 마무리할 것

**표준 정본: `docs/video-guide-standard-grammar-2026-08-04.md`** (만장굴 문법 v1). 신규 8명소는 전부 이 표준으로.

기존 3편(감천·용궁사·성산일출봉)의 >2x 트랜짓 77개를 원격 세션에서 표준 §2로 재단 완료(구조 게이트 통과).
소스를 못 본 채 등간격으로 자른 것이므로 PC 세션에서:
1. 중간 슬라이스 내용 검수 — 어색한 프레임이면 슬라이스 위치만 이동(화면 시간 유지)
2. 폴라로이드 「정지+피사체 전체」 프레임 검수(표준 §3) — 세 편 다 구판 타이밍 그대로다
3. 재렌더: build.mjs → vfull.mjs --wide, -wide-review.mp4 로 사장님 컨펌
4. 만장굴 음악 베드 4곡 mp3 를 assets/audio/video-guide/ 에 복사(리포엔 없다), 감천·용궁사·성산도
   heedotrip 풀에서 본편보다 긴 곡으로 베드 교체 검토(현재 셋 다 구판 베드)
