...
- 트리거: 5s OR 스크롤 30% 선도착, 데스크톱 exit-intent 추가. 세션당 1회(`sessionStorage`).
- 억제: `useSession()` 세션 존재 / `localStorage.atoc_welcome_dismissed_at` 7일 / `atoc_welcome_submitted` / `status==='checking'` 동안 대기.
- UI: 데스크톱 `Dialog`(중앙 카드 480px, 이미지 좌+폼 우), 모바일 `Sheet side="bottom"`. quiet-luxury 톤(§6.1 그대로), 기존 투어 무드컷 재사용.
