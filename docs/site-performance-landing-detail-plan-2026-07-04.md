...
## 3.5 재측정 결과 (2026-07-04, PR #245·#247·#248·#249·#251 전부 프로덕션 반영 후)

**curl 실측 (한국, 웜):**

| 지표 | Before | After |
|------|--------|-------|
| 상세 CDN | MISS 매회 (no-store) | **HIT** (s-maxage 3600) |
| 상세 TTFB→문서완료 | 0.4s → **1.8~3.2s** | 0.11s → **0.13s** |
| 상세 HTML | 352~388KB (빈 셸 아님·flight만) | 428~495KB (콘텐츠 SSR 포함, gz 61~72KB) |
| 홈 HTML | 60KB **빈 셸**(CSR bailout) | 222KB **콘텐츠 SSR** |

**Lighthouse 모바일(4G 시뮬레이션, 동일 방법):**

| 경로 | LCP | FCP | 점수 |
|------|-----|-----|------|
| `/` | **21.3s → 6.5s** | 17.1s → 3.4s | 55 → 65 |
| 상세(jeju-grand) | **61.7s → 8.3~11.3s** (런간 편차) | 21.0s → 3.6~4.8s | 55 → 54~62 |

- 폰트: Pretendard 3.8MB 크리티컬 패스에서 소멸, 렌더 블로킹 리소스 0, Noto css2는 load 이후 로드.
- 남은 상위 항목: 상세 HTML 문서 자체(gz 72KB — T5 RSC 다이어트 후보), 시뮬레이션 4G 특성상 실사용자(p75)는 이보다 훨씬 양호할 것 — **Vercel Speed Insights를 3~7일 관찰해 T5 착수 여부 결정.**
