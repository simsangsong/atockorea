...
**상태:** T0~T2 머지(PR #307)+T3~T5 머지(PR #308) → **T6 가이드 콘솔 완료(aac73183)**. 잔여 = **Wave T7(관제센터·SOS) → T8(하드닝·파일럿)** + 사람 게이트 2건(내부 리허설 §I 실기기 — T6 코드는 게이트와 무관해 선행 구현함, 파일럿 스팟 좌표 검수). 구현 커밋은 세션 브랜치 `claude/tour-mode-iql6ho`.
...
- **T7.3** `SosButton` + 관제 SOS 파이프라인(목록 고정+사운드) + 어드민 알림 이메일 + **SOS 발신 시 1회성 현재 위치 자동 첨부**(위치공유 OFF 참가자도 동의 팝업 후 이 1건만 전송 — 길 잃은 손님을 가이드·관제가 역으로 찾아가는 경로) — AC: SOS→관제 표시 3초 이내, 가이드 콘솔·관제 지도에 SOS 위치 핀.
