...
**상태:** T0~T2 머지(PR #307)+T3~T5 머지(PR #308) → **T6 완료(aac73183) → T7 관제·SOS 완료(f96afc4b)**. 잔여 = **T8(하드닝·파일럿 — 사람 게이트: 내부 리허설 §I 실기기, 파일럿 스팟 좌표 검수, 파일럿 런칭 결정)**. 코드 트랙은 사실상 완료. 구현 커밋은 세션 브랜치 `claude/tour-mode-iql6ho`.
...
- **트랙 1 — 서명 토큰 링크(신규, 주 경로): 링크 클릭 = 즉시 룸. 로그인·회원가입·홈페이지 경유 일절 없음(§O-1).** `lib/tour-room/token.ts` — `quoteToken.ts` 패턴 복제.
  - **손님 토큰**: `{ scope:'booking', bookingId, role:'customer', displayName, iat, exp }`, `exp = tour_date 23:59 KST + 24h`.
  - **가이드 토큰**: `{ scope:'tour-date', tourId, tourDate, role:'guide', displayName, iat, exp }` — 예약 단위가 아니라 **투어일 단위 스코프**(콘솔·팬아웃 인가와 일치, §O-3). 계정 없는 현장 가이드 문제(§A-2 #9)를 토큰으로 해결.
  - URL: 손님 `/tour-mode/room/{bookingId}?rt={token}`, 가이드 `/tour-mode/guide?rt={token}`.
  - 발송 기록·폐기(revoke)를 위해 `tour_room_invites` 테이블에 token_hash 저장. 예약 취소·환불 시 자동 revoke + 룸 closed(§O-1 ⑧).
...
CREATE TABLE tour_room_participants (
  ...
  device_key uuid NOT NULL,                 -- 클라이언트 생성·localStorage 보관. 동명이인 충돌 방지(§O-4)
  tts_capable boolean,                      -- 기기 TTS 케이퍼빌리티 보고(§O-2)
  location_sharing boolean NOT NULL DEFAULT false,
  last_seen_at timestamptz,
  created_at/updated_at,
  UNIQUE(room_id, device_key)               -- (room_id, role, display_name) 아님 — "John" 2명 문제
);
```
...
| ★ `POST /api/tour-rooms/[bookingId]/location` | 15초 간격 핑을 이 API가 rebroadcast(위조 위치 차단, §G R-13) + 30초마다 스냅샷 upsert | 핑 15s=4/min, 리밋 6/min(여유분, §O-5) |
...
- **T1.4** `useTourRoomSession` + `/tour-mode` 진입 페이지(회원 리스트/토큰/게스트 3트랙) — AC: 플래그 OFF 시 안내만. ✅ 완료(edffd9e — 미들웨어 화이트리스트(§O-1 ①)·토큰 URL 위생(§O-1 ③)·룸 페이지 스켈레톤 포함)
...
- **T1.9** E2E: 브라우저 컨텍스트 2개(가이드 토큰/손님)로 왕복 채팅 — AC: playwright 통과. ✅ 완료(83c79209) — 레포 최초 playwright 인프라 신설(config + webServer 자동기동 + 라이브 DB 시드/정리 global setup·teardown, `npm run e2e`). 스펙: 다이렉트 입장·토큰 주소창 제거(§O-1 ③)·가이드↔손님 텍스트 왕복·퀵답장 프리셋 전달, 콜드/웜 모두 통과. 부수 수정: 전역 웰컴 쿠폰 팝업이 투어룸을 덮는 결함 발견 → /tour-mode 경로 억제(§O-1 ②).
...
- **T1.11** 다이렉트 진입 하드닝(§O-1 전 항목) — middleware 화이트리스트, standalone 경량 레이아웃, `replaceState` 토큰 위생, localStorage 재입장, 인앱 웹뷰 탈출 배너, 로비/진행/종료 3상태, dispatch QR, 취소 revoke 훅 — AC: §O-1 ①~⑧ 각각 시나리오 테스트, 비로그인 시크릿 창에서 링크 1클릭 입장. 🟡 부분 완료(53a98d7 → 8c7c30d6) — ①②③④⑤⑦ 구현·테스트 완료, ⑥ 완료(로비 뷰 `LobbyCard`: D-day 카운트다운+픽업+채팅 힌트 / 종료 뷰 `EndedCard`: 읽기전용 안내+분실물 신고 mailto, 5로케일 테스트, 8c7c30d6), ⑧ QR·취소 revoke만 T5.1과 함께 잔여.
...
- **T6.1** `broadcast` API (tour_id+date 전 룸 팬아웃, 트랜잭션 실패 시 부분성공 보고) — AC: 룸 10개 팬아웃 통합테스트. ✅ 완료(aac73183) — 자유텍스트는 번역 1콜 공유, 부분실패 룸별 보고(전달분 롤백 없음), 프리셋/공지는 LLM 0회. 팬아웃 규모 통합테스트는 목 DB 3룸+부분실패 케이스로 고정.
- **T6.2** `/tour-mode/guide` — 예약 그룹 통합 피드(룸별 색 태그), 개별/전체 발신 전환 — AC: 가이드 토큰만 접근. ✅ 완료(aac73183) — overview API(토큰/어드민만, 403 테스트)+콘솔(색태그 룸카드·통합피드·15s 폴링·URL 토큰 스크럽). 개별 발신은 룸카드 탭→해당 룸 진입(같은 토큰)으로 제공; 룸내 실시간 피드는 기존 룸 화면 재사용.
- **T6.3** 집합 공지 UI — 시간·장소 입력 → 기존 `meeting_notice_sent` 재사용 + 손님측 카운트다운 배너 — AC: 로케일별 시간 포맷. ✅ 완료(aac73183) — **설계 조정:** spot 종속(meeting_notice_sent는 spotId 필수) 대신 팬아웃 notice(kind='meeting_notice', 동일 zero-LLM 템플릿) 사용. NoticeBanner가 Intl로 로케일 시간 포맷(테스트).
- **T6.4** 인원 체크(탑승 확인) — 손님 "탑승했어요" 버튼 → 가이드 콘솔 집계 — AC: `metadata.kind='onboard_ack'` 메시지로 구현(스키마 무변경). ✅ 완료(aac73183) — 픽업 보드 원탭 버튼(5로케일, ack 후 ✓ 칩), 콘솔 탑승 N/M 집계+미탑승 명단.
- **T6.5** 자유시간 타이머 — 가이드 "자유시간 N분 / 집합 HH:MM · 장소" 설정(T6.3 집합공지 확장, `metadata.kind='free_time_timer'`) → 손님 카운트다운 배너 + 10분/5분 전 인앱 알림(배너+진동+`speechSynthesis` 낭독 옵션) → 미복귀자(탑승 ack 없음) 리스트를 가이드 콘솔에 표시 — AC: KST 고정, 화면 복귀 시 서버 기준으로 남은 시간 재동기화, 타이머 취소/연장 가능. ✅ 완료(aac73183) — 카운트다운이 KST 벽시계 타깃에서 파생(복귀 재동기화 내재), 10/5분 경고 1회씩(진동+배너 펄스+autoRead 시 기기 낭독), 최신 공지 우선(연장)·취소·만료 로직 유닛 고정.

### Wave T7 — 관제센터 【3티켓】
- **T7.1** `tour-ops/rooms` API + admin Postgres-Changes 구독 — AC: 활성 룸·미읽음·SOS 집계 정확. ✅ 완료(f96afc4b) — 날짜별 룸+참가자+메시지수+SOS 핀 정렬(집계 테스트), 콘솔이 Postgres-Changes INSERT 구독(M-6 정책·publication)+20s 폴 백업. '미읽음'은 읽음커서 스키마가 없어 메시지수/최근메시지로 대체(백로그 §H).
- **T7.2** `/admin/tour-ops` — 룸 목록→상세(피드+지도+참가자), admin 발신, 전체 공지 — AC: 어드민 셸 규칙(프리미엄 모바일) 준수. ✅ 완료(f96afc4b) — 모바일 퍼스트 카드, 확장 피드+관제 발신(자동번역), SOS 위치 딥링크·주문상세 링크. 지도는 SOS 좌표 딥링크로 대체(임베드 지도는 §H), 전체 공지는 T6.1 broadcast(admin 경로) 재사용.
- **T7.3** `SosButton` + 관제 SOS 파이프라인(목록 고정+사운드) + 어드민 알림 이메일 + **SOS 발신 시 1회성 현재 위치 자동 첨부**(위치공유 OFF 참가자도 동의 팝업 후 이 1건만 전송 — 길 잃은 손님을 가이드·관제가 역으로 찾아가는 경로) — AC: SOS→관제 표시 3초 이내, 가이드 콘솔·관제 지도에 SOS 위치 핀. ✅ 완료(f96afc4b) — 원샷 위치 동의 시트(공유 OFF여도 이 1건만, 거부 시에도 SOS 발신), zero-LLM 5로케일 템플릿+룸 브로드캐스트+관제 메일(맵 링크), 관제 핀 고정+2톤 사운드. 3초 표시는 Postgres-Changes 실시간 경로(실측은 리허설).

### Wave T8 — 하드닝 · 검증 · 롤아웃 【2티켓】
- **T8.1** 하드닝 스윕 — §G 리스크 카탈로그 + **§O 애드엔덤 전 항목** 재점검 체크리스트 실행, Realtime 동시접속 쿼터 산정표(§O-10), 레이트리밋 튜닝, 로드 리뷰(룸 30개×참가자 10명 시뮬), `get_advisors` 최종 — AC: 체크리스트 100%. ✅ 코드측 완료(문서: `docs/tour-mode-hardening-T8-2026-07-15.md` — §O 14항목 표, 쿼터 산정표, 레이트리밋 표, advisors 신규 0, 런칭 전 env 체크리스트, 파일럿 메트릭 쿼리). 미결 2건은 §H 이월: O-8 수동 스팟 트리거, O-12 µ$ 적산 카운터.
- **T8.2** 파일럿 롤아웃 — 플래그 ON(파일럿 투어 한정), 실기기 필드 테스트 스크립트, 메트릭(입장률·메시지량·STT 실패율·번역 지연 p95) 대시보드 쿼리 — AC: 파일럿 리허설 1회 완료 보고. 🔒 사람 게이트 — 메트릭 쿼리는 하드닝 문서 §6에 준비됨. 선행: 실기기 리허설(§I-4)+스팟 좌표 검수+`TOUR_ROOM_TOKEN_SECRET` 등 env 설정 후 플래그 ON.
...
| R-14 | **배터리 급감** — 고빈도 GPS+지도+소켓 | 15초 스로틀(§O-5)+이동 10m 필터+지도 탭 비활성 시 렌더 정지 | T3.2/T3.3 |
...
| R-18 | **LLM 비용 폭주**(전 메시지 5로케일 번역+TTS) | §M 전략 전체 — 참가자 로케일만 번역(D-8), TTS 캐시(D-6)+1,000자 캡, 퀵답장 프리셋 무번역, 일일 예산 가드(§O-12) | T1.3/T2.3 |
...
5. ~~"버스로 돌아가기" 도보 안내~~ → **v1로 승격, T3.3에 편입**(2026-07-14).
...
**P3:**
13. 투어일 단일 그룹룸으로 구조 개편(§K-1 결정 후).
14. 가이드 평점/피드백 수집.
15. 관제 자동 이상감지(장시간 정차, 경로 이탈 알림).
...
## §O. 최종 리뷰 애드엔덤 (2026-07-14) — 발견 결함·보완 확정 사항

> 착수 전 적대적 최종 리뷰 결과. **아래 항목은 전부 확정 사양**이며 해당 티켓의 AC에 포함된다.

### O-1. 다이렉트 진입 — "링크 = 곧바로 룸" 보장 8원칙 (T1.11)
1. **무가드 라우트**: `/tour-mode/room/[bookingId]`·`/tour-mode/guide`는 로그인 가드·리다이렉트 대상에서 명시 제외(middleware 화이트리스트 확인). 회원가입 유도 UI 금지.
2. **전용 경량 레이아웃**: 글로벌 네비·푸터·사이트 셸 없이 standalone 레이아웃. 지도·오디오 모듈은 탭 진입 시 lazy-load — 3G에서 첫 채팅 화면 3초 내.
3. **URL 토큰 위생**: 첫 로드에서 `rt` 검증·룸세션 발급 직후 `history.replaceState`로 주소창에서 토큰 제거(방문기록·화면공유·공유시트 유출 차단). 토큰 자체는 만료 전까지 유효 — 링크 재클릭 재입장 가능.
4. **재입장 무마찰**: 룸세션+`device_key`를 localStorage 보관 → 브라우저 재시작·앱 전환 후에도 링크 재클릭 없이 복귀.
5. **인앱 웹뷰 탈출**: 카카오톡·인스타·Gmail 인앱 브라우저는 마이크·GPS·TTS가 제한됨 — UA 감지 시 "Safari/Chrome에서 열기" 배너(Android는 `intent://` 시도, iOS는 메뉴 안내). 채팅 열람 자체는 웹뷰에서도 동작.
6. **수명 전 구간 상태**: 룸 오픈 전 클릭 → 로비(D-day 카운트다운+픽업 정보+가이드 인사), 진행 중 → 룸, 종료 후 → 읽기전용+분실물 신고. "에러 화면"이 나오는 시점이 존재하지 않는다.
7. **이메일 스캐너 안전**: 링크 GET은 무부작용(참가자 등록은 사용자 상호작용 후 POST) — Outlook SafeLinks/Gmail 프리페치가 아무것도 소모하지 않음. 가이드 dispatch 메일에 **QR 코드** 동봉 — 현장에서 손님 폰으로 즉석 스캔 입장.
8. **취소 연동**: 예약 취소/환불 훅에서 invites revoke + 룸 closed — 취소된 예약의 링크는 정중한 안내 화면.

### O-2. TTS 미지원 기기 — 3단 폴백 래더 (T2.9)
1. **1단 — 기기 `speechSynthesis`**: 사용 전 케이퍼빌리티 검증 필수 — `getVoices()`는 비동기 레이스가 있으므로 `voiceschanged` 이벤트 대기 후 **해당 로케일 음성 존재까지 확인**(API 존재 ≠ 음성 존재. Android는 언어팩 미설치 기기 흔함, 인앱 웹뷰는 voices 빈 배열).
2. **2단 — 서버 TTS 폴백**: 1단 불가 시 기존 M-5 캐시 경로(`GET /tts`)로 자동 전환. 같은 메시지·로케일은 **룸 전체가 캐시 공유**(미지원 기기가 3명이어도 생성 1회) + HTML5 `Audio` 재생이라 iOS 무음 스위치에서도 미디어 볼륨으로 출력. 예산 가드·participant당 재생 리밋 적용.
3. **3단 — 오디오 전면 불가**(권한 거부·웹뷰): 자막·텍스트가 항상 1차 채널이므로 기능 상실 없음. "음성 사용 불가" 배지만 표시.
- 감지 결과를 `tour_room_participants.tts_capable`에 보고 → 가이드 공지 발신 시 미지원 참가자가 있으면 서버가 해당 로케일 TTS를 발신 시점에 1회 선생성(수신 지연 제거).

### O-3. [설계 결함 수정] 가이드 토큰 스코프
- 기존 설계(per-booking)로는 가이드가 룸 8개에 각각 다른 링크로 입장해야 하고 팬아웃 API 인가가 성립 안 함. → 가이드 토큰을 `{tourId, tourDate}` 스코프로 변경(D-2 반영 완료). 콘솔·broadcast·captions 인가가 이 스코프로 판정.

### O-4. [설계 결함 수정] 참가자 UNIQUE 키
- `UNIQUE(room_id, role, display_name)`은 동명이인("John" 2명)·이름 변경 시 다른 기기와 병합되는 결함 → **클라이언트 생성 `device_key`(uuid, localStorage)로 UNIQUE(room_id, device_key)** (§C M-1 반영 완료).

### O-5. 위치 핑 정합
- 핑 15s(=4/min) / API 리밋 6/min(재시도 여유) / 스냅샷 30s로 수치 확정(기존 4/min 리밋과 10s 핑의 모순 제거).

### O-6. 이중 전달·유실 방지 프로토콜 (T1.5 AC에 편입)
- 클라이언트는 message `id` 기준 dedupe(Broadcast+SSE 폴백 동시 수신, 재연결 중복 대비).
- `visibilitychange` 복귀 시 마지막 커서 이후 REST 재동기화(백그라운드 중 Broadcast 유실 보정) — 잠금해제 순간 밀린 메시지 복원.

### O-7. 서버리스에서의 Broadcast 송출
- API 라우트는 WebSocket 클라이언트를 만들지 않고 **Supabase Realtime HTTP Broadcast 엔드포인트**(REST)로 송출 — 콜드스타트·소켓 수명 문제 원천 제거(`lib/tour-room/realtime.ts` 구현 방식 확정).

### O-8. 지오펜스 보강 (T4.4 AC에 편입)
- 반경 중첩 스팟: 최근접 1건만 판정 + 도착 간 120초 쿨다운.
- GPS 거부/불가 손님 대비 이중 수동 경로: ① 손님 "여기 도착했어요" 버튼(스팟 리스트에서 선택) ② **가이드 콘솔의 수동 스팟 트리거**(투어일 전 룸 팬아웃) — 위치권한 0%인 그룹에서도 자동 해설 기능 성립.

### O-9. MediaRecorder 미지원(구형 iOS 등)
- 감지 후 음성 버튼 자동 숨김(텍스트·퀵답장 유지). 에러 다이얼로그 금지.

### O-10. Realtime 동시접속 쿼터
- Supabase 플랜별 동시 연결 한도 확인을 T8.1 체크리스트에 명시(파일럿 규모 손님 20명+가이드+관제 ≈ 25연결로 여유이나, 다투어 동시 운영 산정표 첨부).

### O-11. SSE 폴백의 서버리스 한계
- 라우트 `maxDuration` 명시 + 연결 55초 주기 자연 종료 → 클라이언트 `after` 커서로 자동 재연결(무한 스트림 가정 금지).

### O-12. 일일 AI 예산 가드의 실제 집행 방식
- 선언만 있던 `TOUR_ROOM_DAILY_AI_BUDGET`의 구현 확정: 기존 `lib/durable-rate-limit` 카운터를 `tour_ai_cost` 네임스페이스로 재사용, 호출별 추정비용(µ$)을 가중치로 적산. 초과 시: 번역=원문 게시 모드, 자막=Tier A 강제, 비전=일시 중단 안내.

### O-13. 토큰 시크릿 로테이션
- `verify`는 `TOUR_ROOM_TOKEN_SECRET` + `TOUR_ROOM_TOKEN_SECRET_PREV` 듀얼 검증 — 로테이션해도 이미 발송된 링크가 죽지 않음.

### O-14. LLM 세이프티 거부·부분 실패
- Gemini 세이프티 필터가 번역을 거부하는 케이스(욕설 신고 등) → 라우터가 2순위 프로바이더 재시도 → 최종 실패 시 원문 게시(R-6 확장). 자막도 동일.
