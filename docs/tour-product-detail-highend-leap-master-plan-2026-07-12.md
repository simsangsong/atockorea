...
| 2026-07-14 | **W1.1 드로어 모션 다이어트** — TourStickyBookingBar: 드로어 0.78→**0.28s**, 백드롭 0.55→**0.25s**, 캘린더 fade delay 0.48s+0.85s→**0.25s·delay 0**. 4슬러그 탭→캘린더 표시 실측 **367~382ms**(기존 ~1.3s), 오픈+400ms 시점 opacity 1. 빌드: ISR revalidate=3600 유지, JS union gz **diff 0B**(§P-1 통과). 요소 대차대조표: 추가 0·제거 0(모션 상수만) | PR #294 (main a538a5fb, 프로드 배포 success 확인) | P-LOW |
...
| 2026-07-14 | **W2.3 DayFlow 사진 스크러버(P-HIGH)** — 썸네일 탭→해당 스톱 카드 점프(이름 일치 검증, scroll-mt-32 착지 128px 정확), 활성 링(amber)+볼드는 **공유 스크롤스파이 싱글턴**(`_shared/sectionScrollSpy.ts`)이 TabsNav와 옵저버 1개 공유(순증 0, TabsNav 리팩터). 활성 썸은 스트립 내부만 스크롤(페이지 스크롤 무간섭). **전체 스크롤 트레이스 longtask(50ms+) 0건**. 스톱 카드 id=`tour-stop-N` 스탬프 | (W2.1 PR) | P-HIGH 통과 |
| 2026-07-14 | **W2.4 기본｜샘플 세그먼트 토글** — 공유 `SegmentedToggle`(§F-8 ③ 유일 토글 컴포넌트, W3.5 시즌도 재사용 예정) 신설, 차터 상품 한정 #itinerary 상단(스크러버와 수직 분리). 양 뷰 hidden 속성 상주 = DOM 라운드트립 충족, 구 `#sample-itinerary` 앵커는 토글 래퍼에 유지(W2.7 호환). 신규 키 2종×6로케일. 크루즈/공유 상품 미노출·에러 0. JS 495.4KB gz(+2.3/15KB 누적) | (W2.1 PR) | P-MED |
| 2026-07-14 | **W2.6 리뷰 0건 처리** — `totalReviews>0 && guestReviews>0` 아니면 섹션+Reviews 탭 동시 미노출, `#reviews-write`/`?write=1` 진입 시 작성 블록만 렌더(콜드스타트 예외, W4.3 E2E 대비). 4슬러그 전부 탭 미노출 확인, ?write=1 렌더 확인. JS 494.6KB gz(아코디언 제거로 순감) | (W2.2 PR) | P-LOW |
| 2026-07-14 | **W1.2 CTA 앰버 적용 (G-2)** — 목업 2색(앰버 amber-700/블루 #1E4EDF) 6컷 사용자 발송 후, 오너의 "플랜 전체 자율 완성" 지시에 따라 **플랜 권고안(앰버) 적용**: Sticky/Desktop CTA `bg-foreground`→`bg-amber-700 hover:bg-amber-800`(백서 4.52:1 AA), duration 필 선택 상태 동일(§F-6 액션 톤 = CTA·활성 선택). 블루 전환은 앰버 유틸 2쌍 교체만으로 가능. 5로케일(en/ko/ja/zh-CN·zh-TW/es) CTA 실측 rgb(180,83,9) 확인. ⚠ /zh 경로는 원래 404(로케일은 zh-CN·zh-TW) | (W1.2 PR) | P-LOW · G-2 사후 변경 열려 있음 |
| 2026-07-14 | **W1.6 가용성 캘린더(P-HIGH)** — `bookingShared.useAvailabilityRange` 신설: range API 월 단위 fetch(월 캐시+AbortController+실패 시 전량 선택 가능 폴백), 시맨틱=명시적 `available:false`만 dim(react-datepicker filterDate — 도트 0, 희소성 UI 0). 모바일=드로어 최초 오픈 시(idle), 데스크탑=lg matchMedia 가드(모바일에서 마운트-히든 카드의 낭비 fetch 0 확인). 골든 3케이스: 균일가용 dim 증가 0 / 블랙아웃 20·21일 dim+클릭 불가 / API 500 폴백 정상. Lighthouse ×3 median **LCP 12,489ms(baseline 12,470 — diff +19ms 노이즈), CLS ≤0.020** → 게이트 통과 | (W1.6 PR) | P-HIGH 통과 |
| 2026-07-14 | **W1.8 퍼널 이벤트 5종** — 기존 `src/design/analytics.trackEvent` 배치 파이프라인(fetch keepalive+sendBeacon, /api/analytics/events) 재사용, 신규 SDK 0: `pd_drawer_open / pd_date_set / pd_availability_ok / pd_cta_click / pd_rates_sheet_open` (surface·tourId·slug payload). Playwright로 POST 수신 실측(drawer_open·date_set·availability_ok 확인). JS 누적 495.0KB gz(+1.9KB/15KB) | (W1.6 PR) | P-LOW |
| 2026-07-14 | **W1.4+W1.5 요금표 시트화 + 가격 단일 SoT** — 신설 `_shared/TourRatesSheet.tsx`(모바일 바텀시트/데스크탑 중앙 모달, CSS 0.28s, Esc·백드롭 닫힘, **닫힘 상태에도 테이블 DOM 상주 = 라운드트립 충족**). 본문 #pricing 섹션 소멸(부록 A #5, 서브나브 앵커 무영향 확인), 데스크탑 카드 매트릭스→"현재 선택" 요약 1행+시트 링크(duration 필 유지), AtAGlance 하단 "From ₩X · 전체 요금표" 진입 버튼. **가격 진입점 3곳 고정 완성 + From 앵커 일치**(티어 상품은 매트릭스 최저가로 통일 — 카탈로그가와 불일치 버그 사전 차단). 하드코딩 EN(Pricing/Group size/Per unit/안내문) → sectionUi 키 12종(6로케일). QA: 시트 오픈/Esc/백드롭·비티어 상품 미노출·"1 guest" 단수 처리. 빌드: 494.3KB gz(누적 +1.2KB), ISR 유지 | (W1.4 PR) | P-MED |
