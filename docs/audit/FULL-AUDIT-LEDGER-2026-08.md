...
5. ⚠ **새 판정기의 첫판은 216콤보 중 128을 실패로 찍었고 전부 오탐이었다.** 두 부류를 면제했다 —
   ① **SVG 내부**(스킨 풍경은 한 장의 풀블리드 아트워크다. `<svg>` 가 레이아웃 상자이고 그 안의
   `path`/`g`/`ellipse` 기하는 레이아웃이 아니다 — 루트를 재고 그림을 재지 말 것)
   ② **잘라내거나 스크롤하는 조상**(위에서 숨기면 손님은 픽셀을 잃지 않는다 — 그게 버그와 설계의 차이).
   면제 이유를 면제하는 자리에 적었다.
6. **최종: 히어로 216콤보 전부 판정 · 실패 0 · unreachable 0 · keep-all 비용 294**(보고만) [실측].
...
## FA-028 · **P1(오정보)** — 사장님이 받은 보고서의 헤드라인이 본문과 반대를 말했다

**출처: 사장님이 보내주신 2026-07-30 18:00 보고서 캡처.** 상단 배지와 제목은 **"이상 없음"**인데,
같은 메일 ⑤번 목록에는 **오토파일럿 미처리 제안 23건이 빨갛게, 체크 안 된 채로** 들어 있었다.
폰으로 배지만 보는 사람에게 23건은 없는 것과 같다 — 이 감사가 쫓던 "아무것도 안 재면서 초록"의
**손님용이 아니라 사장님용 버전**이다.

**원인:** 같은 숫자를 두 곳에서 셌다. 배지·제목의 `attentionTotal` 은 7개 필드를 **손으로 더하면서
`autopilotOpen` 을 빠뜨렸고**, ⑤번의 "이상 없음" 판단(`clean`)은 포함했다.
이 파일의 다른 주석이 이미 경고하던 형태다 — **"같은 숫자를 두 곳에서 계산하면 반드시 어긋난다"(§H-4).**

🔴 **그리고 내가 몇 시간 전에 넣은 FA-014 `arrivals` 도 같은 함정에 있었다** — `clean` 에만 넣고
`attentionTotal` 에는 안 넣었다. 투어가 돌고 도착이 0인 날, 목록은 띄우고 배지는 "이상 없음"이라
말했을 것이다. **결함을 고치면서 같은 결함을 하나 더 만들 뻔했다.**

**수정:** 목록·합계·배지·제목·"이상 없음" 배너가 **`attentionItems()` 하나**를 읽는다.
게이트 3건(`render.test.ts`): 오토파일럿이 있으면 배지·제목·목록이 모두 요주의 · 합계 = 목록의 합 ·
데이터층 `clean` 과 렌더 판단이 4가지 상태 변형에서 일치. **뮤테이션: 손으로 더하던 합계로 되돌리면 1 failed** [실측].

### FA-029 · P2 — 그 23건은 **전부 지나간 투어일**이었다

라이브 확인: 23건의 `tour_date` 가 7/26~7/30, 전부 오늘 이전이거나 오늘. 제안은 날짜가 지나도 스스로
닫히지 않으므로 **이 숫자는 자라기만 한다** — 자라기만 하는 숫자는 며칠 만에 안 읽힌다.
이 큐의 값은 설계 주석이 직접 적어놨다: *"내일·모레 투어인데 안내가 안 나갔다"*. 지난 날짜 제안은
정의상 **사후 보고**이고, 그건 이 크론이 피하려던 바로 그것이다.

**수정(스키마 변경도 데이터 변형도 없이):** DB CHECK 가 `suggested/done/dismissed` 만 허용하므로
새 상태를 만들지 않았다. 대신 **셈의 의미**를 고쳤다 — 요주의는 `tour_date >= 오늘`(+날짜 없음)만 세고,
지난 것은 `autopilotStale` 로 **따로 조용히 보고**한다(지우지 않는다 — 그 룸이 가이드 없이 돌았다는
기록 자체가 데이터다). 깨끗한 날 배너 옆에도 실린다(FA-014와 같은 이유: 안 보이면 영영 안 치운다).

**실측 효과 [라이브, 오늘 기준]:** 23건 → **조치 가능 5 · 지난 날짜 18**.
같은 메일이 "이상 없음"이 아니라 **"요주의 5건"** 이 되고, 18건은 큐 위생 줄로 뜬다.
