...
## A8 수정 결과 — 26건 중 20건 수정 · 3건 운영/결정 · 3건 관찰 종결
...
| FA-015 | P2 | ChatFeed 발신 행 `min-w-0` + `max-w-full sm:max-w-[76%]` — **예방 수정, 미검증** | ↓ 아래 정직 표기 |
...
### 🔴 FA-015 — 수정은 넣었지만 **검증하지 못했다** (정직 표기)
...
**수정하지 않고 남긴 3건 — 이유가 있다:**
- **FA-014 (P1, 운영)** 도착 캡슐 실발동 0. 코드는 정상(K4 주행 0→2행). 코드로 고칠 것이 없고,
  현장 원인(실기기 GPS·[도착] 미사용)은 **실기기 리허설 사람 게이트**에서만 판별된다.
  **다음 세션 최우선 후보:** "오늘 도착 0건"을 운영이 당일 알 수 있는 텔레메트리 1줄.
- **FA-024 (P3, 사장님 결정)** `ConciergeEntryRow`가 렌더되지 않는다. 컨시어지 문은 이미 둘
  (홈 타일 `smart-guide`·셸 헤더 `concierge-open`)이고 **채팅 탭에는 없다** — 이 컴포넌트가
  설계된 자리다. 지울지/채팅 탭에 붙일지는 **제품 결정**이라 감사 수정 웨이브에서 손대지 않았다.
  붙이는 쪽이면 `ChatFeed` 컴포저 위 1줄. 임포터 0 모듈 4건(홈 트랙 잔해 3 + `client-sse`)도 같은 칸.
- **FA-010 (P3, 관찰)** CJK suspect 래칫 여유 0. 게이트는 통과하고 상한을 **올리는 것은 금지**(§E).
