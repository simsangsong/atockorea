...
|---|---|---|---|---|
| #5 메시지 전송 → 화면 반영 | 300 ms | 22.3 ms | **16.7 ms** | ✅ **PASS** (여유 13배) |
| #6 카드 탭 → 시트 열림(서랍) | 150 ms | **338.6 ms** | **17.6 ms** | ❌ **첫 탭만 FAIL** (2.3배) |

🔴 **n 이 작을 때 p95 를 그대로 읽으면 오독한다.** #6 의 p95(338.6)는 사실상 **첫 회 그 자체**다.
두 번째부터는 17.6ms 로 예산의 1/8 이다. 즉 **"서랍이 상시 느리다"가 아니라 "첫 탭이 느리다"** 이고,
