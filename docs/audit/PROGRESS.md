...
**→ §L L4 (Tier0 커버리지 루프) · 이어서 B1.5 큐 조치 · §L L3/L5 · A0.3/A0.4 → A1.0**

## 기준선

- main = `f2980160` (origin/main과 동일, 미푸시 0) — tsc 0 · `next build --webpack` ✓ · 204스위트 2118 green 재검증 완료
...
| 15 | §L L2 Tier1 응답 캐시 · L6 일일 예산 표면 | ✅ `38e9b915` · `e55a973e` (L3·L4·L5 미완) |
| 16 | A0.3 성능 베이스라인 · A0.4 로케일 하니스 · A4 코드 건강도 | ⬜ |

## 남은 것 (우선순위순)

1. **§L L4** Tier0 커버리지 루프 — `chat_messages`에서 Tier1로 샌 질문을 정규화·빈도 집계해
   Tier0 사전 후보를 뽑는 스크립트. **자동 추가 금지**(오답을 사전에 굳히면 되돌리기 어렵다) — 목록만
2. **B1.5** 파싱 실패·리뷰 큐에서 한 번에 조치 (기존 `OpsReviewQueueView`와 같은 엔드포인트로 수렴)
3. **§L L3** 프리워밍 — 투어 시작 게이트/모닝 브리핑 시점에 스팟 콘텐츠·다이닝 셀·번역 메모리 예열
4. **§L L5** 컨텍스트 다이어트 — Tier1 프롬프트 입력 토큰 실측 후 품질 무저하 상한
5. **A0.3** 성능 베이스라인 · **A0.4** 로케일 하니스 → **A1.0 커버리지 원장** → A1 전수
6. **A4** 코드 건강도 (A0.1 무관, 언제든 병행 가능)
7. 🟡 **머지 게이트 확대** — 표준 게이트 스위트 목록에 `__tests__/`가 빠져 있어 L0 회귀를
