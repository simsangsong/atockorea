...
- **[38]** A8 종합 캡스톤 (`docs/audit/A8-synthesis.md`) — S1~S6 채점: 🟢 **S5 회복력·S6 신뢰가 이번 감사로 목표 근접**
  ...
  **A0.2/A7.4 결정 확정**: 선택지1(정의 변경) — Tier0 비콘은 손님 트래픽+마이그레이션 비용이 이득 초과, L4가
  유출을 이미 잼 → A7.4를 "Tier1 유출 추세"로 재정의(비용 0). A8.2 우선순위 백로그(P1 아바타 1건 최상위) +
  A8.3 로드맵(🔴 **공유 훅 4종**으로 "형제가 아는 것" 5회 반복 구조 소거 · §G는 A5 뒤로 게이트). → (docs)
