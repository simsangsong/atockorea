...
**→ §K B0.4 (차량·좌석판·시작게이트를 그룹 기준으로 이관) — 스키마는 준비됨, 코드만 남았다**

## 기준선

- main = `c8e256db` (origin/main과 동일, 미푸시 0) — tsc 0 · `next build --webpack` ✓ · 204스위트 2118 green 재검증 완료
...
1. **B0.4/B0.5** 차량·좌석판·시작게이트를 그룹 기준으로 이관 + 회귀 3시나리오
   (스키마는 준비됨 — `ops_room_vehicles.group_id` 존재, 코드가 아직 `room_id`로 읽는다)
2. **B2.4** 2호차 분리 동선 (B0.4 선행 필수)
