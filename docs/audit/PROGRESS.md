...
**→ A4.1 (중복 진실 사냥) · 이어서 A0.3/A0.4 → A1.0 커버리지 원장 → A1 전수 · §L L5는 A7.2와 함께**

## 기준선

- main = `422e1fc8` (origin/main과 동일, 미푸시 0) — tsc 0 · `next build --webpack` ✓ · 204스위트 2118 green 재검증 완료
...
| 16 | A4.2 client/server 경계 정적 검사 | ✅ `422e1fc8` · `docs/audit/A4-code-health.md` |
...
1. **A4.1** 중복 진실 사냥 — B1-D3(집계 중복 금지)·B2.1b(정원 우선순위)가 실제로
   단일 지점을 지켰는지의 **사후 검증**이기도 하다
2. **A4.3~A4.6** 죽은 코드 · 테스트 공백 지도 · 타입 거짓말(`as`) · 사전존재 결함 청산
