...
**→ A1.1 (채팅 코어 6개) — 원장의 첫 티켓. 이어서 A1.2~A1.8 · A4.3~A4.5 · §L L5는 A7.2와 함께**

## 기준선

- main = `3640eb1e` (origin/main과 동일, 미푸시 0) — tsc 0 · `next build --webpack` ✓ · 204스위트 2118 green 재검증 완료
...
| 20 | A1.1~A1.8 손님 앱 전수 (65개) · A4.3~A4.5 · §L L5 | ⬜ |

## 남은 것 (우선순위순)

> **§K 확정 빌드 트랙(B0~B5)은 전부 완료됐다.** 아래는 감사(A)와 §L 잔여다.

1. **A1.1~A1.8 손님 앱 전수 (65개)** — 원장(`docs/audit/A1-coverage.md`)의 `판정` 칸을 채운다.
   빈 칸이 남아 있으면 A1은 미완이고, 파일이 새로 생기면 **테스트가 먼저 운다**.
