...
| `2026-06-24-14-tour-lessidealfor-backfill-3-tours.sql` | ⏳ pending | **AI-readability ("Not suitable for"):** fill `whyTourWorks.lessIdealFor` for the **3 tours** that were empty across all **6 locales** (18 rows) — busan plum-cherry, busan spring-cherry-Gyeongju, busan private car-charter. Tour-specific copy (season window / per-vehicle pricing / cruise timing). **IDEMPOTENT + NON-DESTRUCTIVE** — writes only when `lessIdealFor` is absent/empty (`jsonb_array_length … = 0`); never touches the 31 tours that already have it. Mirrors the static-bundle backfill committed alongside. Standalone. |
